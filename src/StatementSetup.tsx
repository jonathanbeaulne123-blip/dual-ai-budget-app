...
import { mergeStatementSource, newStatementSource, parseStatementExport, scanStatementPages, statementFileHash, statementFileKind, statementHistoryInput, statementAmountCents, verifyStatementAttachment } from './imports/statementSetup/intake.ts';
...
export function StatementSetup({ active = true, household, memberId, authUserId, view, onReviewHistory, onUseSuggestions, acceptedCoverage, onDone }: StatementSetupProps) {
  ...
  useEffect(() => {
    ...
    setLoaded(false); setScopeChosen(false); setBusy(false); setAttachment(null); setPages(''); setSelectedSuggestions([]); setAmountText({}); setSuggestionMessage(''); setError(''); setPreviewUrl(null);
  ...
  const invalidAmount = currentDraft?.rows.some(row => amountText[row.id] !== undefined && statementAmountCents(amountText[row.id]!) === null) ?? false;
  const sourceLabel = (accountRef: string) => {
    const source = currentDraft?.sources.find(item => item.checkpoints.some(point => point.accountRef === accountRef) || currentDraft.rows.some(row => row.sourceHash === item.hash && row.accountRef === accountRef));
    const last4 = source?.checkpoints.find(point => point.accountRef === accountRef)?.accountLast4 || currentDraft?.rows.find(row => row.accountRef === accountRef)?.accountLast4;
    return `${source?.name ?? 'Statement'}${last4 ? ` · account ending ${last4}` : ' · account identifier not read'}`;
  };
  const mappedAccountLabel = (accountRef: string) => household.accounts.find(account => account.id === currentDraft?.accounts.find(mapping => mapping.accountRef === accountRef)?.accountId)?.name ?? 'Choose the source account above';
  ...
  async function scan() {
    ...
    setBusy(true); setError(''); setAmountText({});
    try {
  ...
  async function discard() {
    ...
    try { await saves.current; await discardStatementDraft(scope); if (start !== generation.current) return; const next = emptyStatementDraft(scope); setDraft(next); draftRef.current = next; setSaved(false); setAmountText({}); setAttachment(null); setDiscarding(false); setError(''); setBusy(false); for (const item of attachments.current.values()) await item.pdf?.close(); attachments.current.clear(); }
  ...
  return <section className="statement-setup" aria-labelledby="statement-setup-title" aria-busy={busy}>
    ...
    <Whisper mode="line" className="statement-setup__hint">Choose Personal books before attaching a private statement.</Whisper>
    <Whisper mode="aside" id="statement.attach">PDFs open on this device; selected PDF pages and images go to the document scanner only when you choose Read pages. OFX/QFX exports are read locally.</Whisper>
    ...
    {attachment && currentDraft?.sources.some(source => source.hash === attachment.hash && (source.kind === 'pdf' || source.kind === 'image')) && <div className="statement-setup__reader"><strong>{attachment.file.name}</strong><label>Pages to read<input value={pages} placeholder={`All ${attachment.pdf?.pageCount ?? 1} pages`} disabled={busy} onChange={event => setPages(event.target.value)} /></label><Whisper mode="line">Use 1-3, 5 to select pages. Omitted pages stay visible for review.</Whisper><Whisper mode="aside" id="statement.pages">Reading again replaces this source’s extracted rows and their review choices.</Whisper><button type="button" className="primary" disabled={busy || !scopeChosen} onClick={() => void scan()}>Read pages</button>{busy && <button type="button" onClick={() => abortRef.current?.abort()}>Stop reading</button>}</div>}
