...
export function PlanBridgeEditor({ household, memberId, month, householdDraft, busy, onCommand }: {
  ...
  const savePrivate = (supersedesId?: string) => {
    ...
    void onCommand((current) => savePlanBridgeDraft(current, { ...(editing && privateDraft ? { id: privateDraft.id } : {}), monthKey: month, kind: "contribution", label,
  ...
  const cards = (rows: typeof decisions) => rows.map((row) => {
    ...
    return <article className="plan-bridge-card" key={row.id}>
      <div><strong>{row.label}</strong><span>{row.amountCents !== undefined ? formatCad(row.amountCents) : "No amount disclosed"}</span></div>
      <small>{row.state}{row.heldReason ? ` · ${row.heldReason}` : ""}{row.declineReason ? ` · ${row.declineReason}` : ""}</small>
  ...
  return <section className="plan-editor plan-bridge-editor" aria-labelledby="bridge-editor-title">
    ...
    {privateDraft && !editing ? <section className="plan-disclosure-review"><strong>Exact disclosure review</strong><dl><div><dt>Shared wording</dt><dd>{privateDraft.label}</dd></div><div><dt>Shared amount</dt><dd>{privateDraft.amountCents === undefined ? "No amount" : formatCad(privateDraft.amountCents)}</dd></div><div><dt>Never shared</dt><dd>Source account, balance, transactions and private assumptions</dd></div></dl><div className="plan-editor-actions"><button disabled={busy} onClick={() => void onCommand((current) => sharePlanBridgeDraft(current, { draftId: privateDraft.id, memberId, createdBy: memberId }))}>Share this exact proposal</button><button onClick={() => { setLabel(privateDraft.label); setAmount(privateDraft.amountCents === undefined ? "" : String(privateDraft.amountCents / 100)); setEditing(true); }}>Edit privately</button></div></section> : <form onSubmit={(event) => { event.preventDefault(); savePrivate(); }}><label>One fact I may share<input value={label} onChange={(event) => setLabel(event.target.value)} placeholder="I can contribute…" /></label><label>Amount, if useful<input inputMode="decimal" value={amount} onChange={(event) => setAmount(event.target.value)} /></label><button disabled={busy || !label.trim()}>Save privately and review</button>{editing && <button type="button" onClick={() => setEditing(false)}>Cancel edit</button>}</form>}
