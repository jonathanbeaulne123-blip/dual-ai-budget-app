...
export async function createWardrobeScene(host:HTMLElement,options:{play?:boolean;onPlayPick?:(id:string)=>void;palette:RoomPalette;signal:AbortSignal;paused:boolean;onError:()=>void;onReady:()=>void;onPick:(id:string)=>void;onFittingState?:(state:'loading'|'ready'|'error')=>void;onCollectionState?:(state:'loading'|'ready'|'error')=>void}):Promise<WardrobeScene>{
 let suspendLoop=()=>{},resumeLoop=()=>{};
 const rendererLease=acquireWorldRenderer(host,{parameters:{antialias:true,alpha:true,powerPreference:'low-power'},configure(renderer){renderer.setPixelRatio(Math.min(window.devicePixelRatio||1,1.5));renderer.outputColorSpace=T.SRGBColorSpace;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=1.05;renderer.shadowMap.enabled=false;renderer.localClippingEnabled=false;renderer.domElement.setAttribute('aria-hidden','true');renderer.domElement.className='';renderer.domElement.style.width='';renderer.domElement.style.height='';renderer.domElement.style.touchAction='';},onSuspend:()=>suspendLoop(),onResume:()=>resumeLoop()});
 ...
 const playRoom=options.play?createPlayRoom(scene,options.palette,()=>render()):null;
 const room=playRoom??createRoom(scene,options.palette);const {wood,trim,cloth,accent}=room.materials;host.dataset.roomLighting=options.palette.dark?'dark':'light';
 ...
 let zone:PlayArea|'room'='room',cameraTarget=new T.Vector3(0,.26,0),cameraDestination=camera.position.clone(),targetDestination=cameraTarget.clone();
 ...
 suspendLoop=()=>{cancelAnimationFrame(frame);frame=0;last=0;host.dataset.animating='false';};resumeLoop=()=>{resize();schedule();};
 ...
 const resize=()=>{if(disposed)return;const w=Math.max(host.clientWidth,1),h=Math.max(host.clientHeight,1);renderer.setSize(w,h,false);camera.aspect=w/h;camera.updateProjectionMatrix();if(options.play){zoneCamera();camera.position.copy(cameraDestination);cameraTarget.copy(targetDestination);camera.lookAt(cameraTarget);}render();};
 const resizeObserver=new ResizeObserver(resize);resizeObserver.observe(host);
 const lose=(event:Event)=>{event.preventDefault();dispose();options.onError();};const unlistenLost=rendererLease.listenCanvas('webglcontextlost',lose);
 const pick=(event:PointerEvent)=>{if(!model)return;const rect=renderer.domElement.getBoundingClientRect();const point=new T.Vector2((event.clientX-rect.left)/rect.width*2-1,-(event.clientY-rect.top)/rect.height*2+1),ray=new T.Raycaster();ray.setFromCamera(point,camera);const hits=ray.intersectObjects(scene.children,true);const playHit=hits.find(hit=>hit.object.userData.playTarget);if(options.play&&playHit){options.onPlayPick?.(String(playHit.object.userData.playTarget));return;}const hit=hits.find(hit=>hit.object.userData.fittingItemId);if(hit)options.onPick(hit.object.userData.fittingItemId as string);};const unlistenPick=rendererLease.listenCanvas('pointerup',pick);
 function dispose(){collectionAbort.abort();if(disposed)return;disposed=true;cancelAnimationFrame(frame);resizeObserver.disconnect();intersection?.disconnect();document.removeEventListener('visibilitychange',visibility);unlistenLost();unlistenPick();mixer?.stopAllAction();if(model)mixer?.uncacheRoot(model);scene.traverse(n=>{if(n instanceof T.SkinnedMesh)skeletons.add(n.skeleton);if(n instanceof T.Mesh){geometries.add(n.geometry);for(const m of Array.isArray(n.material)?n.material:[n.material])materials.add(m);}});for(const skeleton of skeletons)skeleton.dispose();for(const g of geometries)g.dispose();for(const m of materials)m.dispose();mirror.getRenderTarget().dispose();room.dispose();rendererLease.release();host.dataset.animating='false';}
