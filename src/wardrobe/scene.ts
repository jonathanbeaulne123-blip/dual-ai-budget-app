...
export type WardrobeScene={setLook:(look:LookV1)=>void;setCollection:(id:string)=>void;setKeepsake:(id:string|null)=>void;setPose:(pose:FittingReaction)=>void;setPaused:(paused:boolean)=>void;setCamera:(angle:number,zoom:number)=>void;setMirror:(show:boolean)=>void;dispose:()=>void};
export async function createWardrobeScene(host:HTMLElement,options:{theme:RoomTheme;personal:boolean;signal:AbortSignal;paused:boolean;onError:()=>void;onReady:()=>void;onPick:(id:string)=>void;onFittingState?:(state:'loading'|'ready'|'error')=>void;onCollectionState?:(state:'loading'|'ready'|'error')=>void}):Promise<WardrobeScene>{
