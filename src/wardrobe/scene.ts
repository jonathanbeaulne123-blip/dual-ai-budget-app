...
export async function createWardrobeScene(host:HTMLElement,options:{play?:boolean;onPlayPick?:(id:string)=>void;palette:RoomPalette;signal:AbortSignal;paused:boolean;onError:()=>void;onReady:()=>void;onPick:(id:string)=>void;onFittingState?:(state:'loading'|'ready'|'error')=>void;onCollectionState?:(state:'loading'|'ready'|'error')=>void}):Promise<WardrobeScene>{
 ...
 function dispose(){collectionAbort.abort();if(disposed)return;disposed=true;rendererLease.cancelFrame(frame);resizeObserver.disconnect();intersection?.disconnect();document.removeEventListener('visibilitychange',visibility);unlistenLost();unlistenPick();mixer?.stopAllAction();if(model)mixer?.uncacheRoot(model);scene.traverse(n=>{if(n instanceof T.SkinnedMesh)skeletons.add(n.skeleton);if(n instanceof T.Mesh){geometries.add(n.geometry);for(const m of Array.isArray(n.material)?n.material:[n.material])materials.add(m);}});for(const skeleton of skeletons)skeleton.dispose();for(const g of geometries)g.dispose();for(const m of materials)m.dispose();mirror.getRenderTarget().dispose();room.dispose();rendererLease.release();host.dataset.animating='false';}
