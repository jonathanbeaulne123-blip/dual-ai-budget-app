...
type Props={embedded?:boolean;initialPreview?:LookV1;onPortrait?:(look:LookV1)=>void;environment:string;householdId:string;memberId:string;view:'household'|'personal';onClose:()=>void;household?:Household;onCommand?:KitchenCommand;connected?:boolean};
...
export default function HerculesDressingRoom({embedded=false,initialPreview,onPortrait,environment,householdId,memberId,view,onClose,household,onCommand,connected=false}:Props){
 ...
 const initial=initialPreview??profile?.wornLook.value??legacy?.look??COZY_LOOK;
 ...
 const [history,setHistory]=useState<FittingHistory>(()=>({past:[],present:initialPreview??loadFittingDraft(storage,key,initial),future:[]}));
 const [compareWorn,setCompareWorn]=useState(false);
 const look=history.present;const visibleLook=compareWorn?(profile?.wornLook.value??initial):look;const latestLook=useRef(look);latestLook.current=look;
 ...
 const host=useRef<HTMLDivElement>(null),controller=useRef<WardrobeScene|null>(null),dialog=useDialog(!embedded,onClose,wardrobeReturnFocus),searchBox=useRef<HTMLInputElement>(null);
 ...
 useEffect(()=>{controller.current?.setLook(visibleLook);if(!canPlayReaction(pose,look)){setPose('breathe-blink');controller.current?.setPose('breathe-blink');}},[look,visibleLook,pose]);
 ...
 function preview(next:LookV1){setCompareWorn(false);const first=Object.values(next.selections).find(value=>value&&FITTING_ITEMS.some(item=>item.id===value.itemId));if(first)setSelected(first.itemId);resolveLegacy(legacy?.conflicts.map(c=>c.slot)??[]);setReplacement(null);setHistory(s=>fitEdit(s,structuredClone(next)));}
 ...
 const content=<div ref={dialog} className="hercules-fitting-backdrop" role={embedded?"region":"dialog"} aria-modal={embedded?undefined:true} aria-labelledby="fitting-title" data-fitting-theme={theme} data-fitting-view={view} data-fitting-lighting={scene.dark?'dark':'light'} data-fitting-sheet={sheet} onKeyDown={shortcuts}>
  <div className="hercules-fitting-room">
   ...
   <main className="fitting-stage-wrap">
    ...
    <div className="fitting-stage" data-fitting-stage>
     ...
     {(flat||status!=='ready')&&<div className="fitting-fallback"><FittingFigure look={visibleLook} size={230}/><p>{flat?'2D outfit preview':status==='error'?'3D is unavailable. Your outfit preview is still here.':'Opening the dressing room…'}</p>{status==='error'&&!flat&&<button type="button" onClick={()=>setAttempt(n=>n+1)}>Retry 3D</button>}</div>}
