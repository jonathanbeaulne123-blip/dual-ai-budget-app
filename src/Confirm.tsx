...
export function ConfirmSheet({
  ...
  if(danger&&review&&review.readIdentity()!==review.identity)latch.current.invalid=true;
  ...
  const latest=useRef({busy,phoneDanger,revealKey,revealed,dragging});latest.current={busy,phoneDanger,revealKey,revealed,dragging};
  const valid=!danger||Boolean(review&&!latch.current.invalid);
  const isCurrent=()=>{
  ...
  const confirm=<button type='button' className={danger?'danger':'primary'} style={{width:'100%',marginTop:12}} disabled={busy||!valid||dragging} aria-busy={busy||undefined} onClick={()=>{const now=latest.current;if(!now.busy&&!now.dragging&&(!now.phoneDanger||now.revealed===now.revealKey)&&isCurrent())onConfirm();}}>{confirmLabel}</button>;
