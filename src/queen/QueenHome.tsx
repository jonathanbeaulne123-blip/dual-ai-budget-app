      ...
      <aside ref={panelRef} id={panelId} className="queen-panel" role="dialog" aria-modal={mode === "sheet"} aria-labelledby={`${ids}-panel-title`} inert={!open}>
        ...
        {open === "status" && (
          <>
            ...
            <QueenWindow household={household} today={today} liveDerivedScene={supported.source === "current"} onWalk={() => onGo("plan")} />
 ...
 */
function QueenWindow({ household, today, liveDerivedScene, onWalk }: { household: Household; today: DateKey; liveDerivedScene: boolean; onWalk: () => void }) {
  const id = useId();
  ...
  return (
    <section className="queen-window" aria-labelledby={`${id}-window`}>
      ...
      <div className="queen-window__pane">
        <PathMiniMap household={household} today={today} liveDerivedScene={liveDerivedScene} />
      </div>
