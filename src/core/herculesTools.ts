...
export const HERCULES_READ_TOOL_NAMES = [
  ...
  "plan_overview",
  "plan_line_detail",
  "plan_cashflow_runway",
  "plan_coverage",
  "plan_assumptions",
  "plan_version_diff",
  "plan_scenario_compare",
  "plan_actual",
  "plan_drift",
  "plan_bridge_status",
  "plan_sitdown_status",
  "plan_learning_context",
] as const;
...
export const HERCULES_READ_TOOL_CATALOG: ReadonlyArray<{ name: HerculesReadToolName; description: string }> = [
  ...
  { name: "plan_actual", description: "Compare visible accepted Plan lines with accepted outcomes and posted actuals." },
  { name: "plan_drift", description: "Read deterministic, thresholded Plan drift findings. Never grades behavior." },
  { name: "plan_bridge_status", description: "Read only deliberately submitted Bridge facts and their Plan status." },
  { name: "plan_sitdown_status", description: "Read Shared Sitdown progress, participants, and open Plan acknowledgement work." },
  { name: "plan_learning_context", description: "Read the contextual, skippable lesson for the visible Plan month." },
];
...
function cleanMonth(value: unknown): MonthKey | undefined {
  const month = cleanString(value, 7);
  return month && /^\d{4}-\d{2}$/.test(month) ? month as MonthKey : undefined;
}
...
function cleanArgs(name: HerculesReadToolName, raw: unknown, context?: HerculesAskContext): Record<string, unknown> {
  ...
  if (name.startsWith("plan_")) return {
    monthKey: cleanMonth(input.monthKey),
    planVersionId: cleanString(input.planVersionId, 128),
    compareVersionId: cleanString(input.compareVersionId, 128),
    planLineId: cleanString(input.planLineId, 128),
    scenarioId: cleanString(input.scenarioId, 128),
  };
...
function fact(call: HerculesReadToolCall, index: number, label: string, value: string, source: HerculesNumberSource, basis: "journal" | "projection" | "plan" = "journal"): HerculesGroundedFact {
...
function visiblePlanVersion(household: Household, call: HerculesReadToolCall, today: DateKey, context: HerculesAskContext): PlanVersion | null {
...
function executeCall(household: Household, call: HerculesReadToolCall, today: DateKey, context: HerculesAskContext): HerculesReadToolResult {
  if (call.name.startsWith("plan_")) return executePlanCall(household, call, today, context);
