...
export type CompanionSourceReference = {
  kind: "account" | "transaction" | "goal" | "shift" | "page" | "plan-version" | "plan-draft" | "plan-line" | "plan-scenario" | "plan-assumption" | "plan-bridge" | "plan-sitdown"; id: string;
};
...
function turn(value: unknown): CompanionConversationTurn {
  ...
  return {
    ...
    sourceReferences: list(row.sourceReferences, 20, value => {
      ...
      return { kind: literal(source.kind, ["account", "transaction", "goal", "shift", "page", "plan-version", "plan-draft", "plan-line", "plan-scenario", "plan-assumption", "plan-bridge", "plan-sitdown"]), id: id(source.id) };
    }),
  };
}
