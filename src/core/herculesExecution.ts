...
function release(h: Household, input: Claim): CommitResult {
    ...
    const next = { ...h, companionProfile: { ...profile, workflows: profile.workflows!.map(r => r.id === resource.id ? { ...r, revision: r.revision + 1, value: nextHerculesTask(resource.value!) } : r) } };
