...
function NextChapterReview({ household, next }: { household: Household; next: NonNullable<ReturnType<typeof reviewChapterClosure>["terms"]["nextChapter"]> }) {
  const foundation = FOUNDATION_CHAPTERS.find(row => row.id === next.foundationId);
  return <section aria-label="The next Chapter in this review"><h5>{foundation?.title ?? next.custom?.title} · {next.month}</h5><p>{foundation?.meaning ?? next.custom?.meaning}</p><p>Opened by {memberName(household, next.ownerMemberId)} after both agreements.</p>{foundation && <><p>New Ritual: {foundation.ritual.title} · owned by {memberName(household, next.ownerMemberId)} · {foundation.ritual.doneDefinition}</p><p>Cue: {foundation.ritual.cue} {foundation.ritual.cueNote} · Recovery: {foundation.ritual.recoveryMove}</p><p>First Move: {foundation.firstMove}. Its own Task and responsibility review still applies.</p></>}</section>;
}
  ...
  return (
    <div className="chapter-close">
      ...
      <div className="chapter-outcomes">
        {outcomes.map((row) => (
          <button key={row.outcome} type="button" disabled={busy} onClick={() => { setError(null); try { setLocalReview((sortedMonths ? reviewChapterClosureAtSitdown : reviewChapterClosure)(household, { memberId, today, chapterId: chapter.id, outcome: row.outcome, carryForward: carry, sitdownId: sitdownId ?? undefined })); } catch (cause) { setError(cause instanceof Error ? cause.message : "Review is unavailable."); } }}>
