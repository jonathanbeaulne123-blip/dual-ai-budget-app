  ...
  if (level <= 1) {
    ...
    for (let d = from; d <= to; d += 1) {
      ...
      day.items.forEach((item, i) => {
        if (item.kind === "bill") {
          ...
          lanes.filter(([lane]) => (item.split?.[lane] ?? 0) > 0).forEach(([lane, y], m) => out.push(<path key={key("c", d, i, m)} d={`M${X} 116 C${X} ${y - 10} ${X + 14 + m * 8} ${y - 6} ${X + 26 + m * 12} ${y}`} fill="none" stroke={P[lane]} strokeWidth={2} opacity={item.expected ? 0.55 : 1} />));
        }
      });
    ...
    banks.forEach((bank, i) => {
      ...
      out.push(<g key={bank.goalId}>
        ...
        {bank.fill !== null && <rect x={colX + 2} y={y + 16 - 14 * bank.fill} width={12} height={14 * bank.fill} rx={1} fill={bank.bought ? P.today : P.gold} />}
