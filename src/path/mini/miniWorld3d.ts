...
export function createMiniWorld(host: HTMLElement, options: {
  ...
  function kittyBank(parent: THREE.Object3D, bank: { fill: number; full: boolean; bought: boolean }, scale: number, group?: string): THREE.Group {
    const g = new THREE.Group();
    ...
    fill.userData.fill = Math.max(0.04, bank.fill);
    ...
    spots.forEach(({ era, x, y, z: zz, r }, i) => {
      ...
      if (i !== f) {
        ...
        era.plans.slice(0, 6).forEach((plan, p) => {
          ...
          if (plan.kind === "bank") { const b = kittyBank(g, { fill: plan.fill ?? 0, full: (plan.fill ?? 0) >= 1, bought: false }, 1.5); b.position.set(px, 0, pz); }
