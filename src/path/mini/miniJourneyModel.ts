...
import { goalFundReserve, goalRemainingClaim } from "../../core/goalEnvelopes.ts";
...
import { defaultFundSnapshotSource, type FundSnapshotSource } from "../../plan-v3/model.ts";
 ...
 * `prepareFundInflows`, Plan Studio's `defaultFundSnapshotSource`); the lanes are a way of thinking
...
export type MiniContribution = {
  ...
  expected: boolean;
  /** Only the exact, jointly confirmed division may draw branches into lanes. */
  split: Record<MiniLane | "everyday", number> | null;
};
...
export type MiniLaneTracker = { lane: MiniLane | "everyday"; label: string; amountCents: number | null; targetCents: number | null };
...
export function miniCad(cents: number | null, sign = false): string {
...
function bankOf(household: Household, goalId: string, today: DateKey): MiniBank | null {
  ...
  const readable = safe(() => {
    goalRemainingClaim(household, goal, today);
    return !goalFundReserve(household, goal.id, today).unresolved;
  }, false);
  const step = bought ? 10 : readable ? kittyBankBackingStep(household, goal, today) : null;
  return { goalId, worldId: `goal:${goalId}`, name: goal.name, step, fill: step === null ? null : step / 10, full: step !== null && step >= 10, bought };
}
...
export function miniMonth(household: Household, monthKey: string, options: {
  ...
  if (fund) {
    ...
    for (const event of events) {
      ...
      const division = clientFundModelVersion() === 2 && fundModelMode(household) === 2 ? divisionFor(household, event.id) : null;
      items.push({ kind: "contribution", id: `contribution:${event.id}@${event.date}`, date: event.date, memberId: event.contributorMemberId ?? null, who, amountCents: event.amountCents, expected: false, split: division?.state === "confirmed" ? { ...division.split } : null });
    }
    if (end >= today) {
      ...
      for (const row of ahead) {
        ...
        items.push({ kind: "contribution", id: `contribution:${row.sourceId ?? `estimate:${row.date}`}@${row.date}`, date: row.date, memberId: row.memberId, who, amountCents: row.amountCents, expected: true, split: null });
      }
    }
  }
...
export function miniFund(household: Household, memberId: string, today: DateKey, source: FundSnapshotSource = defaultFundSnapshotSource()): MiniFund {
  // Use the same release-gated, data-aware source as Plan Studio. The adapter owns all money meaning,
  // including the transitional household and unavailable backing; Journey only names its readings.
  const snapshot = safe(() => source(household, { memberId, view: "household", today }), null);
  const pick = (lane: MiniLane): MiniLaneTracker => ({
    lane, label: MINI_LANE_LABEL[lane], amountCents: snapshot?.[lane].amountCents ?? null,
    targetCents: snapshot?.[lane].targetCents ?? null,
  });
  return {
    ready: Boolean(shapeHouseholdFundConfig(household.householdFund) && snapshot),
    lanes: { prepare: pick("prepare"), protect: pick("protect"), build: pick("build") },
    everyday: { lane: "everyday", label: MINI_LANE_LABEL.everyday, amountCents: snapshot?.now.amountCents ?? null, targetCents: null },
    source: "Household Fund · Plan Studio",
  };
}
...
export function miniItemWords(item: MiniItem): string {
  ...
  if (item.kind === "contribution") return `${item.who} · ${miniCad(item.amountCents)} into the Fund · ${item.expected ? "expected, not yet confirmed" : item.split ? "confirmed division" : "confirmed"}`;
