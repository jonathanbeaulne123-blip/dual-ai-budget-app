...
export function JourneyMini(props: JourneyMiniProps) {
  ...
  const sceneInput: MiniSceneInput | null = useMemo(() => {
    ...
    return {
      ...
      days: month.days.map((d) => ({
        ...
        contributions: d.items.flatMap((i) => (i.kind === "contribution" ? [{ expected: i.expected }] : [])),
  ...
  const buildCard = useCallback((id: string): Card | null => {
    ...
    if (id.startsWith("lane:")) {
      ...
      const meaning = { prepare: "Costs coming around: the month's bills are planned from here.", protect: "Bills and breathing room: the cushion.", build: "Dreams we are growing: the Kitty Banks.", everyday: "Day-to-day choices: the water the path crosses." }[lane];
      return {
    ...
    if (id.startsWith("bank:") || id.startsWith("goal:")) {
      ...
      return {
        ...
        lines: [bank.bought ? "Bought. It stays on the finish line." : bank.full ? "Full: every step is backed." : `${bank.step} of 10 steps backed`, "The steps are the same ones the open world shows."],
    ...
    if (id === "finish") {
      ...
      return {
        ...
        lines: [era.finishWhy || `${era.banksFull} of ${era.banks.length} Kitty Banks full`, ...era.banks.map((b) => `${b.name}: ${b.bought ? "bought" : b.full ? "full" : `${b.step} of 10 steps`}`)],
    ...
    if (id.startsWith("era:") || id === "era-home" || id === "here") {
      ...
      return {
        ...
        lines: [
          ...
          ...era.plans.filter((p) => p.kind !== "bank" || era.finishKind !== "banks").slice(0, 4).map((p) => `${p.label}${p.bank ? ` · ${p.bank.bought ? "bought" : `${p.bank.step} of 10 steps`}` : ""}${p.sketched ? " · suggested" : ""}`),
        ],
        actions: [
        ...
        out.push({
          ...
          content: <>
            {money.slice(0, 2).map((i) => i.kind === "bill"
              ...
              : i.kind === "contribution" ? <span key={i.id} className={`journey-mini__tag-row journey-mini__tag-row--in${i.expected ? " is-expected" : ""}`}><b>{i.who}</b><span>{miniCad(i.amountCents, true)}{i.expected ? " expected" : ""}</span></span> : null)}
      ...
      if (focusEra.banks.length) {
        ...
        for (const b of focusEra.banks) {
          out.push({
            ...
            aria: `${b.name}: ${b.bought ? "bought" : b.full ? "full" : `${b.step} of 10 steps`}`,
            content: <><b>{b.name}</b><span>{b.bought ? "bought" : b.full ? "full" : `${b.step}/10`}</span></>,
          });
        }
      }
    }
  ...
  const stageEl = (
    ...
    >
      ...
      {!list && labelLayer}
      {list}
      {!journey && <p className="journey-mini__loading">Reading our journey…</p>}
  ...
  if (level <= 1) {
    ...
    body = <>
      ...
      {era.banks.length > 0 && <>
        ...
        <ul>{era.banks.map((b) => <li key={b.goalId}><button type="button" onClick={() => onPick(`bank:${b.goalId}`)}>{b.name}: {b.bought ? "bought" : b.full ? "full" : `${b.step} of 10 steps`}</button></li>)}</ul>
      </>}
    </>;
  } else {
    body = <>
