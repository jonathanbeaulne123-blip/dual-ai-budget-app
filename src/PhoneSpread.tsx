...
function Reading({ view, scenarioSource, id, household, memberId, today, busy, onKitchen, onOpen }: {
  ...
  return <FundStage view={view} scenarioSource={scenarioSource} widgetId={widgetId} household={household} memberId={memberId} today={today} busy={busy}
  ...
  return <section className="phone-spread" aria-label={`${PHONE_CHAPTERS[chapter].name} chapter`} onKeyDown={event => {
    ...
    <div ref={panel} id={`${id}-page`} className="spread-page" tabIndex={-1} role="tabpanel" aria-labelledby={`${id}-heading`}
      ...
      <div style={{ transform: `translateX(${offset}px)` }} key={`${reading}:${household.environment}:${household.householdId}:${memberId}:${today}`}>
        <Reading view={view} scenarioSource={scenarioSource} id={reading} household={household} memberId={memberId} today={today} busy={busy} onKitchen={onKitchen} onOpen={destination => { onClose(); onOpen(destination); }} />
      </div>
    </div>
