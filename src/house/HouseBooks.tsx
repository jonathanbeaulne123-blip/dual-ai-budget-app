...
export function HouseBooks({household,memberId,view,children}:{household:Household;memberId:string;view:LedgerView;children:(division:BookDivision)=>ReactNode}){
  ...
  return <section className="house-book" data-book-theme={appearance.scene.theme} data-book-division={division}>
    <header className="house-book__cover"><div><p className="kicker">{view==="personal"?"My private ledger":"Our household ledger"}</p><h2>The Standing Book</h2><p>Open on {today}. Figures stay in the books; the room gives them a place.</p></div><span className="house-book__clasp" aria-hidden="true">H</span></header>
    ...
    <div className="house-book__spread"><header className="house-book__page-title"><h3>{division}</h3><button aria-pressed={flat} onClick={()=>setFlat(!flat)}>{flat?"Return to the illustrated page":"Read exact figures"}</button></header>
      ...
      {division==="Goals"&&<section className={`book-glasshouse ${flat?"is-flat":""}`} aria-label="Goals Glasshouse"><div className="book-glasshouse__roof" aria-hidden="true"/><div className="book-glasshouse__shelf">{banks.filter(bank=>bank.tier==="goal").map(bank=><article key={bank.id}><span className="book-glasshouse__pot" aria-hidden="true"><i/><b/></span><h4>{bank.name}</h4><p>{bank.amountCents===null?"Backing unavailable":formatCad(bank.amountCents)} <small>of {formatCad(bank.targetCents)}</small></p>{bank.date&&<time>{bank.date}</time>}<small>Your original ceramic bank</small></article>)}</div>{!banks.some(bank=>bank.tier==="goal")&&<p>The glasshouse has room for your next goal. Open a bank below when you are ready.</p>}</section>}
