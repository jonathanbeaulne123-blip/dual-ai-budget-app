...
export function HouseWorld({household,memberId,scope,today,route,ready,freshness,onNavigate,onOpen,onClose}:Props){
  ...
  return <section className={`house-world house-world--${theme}${route.surface?" has-open-object":""}`} data-world-status={status} data-world-scope={scope} aria-label={`${scope==="personal"?"My":"Our"} house`}>
    <header className="house-world__header"><div><span className="house-world__scope">{scope==="personal"?"My private house":"Our home"}</span><h1 id="house-world-title" tabIndex={-1}>{ROOM_NAMES[route.room]}<span> / {place.title}</span></h1></div><button onClick={()=>setOverview(value=>!value)} aria-pressed={overview}>{overview?"Return to room":"See the whole house"}</button></header>
    <div className="house-world__stage" tabIndex={walking?0:undefined} aria-label={walking?"Walking area. Arrow keys move your avatar through doorways and stairs. Tap a path to walk there. Furniture remains directly available.":undefined} onPointerDown={event=>{pointerDown.current={x:event.clientX,y:event.clientY};}} onPointerUp={event=>{const start=pointerDown.current;pointerDown.current=null;if(walking&&start&&Math.hypot(start.x-event.clientX,start.y-event.clientY)<8&&!(event.target as HTMLElement).closest("button,input,textarea,select,a"))runtime.current?.walkTo(event.clientX,event.clientY);}} onKeyDown={event=>{if(!walking||event.target!==event.currentTarget)return;if(["ArrowLeft","ArrowRight","ArrowUp","ArrowDown"].includes(event.key)){event.preventDefault();runtime.current?.walk(event.key.slice(5).toLowerCase() as "left"|"right"|"up"|"down");}}}>
      ...
      <nav className="house-world__levels" aria-label={`${ROOM_NAMES[route.room]} levels`}>{HOUSE_LEVELS.map(level=><button key={level} onClick={()=>{setOverview(false);onNavigate(route.room,level);}} aria-current={route.level===level?"location":undefined}><span aria-hidden="true">{level==="above"?"↑":level==="below"?"↓":"·"}</span>{HOUSE_PLACES[route.room][level].title}</button>)}</nav>
      {overview?<nav className="house-world__section-nav" aria-label="Enter a wing">{HOUSE_ROOMS.map(room=><button key={room} onClick={()=>{setOverview(false);onNavigate(room,"middle");}}><span>{ROOM_NAMES[room]}</span><small>{HOUSE_PLACES[room].above.title} / {HOUSE_PLACES[room].below.title}</small></button>)}</nav>:<>
        ...
        {!route.surface&&<div className="house-world__floor-scroll" ref={floorScroller} onScroll={floorScroll} aria-label="Scroll through this wing">{HOUSE_LEVELS.map(level=><section className="house-world__floor" key={level} data-house-level={level} inert={level!==route.level} aria-label={HOUSE_PLACES[route.room][level].title}><div className="house-world__invitation"><p className="kicker">{ROOM_NAMES[route.room]} · {level==="above"?"Upstairs":level==="below"?"Downstairs":"Come inside"}</p><h2>{HOUSE_PLACES[route.room][level].title}</h2><p>{scope==="personal"&&route.room==="together"&&level==="middle"?"A studio and a quiet place for your own words.":HOUSE_PLACES[route.room][level].subtitle}</p><div className="house-world__object-actions">{houseTargets(scope,route.room,level).map(({id,label})=><button id={`house-action-${route.room}-${level}-${id}`} key={id} onClick={()=>choose(id)}>{label} <span aria-hidden="true">↗</span></button>)}</div></div></section>)}</div>}
      </>}
