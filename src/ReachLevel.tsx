...
export function ReachLevel({ horizon, scenario, capacityCents }: {capacityCents: number; horizon: FundHorizon; scenario: Extract<FundScenarioResult, {kind: "scenario"}> | null}) {
  ...
  const tail = (points: readonly WalkPoint[], end: number): Point[] => [anchor, ...points, {date: horizon.through, balanceCents: end}];
  const baseline = tail(horizon.future, horizon.endBalanceCents);
  const lower = scenario ? tail(scenario.lower.future, scenario.lower.endBalanceCents) : baseline;
  const expected = scenario ? tail(scenario.expected.future, scenario.expected.endBalanceCents) : baseline;
  ...
  const differs = !!scenario && lower.some((p,i) => p.balanceCents !== expected[i]?.balanceCents);
  ...
  return <svg className="reach-level" viewBox="0 0 344 148" role="img" aria-label={`Shared Fund. Accepted today ${formatCad(horizon.anchorCents)}. Baseline end ${formatCad(horizon.endBalanceCents)} through ${horizon.through}.${scenario ? ` Chosen scenario end: lower ${formatCad(scenario.lower.endBalanceCents)}, expected ${formatCad(scenario.expected.endBalanceCents)}.` : ''}`}>
