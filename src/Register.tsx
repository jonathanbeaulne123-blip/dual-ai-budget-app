        ...
        return (
          <g key={`${item.tone}-${item.label}`} transform={`translate(${here},4)`}>
            {item.tone === "unfunded" ? (
              ...
              <rect data-register-tone={item.tone} x="0" y="1" width={swatch} height={swatch} fill={toneFill(item.tone)} />
            )}
    ...
    >
      <g aria-hidden="true">
        ...
        {register.byMember.map((row, index) => {
          ...
          return (
            <g key={row.memberId}>
              <rect data-register-tone={member.tone} x="0" y={y - 9} width="9" height="9" fill={toneFill(member.tone)} />
