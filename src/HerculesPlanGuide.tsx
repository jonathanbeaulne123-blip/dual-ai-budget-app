...
export function HerculesPlanGuide({context,values,busy,onAnswer,onBack,onPause,onSkip}:{context:ActionContext;values:ActionValues;busy:boolean;onAnswer:(text:string)=>void;onBack:(key?:string)=>void;onPause:()=>void;onSkip:()=>void}) {
 ...
 useEffect(()=>{if(document.activeElement?.tagName!=='TEXTAREA')questionHeading.current?.focus();},[field?.key]);
 const choices=field?.choices?.(context,values)??[];
 return <section className="hercules-plan-guide" aria-label="Plan conversation">
  ...
  {field ? <div key={field.key} className="hercules-plan-question">
   ...
   {field.kind==='date'&&<><button type="button" onClick={()=>setDatePicker(!datePicker)} aria-expanded={datePicker}>Choose a date</button>{datePicker&&<label>Pick {field.label.toLowerCase()}<input type="date" disabled={busy} onChange={e=>{if(e.target.value){onAnswer(e.target.value);setDatePicker(false);}}}/></label>}</>}
   <small>Reply in the conversation below. You can ask “why?”, go back, or leave this part open.</small>
