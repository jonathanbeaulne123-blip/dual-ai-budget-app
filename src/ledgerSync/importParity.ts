...
export async function compareImportParity(input: {
  ...
  for(const [label,envelopes] of [["source",source],["target",target]] as const) {
    ...
    if((envelopes.shared as unknown as Record<string,unknown>).personalLife!==undefined || envelopes.shared.kittyNestDesigns?.some(row=>row.visibility==='personal') || envelopes.shared.fundModelRows?.some(row=>row.visibility==='personal') || envelopes.shared.tasks?.some(row=>row.visibility==='personal') || envelopes.shared.taskLists?.some(row=>row.visibility==='personal') || envelopes.shared.nativeEvents?.some(row=>row.visibility==='personal') || envelopes.shared.transactions.some(row=>row.visibility==='personal') || envelopes.shared.shifts.some(row=>row.visibility==='personal') || envelopes.shared.potentialExpenses?.some(row=>row.visibility==='personal') || envelopes.shared.accounts.some(row=>row.scope==='personal') || envelopes.shared.planVersions?.some(row=>row.scope!=='household') || envelopes.shared.planReflections?.some(row=>row.scope!=='household') || 'planBridgeDrafts' in envelopes.shared)differences.push(`${label}.sharedPrivacy`);
    if(envelopes.personal.kittyNestDesigns?.some(row=>row.visibility!=='personal'||row.createdBy!==member) || envelopes.personal.fundModelRows?.some(row=>row.visibility!=='personal'||!('setBy' in row)||row.setBy!==member) || envelopes.personal.tasks?.some(row=>row.visibility!=='personal'||row.createdBy!==member) || envelopes.personal.taskLists?.some(row=>row.visibility!=='personal'||row.createdBy!==member) || envelopes.personal.nativeEvents?.some(row=>row.visibility!=='personal'||row.createdBy!==member) || envelopes.personal.transactions.some(row=>row.visibility!=='personal'||row.createdBy!==member) || envelopes.personal.shifts.some(row=>row.visibility!=='personal'||row.createdBy!==member)
      || envelopes.personal.personalLife?.ownerMemberId!==undefined && envelopes.personal.personalLife.ownerMemberId!==member || envelopes.personal.potentialExpenses?.some(row=>row.visibility!=='personal'||row.createdBy!==member) || envelopes.personal.accounts?.some(row=>row.scope!=='personal'||row.ownerMemberId!==member) || envelopes.personal.goals?.some(row=>row.shared||row.ownerMemberId!==member) || envelopes.personal.planDrafts?.some(row=>row.ownerMemberId!==member) || envelopes.personal.planVersions?.some(row=>row.scope!=='personal'||row.ownerMemberId!==member) || envelopes.personal.planScenarios?.some(row=>row.ownerMemberId!==member) || envelopes.personal.planBridgeDrafts?.some(row=>row.ownerMemberId!==member))differences.push(`${label}.personalPrivacy`);
  }
