...
export function FuturePath({household,memberId,experience,onReference}:{household:Household;memberId:string;experience:SharedExperience;onReference:(reference:SharedReference)=>void}){
 ...
 return <section className="hearthside-future-path" aria-label={experience.state==='lived'?"This lived intention":"This imagined possibility"}><p className="hearthside-eyebrow">{experience.state==='lived'?`Lived · ${experience.livedOn??'date not recorded'}`:`Imagined · ${horizon.label}`}</p><h3>{experience.state==='lived'?'What helped this happen':experience.state==='paused'?'A wish with room to wait':'What could help this happen?'}</h3><p>{experience.state==='paused'?'Resting here does not create a deadline or transfer a task to either of you.':horizon.invitation}</p>
