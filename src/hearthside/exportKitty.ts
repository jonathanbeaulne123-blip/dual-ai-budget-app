...
export async function finishKittyExport(prepared: PreparedKittyExport, options: { approvedRepairDigest?: string; wasmUrl?: string } = {}): Promise<ExportPackage> {
  ...
  const profileSheet = selection.manufacturingProfile === undefined ? [] : [
    ...Array.from({ length: Math.ceil(selection.manufacturingProfile.length / 48) }, (_, index) => `${index ? 'Manufacturing profile (continued):' : 'Manufacturing profile (reference only):'} ${selection.manufacturingProfile.slice(index * 48, (index + 1) * 48)}`),
