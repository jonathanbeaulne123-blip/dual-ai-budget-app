...
export type HearthsideProps = {
  ...
  /**
   * K10 (Tool Atlas §7): the practical "Sit together" board merges into Glasshouse steps (its to-dos) and the
   * Campfire's Look ahead (its agenda). When given, the room's doors go there instead of the board.
   */
  onSteps?: () => void;
  onLookAhead?: () => void;
};
...
function HearthsideHousehold({household:h,memberId,identity,connected,busy,onCommand,onReadSubmission,onReadAcceptedCommand,onReadAcceptedHousehold,practical,studio,letters,onReference,onWorkspace,initialRoom,vaultSource,controlledRoute,onNavigate,onReturnToOrigin,onSteps,onLookAhead}:HearthsideProps) {
  ...
  const [route,setRoute]=useState<HearthsideRoute>(()=>controlledRoute??parseHearthsideRoute(window.location.href,h.householdId)??{version:1,householdId:h.householdId,room:initialRoom??'common',mode:'present'});
  ...
  const [livedDate,setLivedDate]=useState(todayKey());
  ...
  },[controlledRoute?.room,controlledRoute?.surface,controlledRoute?.studioTab,controlledRoute?.studioSelection?.designId,controlledRoute?.studioSelection?.pieceId,controlledRoute?.object?.id,controlledRoute?.object?.kind,controlledRoute?.object?.kind==="piece"?controlledRoute.object.designId:undefined]);
  useEffect(()=>{
  ...
  function navigate(next:HearthsideRoute) {
    ...
    if(!onNavigate)window.history.replaceState({...window.history.state,hearthsideFocus:returnFocus.current},'',window.location.href);
    setRoute(next);setTool(next.surface??null);setBank(null);setLifeStep(null);
    if(onNavigate)onNavigate(next);else window.history.pushState({hearthTab:'play'},'',hearthsidePath(next));
  ...
  function openObject(kind:NonNullable<HearthsideRoute['object']>['kind'],id:string,designId?:string){const origin=document.activeElement instanceof HTMLElement?document.activeElement.id||'hearthside-title':'hearthside-title';navigate({...route,surface:undefined,studioSelection:undefined,studioTab:undefined,object:kind==='piece'?{kind,id,designId:designId!}:{kind,id},returnContext:{path:hearthsidePath({...route,returnContext:undefined}),focusId:origin}});}
  ...
  function rememberStudioSelection(designId:string,pieceId:string) {
    ...
    setRoute(next);
    if(onNavigate)onNavigate(next,true);else window.history.replaceState(window.history.state,'',hearthsidePath(next));
  }
  ...
  return <main className="hearthside" data-world={theme} data-room={route.room} data-paused={appearance.paused} ref={root} aria-labelledby="hearthside-title">
    ...
    {!focused&&<>
      ...
      <section className="hearthside-room-actions" aria-label={`Things to do in the ${ROOMS[route.room].name.toLowerCase()}`}>
        <div className="hearthside-scene-actions">
          ...
          {route.room==='common'&&<><button id="hearthside-add-intention" onClick={()=>beginExperience()}>Add an intention</button><button id="hearthside-write-note" onClick={beginNote}>Leave a little note</button>{onSteps?<><button id="hearthside-steps" onClick={onSteps}>Our steps</button>{onLookAhead&&<button id="hearthside-look-ahead" onClick={onLookAhead}>Look ahead at the Campfire</button>}</>:<button id="hearthside-sit-together" onClick={()=>openTool('practical')}>Sit together</button>}</>}
      ...
      <section className="hearthside-invitations" aria-label="Things we can do">{JOURNEYS.filter(j=>j.room===route.room).map(j=><button id={`hearthside-journey-${j.id}`} key={j.id} onClick={()=>{if(j.id==='leave-something')beginNote();else if(j.id==='remember')beginMemory(null);else if(j.id==='care-for-life'){if(onSteps)onSteps();else openTool('practical');}else if(j.id==='make-together')openTool('studio');else beginExperience(j.prompt);}}>{j.label}<span aria-hidden="true">↗</span></button>)}</section>
    </>}
    {focused&&<section className="hearthside-focused"><button className="hearthside-back" onClick={closeFocused}>← Back to {ROOMS[route.room].name.toLowerCase()}</button>
      {tool==='restore'?<SharedLifeRestore household={h} memberId={memberId} identity={identity} theme={theme} connected={connected} source={vaultSource??(()=>null)} onCommand={onCommand} onClose={closeFocused} onEditMemory={id=>{const memory=state.memories.find(m=>m.id===id&&!m.withdrawn);if(memory){openObject('memory',id);editMemory(memory);}}} renderDesign={reference=><SavedPiecePreview reference={reference}/>}/>:tool==='encounters'||!tool&&route.object?.kind==='encounter'?encounterEntry(route.object?.kind==='encounter'?route.object.id:undefined):tool==='guests'?<GuestVisitsEntry household={h} theme={theme} onClose={closeFocused}/>:tool==='worktable'&&selectedExperience?<ExperienceWorktable hearthside={{scope:{identity,environment:h.environment,householdId:h.householdId,memberId},experience:workspaceExperienceContext(selectedExperience),onReturn:closeFocused}}/>:tool==='history'?<RoomHistory household={h} memberId={memberId} identity={identity} room={route.room} horizon={horizon} theme={theme} canSave={commands.canStart} submit={commands.submit} onCurrent={target=>openObject(target.kind,target.id,target.kind==='piece'?target.designId:undefined)}/>:tool==='projector'?<ProjectorEntry household={h} memberId={memberId} identity={identity} source={vaultSource} theme={theme} onClose={closeFocused} onReviewMemory={id=>openObject('memory',id)}/>:tool==='occasions'||route.object?.kind==='occasion'?<Occasions state={state} memberId={memberId} identity={identity} canSave={commands.canStart} submit={commands.submit} onOpen={openObject} selectedId={route.object?.kind==='occasion'?route.object.id:undefined}/>:tool==='letters'?letters?.(closeFocused,theme):tool==='practical'?practical:tool==='studio'||tool==='wardrobe'||route.object?.kind==='piece'?<CollaborativeStudio key={tool==='wardrobe'?'wardrobe':'clay'} initialTab={tool==='wardrobe'?'wardrobe':route.studioTab??'shape'} onTabChange={studioTab=>{const next:HearthsideRoute={...route,surface:'studio',room:'studio',studioTab};setRoute(next);setTool('studio');if(onNavigate)onNavigate(next,true);else window.history.replaceState(window.history.state,'',hearthsidePath(next));}} handoff={(documentId,pieceId,revision,pending)=><StudioHandoff household={h} memberId={memberId} identity={identity} design={{version:1,documentId,pieceId,revision}} canSave={commands.canStart&&!pending} submit={commands.submit}/>} acceptedReceipt={nativeFunding.receipt} onFundingIntent={intent=>{const id=nativeFunding.begin(intent);if(id)setBank({id});}} initialDesignId={route.object?.kind==='piece'?route.object.designId:route.studioSelection?.designId} initialPieceId={route.object?.kind==='piece'?route.object.id:route.studioSelection?.pieceId} onSelection={rememberStudioSelection} returnPath={hearthsidePath(route)} household={h} memberId={memberId} wardrobe={studio} onKeep={selectedExperience?(designId,pieceId,revision)=>{void commands.submit({kind:'experience.save',expectedRevision:selectedExperience.revision,value:{...selectedExperience,revision:selectedExperience.revision+1,references:[...selectedExperience.references.filter(r=>!(r.kind==='piece'&&r.id===pieceId&&r.designId===designId)),{kind:'piece',id:pieceId,designId,revision}]}});}:undefined}/>:selectedExperience?<>
        ...
        <section className="house-lived-review" aria-label="Record a lived experience"><label>We lived this on<input type="date" value={livedDate} onChange={event=>setLivedDate(event.target.value)}/></label><button disabled={!commands.canStart||!livedDate} onClick={()=>void commands.submit({kind:'experience.mark-lived',id:selectedExperience.id,expectedRevision:selectedExperience.revision,livedOn:livedDate})}>{selectedExperience.livedOn?'Correct the lived date':'We lived this'}</button>{selectedExperience.livedOn&&<p>Lived on {selectedExperience.livedOn}. A purchase did not make this decision.</p>}</section>
    ...
    {bank&&<KittyBankRoom household={h} view="household" memberId={memberId} identity={identity} busy={busy} onCommand={nativeFunding.command} onReadSubmission={nativeFunding.read} onReadAcceptedCommand={onReadAcceptedCommand} initialGoalId={bank.id} returnTo="the Boathouse" onClose={()=>{setBank(null);requestAnimationFrame(()=>document.getElementById(returnFocus.current)?.focus());}}/>}
  </main>;
}
