...
export function readHearthsideToolReturn(value:unknown,scope:string,householdId:string):HearthsideToolReturn|null {
  try{
    ...
    const canonicalTogether = typeof v.path === 'string' && parseHouseRoute(v.path, householdId)?.room === 'together' && new URL(v.path, 'https://hearth.invalid').searchParams.get('household') === householdId;
