...
export function PlanStudioClassic({ household, view, memberId, today, busy, onCommand, onSharedHerculesReply, onAskHercules, onContextChange, sourceFocus, goalsContent, contextIdentity, onOpenWorkspace, workspaceCards, embeddedSection, onEmbeddedClose }: PlanStudioProps & {
  ...
  const run = useCallback(async (command: (current: Household) => CommitResult) => {
    ...
    try { const result = await onCommand(command) as {ok?:boolean;household?:Household;userMessage?:string}|null; if(!result||result.ok===false||!result.household)throw new Error(result?.userMessage||"This Plan change was not saved. Your input is still here."); return result; }
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => { if (!sourceFocus.goalId && sourceFocus.label !== "Goals & reserves" && sourceFocus.planLineId) document.querySelector<HTMLElement>(`[data-plan-line-id="${CSS.escape(sourceFocus.planLineId)}"]`)?.focus(); });
  ...
  return <Root className={`plan-studio plan-studio--${scope}${embedded ? " plan-studio--embedded" : ""}`} aria-label={`${scope === "household" ? "Household" : "Personal"} Plan Studio`}>
    ...
    <section className="plan-studio__canvas">
      ...
      {section === "sitdown" && <section className="plan-section plan-sitdown plan-editor"><p className="kicker">Our check-in</p><h3>{scope === "personal" ? "Arrive with your own thoughts" : session?.state === "closed" ? "Our Chapter, ready to carry forward" : stages[stage]}</h3><Whisper mode="line">The Plan keeps a month. The Chapter keeps what we chose and learned.</Whisper><ModeLabel mode="private" /><label>Your private preparation<textarea value={privateNote ?? draft?.note ?? ""} onChange={event => setPrivateNote(event.target.value)} /></label><button disabled={locked} onClick={() => void run(current => savePlanDraft(current, { ...(draft ? { id: draft.id, expectedUpdatedAt: draft.updatedAt } : {}), memberId, createdBy: memberId, scope, targetMonth: month, baseVersionId: draft?.baseVersionId, lines: [...(draft?.lines ?? working)].map(line => ({ ...line, createdBy: memberId })), assumptions: draft?.assumptions ?? [...selection.assumptions], note: privateNote ?? draft?.note ?? "" }))}>Save preparation privately</button>{scope === "household" && <>{session?.state === "closed" && <button disabled={locked} onClick={() => void sharedTurn("We are beginning a new Sitdown.", { stage: 0 })}>Begin our next Sitdown</button>}<ol>{stages.map((title, index) => <li key={title} className={index === stage ? "active" : index < stage ? "done" : ""}>{index + 1}. {title}</li>)}</ol><div className="plan-stage-content">{stage === 0 ? <><SitdownBriefCard household={household} memberId={memberId} today={today} /><Whisper mode="line">Name one act of care, progress or honest recovery.</Whisper></> : stage === 1 ? <><ChapterClose household={household} memberId={memberId} today={today} sitdownId={session?.id ?? null} onCommand={run} busy={locked} /><button onClick={() => setSection("reflection")}>Compare intention with what actually happened</button></> : stage === 2 ? <><button onClick={() => setSection("assumptions")}>Refresh the money and timing assumptions</button><button onClick={() => setSection("overview")}>See now, this Chapter, ahead, and longer</button></> : stage === 3 ? <button onClick={() => setSection("learn")}>Learn one useful thing</button> : stage === 4 ? <div className="plan-editor-actions">{PLAN_LENSES.map(lens => <button key={lens} onClick={() => setSection(lens)}>Decide {lens}</button>)}<button onClick={() => setSection("bridge")}>Resolve shared offers and responsibilities</button></div> : stage === 5 ? <RitualForm household={household} memberId={memberId} onCommand={run} busy={locked} /> : stage === 6 ? <><button onClick={() => setSection("prepare")}>Preview costs that could change next month</button><button onClick={() => setSection("bridge")}>Check what is still waiting across the Bridge</button></> : <><p>Read the exact Plan below. Each partner acknowledges for themselves; then the next Chapter opens in this same Sitdown.</p><ChapterClose household={household} memberId={memberId} today={today} sitdownId={session?.id ?? null} onCommand={run} busy={locked} /></>}</div><div className="plan-shared-chat"><ModeLabel mode="shared" /><strong>Everything below is Shared</strong><p>Your private preparation is only shared if you deliberately write it here.</p>{session?.turns.map(turn => <blockquote key={turn.id}><b>{turn.role === "hercules" ? "Hercules" : household.members.find(row => row.id === turn.memberId)?.name ?? "Member"}</b>{turn.text}{turn.role === "member" && session.state === "active" && onSharedHerculesReply && !session.turns.some(reply => reply.inReplyToTurnId === turn.id) && turn.memberId === memberId && <button disabled={locked} onClick={() => void onSharedHerculesReply(session.id, turn.id).catch(() => setError("The talking point remains saved. Retry when connected."))}>Retry Hercules reply</button>}</blockquote>)}<form onSubmit={event => { event.preventDefault(); void sharedTurn(message, undefined, true); }}><label>A talking point we choose to share<textarea value={message} onChange={event => setMessage(event.target.value)} /></label><button disabled={locked || session?.state === "closed" || !message.trim()}>Share and ask Hercules</button></form></div><label>A decision to carry forward<input value={decision} onChange={event => setDecision(event.target.value)} /></label><label>One practical rhythm for this Chapter<input value={rhythm ?? session?.rhythm ?? ""} onChange={event => setRhythm(event.target.value)} placeholder="Ten minutes together on Sunday…" /></label><div className="plan-editor-actions"><button disabled={locked || session?.state === "closed" || stage === 0} onClick={() => void sharedTurn(`We returned to ${stages[Math.max(0, stage - 1)]}.`, { stage: Math.max(0, stage - 1), decision, rhythm: rhythm ?? session?.rhythm })}>Back, saving our work</button><button disabled={locked || session?.state === "closed" || stage === 7} onClick={() => void sharedTurn(`We completed ${stages[stage]}.${decision.trim() ? ` Decision: ${decision.trim()}` : ""}`, { stage: Math.min(7, stage + 1), decision, rhythm: rhythm ?? session?.rhythm })}>Save and continue</button>{stage === 7 && <button type="button" onClick={()=>setSection("review")}>Review our proposal</button>}{stage === 7 && <button disabled={locked || session?.state === "closed" || !acknowledgement?.complete} onClick={() => void sharedTurn(`We are carrying this Plan forward.${decision ? ` ${decision}` : ""}`, { stage, decision, rhythm: rhythm ?? session?.rhythm, close: true, planVersionId: version?.id })}>Close this Sitdown with our agreed Plan</button>}</div>{session?.decisions?.length ? <section><h4>What we chose</h4>{session.decisions.map(row => <p key={row.id}>{row.text}</p>)}<h4>What we carry forward</h4><p>{session.rhythm || "Choose one manageable rhythm."}</p><button onClick={() => setSection("reflection")}>Review what happened and what we learned</button></section> : null}</>}</section>}
      {section === "review" && draft && !scenario && !versionId && <footer className="plan-publish plan-editor"><strong>Private draft → exact review → our decision</strong><label>Why this version?<input value={reason} onChange={event => { setReason(event.target.value); setReviewedAt(null); }} /></label>{reviewedAt ? <section className="plan-disclosure-review"><h4>Review the exact {scope === "household" ? "Shared proposal" : "Personal version"}</h4><p>{working.length} lines and {selection.assumptions.length} assumptions. {scope === "household" ? "The labels, amounts, responsibility, dates, time constraints and next steps in these lines will be visible to your household. Your private preparation stays private." : "This creates an immutable private version."}</p><PlanDecisionReview household={household} lines={working} assumptions={selection.assumptions} /><button disabled={locked || reviewedAt !== draft.updatedAt} onClick={() => void run(current => scope === "household" ? proposeHouseholdPlan(current, { memberId, draftId: draft.id, expectedUpdatedAt: reviewedAt, expectedBaseVersionId: reviewedVersionId, reason, createdBy: memberId }) : lockPersonalPlan(current, { memberId, draftId: draft.id, expectedUpdatedAt: reviewedAt, expectedBaseVersionId: reviewedVersionId, reason, createdBy: memberId }))}>{scope === "household" ? "Share this exact proposal for independent agreement" : "Lock this exact private Plan"}</button>{reviewedAt !== draft.updatedAt && <p>The draft changed. Refresh the review before continuing.</p>}<button onClick={() => setReviewedAt(null)}>Return to editing</button></section> : <button disabled={locked} onClick={() => { setReviewedAt(draft.updatedAt); setReviewedVersionId(version?.id ?? null); }}>Review the complete proposal</button>}</footer>}
      {section === "review" && scope === "personal" && version && selection.kind === "version" && <section className="plan-section plan-disclosure-review"><h3>Your accepted Personal Plan</h3><p>Version {version.sequence} · {version.state} · {version.createdAt.slice(0,10)}</p><p>{version.reason || "No reason recorded."}</p><PlanDecisionReview household={household} lines={version.lines} assumptions={version.assumptions} /><p>This exact version stays private. You can begin a private draft to consider changes.</p><button disabled={locked} onClick={() => {if(draft){setVersionId(null);setScenarioId(null);setSection("overview");}else void saveWorking(working).then(saved=>{if(saved)setSection("overview");});}}>{draft?"Return to my working draft":"Start a private draft from this version"}</button></section>}
