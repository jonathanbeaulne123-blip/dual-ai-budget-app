...
import {
  PLAN_LENSES, addDays, createPlanScenario, currentPlanVersion, evaluatePlanDrift,
...
export type PlanStudioProps = {
  ...
  /** The Sitdown and household acknowledgement live at the Campfire (Tool Atlas K3, D3); this opens its sheet. */
  onOpenCampfire?: () => void;
};
 ...
 */
export function PlanStudioClassic({ household, view, memberId, today, busy, onCommand, onAskHercules, onContextChange, sourceFocus, goalsContent, contextIdentity, onOpenWorkspace, workspaceCards, onOpenCampfire, embeddedSection, onEmbeddedClose }: PlanStudioProps & {
  ...
  useEffect(() => { setReviewedAt(null); }, [month, memberId, scope]);
  ...
  return <Root className={`plan-studio plan-studio--${scope}${embedded ? " plan-studio--embedded" : ""}`} aria-label={`${scope === "household" ? "Household" : "Personal"} Plan Studio`}>
    {!embedded && <><header className="plan-studio__masthead"><div><p className="kicker">{scope === "household" ? "The kitchen table" : "My private path"}</p><h2>A plan for the life we choose</h2></div><div className="plan-month-switcher"><button aria-label="Previous month" onClick={() => monthChange(-1)}>←</button><strong>{month}</strong><button aria-label="Next month" onClick={() => monthChange(1)}>→</button><span>{scenario ? scenario.name : selection.kind === "draft" ? "Private draft" : version?.state ?? "Not started"}</span></div></header>
    ...
    <section className="plan-studio__canvas">
      ...
      {section === "sitdown" && <section className="plan-section plan-sitdown"><CampfireDoor household={household} memberId={memberId} today={today} onOpenCampfire={onOpenCampfire} why={scope === "personal" ? "The monthly Sitdown is the two of you at the Campfire." : "The Sitdown, the Chapter and the seal are one evening at the Campfire."} /></section>}
