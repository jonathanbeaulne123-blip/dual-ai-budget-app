import { shapeFundSourceClaims } from "../core/fundContributionSources.ts";
...
const PROJECTION_DIGEST_TABLES = [
  ...
  ["fund_contribution_source_claims", "id", "household_id"],
...
function projectBooksTables(household: Household, compiled: CompiledBooks, snapshotUpdatedAt: string): ProjectionTable[] {
  ...
  return [
    ...
    { table: "fund_events", keyColumn: "id", columns: ["id", "household_id", "fund_id", "kind", "amount_cents", "date_key", "created_by", "confirmed_by_member_id", "contributor_member_id", "destination_account_id", "related_event_id", "related_transaction_ids", "evidence_digests", "reconciliation_tied", "note", "created_at", "updated_at", "source_declaration"], rows: fund ? shapeHouseholdFundEvents(household.fundEvents).map((event) => [event.id, compiled.householdId, event.fundId, event.kind, event.amountCents, event.date, event.createdBy, event.confirmedByMemberId, event.contributorMemberId, event.destinationAccountId, event.relatedEventId, JSON.stringify(event.relatedTransactionIds), JSON.stringify(event.evidenceDigests), event.reconciliationTied, event.note, event.createdAt, event.updatedAt, event.sourceDeclaration ? JSON.stringify(event.sourceDeclaration) : null]) : [] },
    { table: "fund_contribution_source_claims", keyColumn: "id", columns: ["id","household_id","fund_id","owner_member_id","proposal_event_id","source_transaction_id","amount_cents","source_fingerprint","created_at","updated_at"], rows: shapeFundSourceClaims(household.fundContributionSourceClaims).map(c => [c.id,compiled.householdId,c.fundId,c.ownerMemberId,c.proposalEventId,c.sourceTransactionId,c.amountCents,c.sourceFingerprint,c.createdAt,c.updatedAt]) },
    ...
    { table: "fund_private_reconciliations", keyColumn: "id", columns: ["id", "household_id", "fund_id", "member_id", "date_key", "bank_total_cents", "operating_fund_cents", "kitty_cents", "personal_remainder_cents", "difference_cents", "tied", "shared_event_id", "created_at", "updated_at"], rows: fund && privateState ? privateState.reconciliations.map((reconciliation) => [reconciliation.id, compiled.householdId, reconciliation.fundId, reconciliation.memberId, reconciliation.date, reconciliation.bankTotalCents, reconciliation.operatingFundCents, reconciliation.kittyCents, reconciliation.personalRemainderCents, reconciliation.differenceCents, reconciliation.personalRemainderCents < 0 || reconciliation.bankTotalCents < reconciliation.operatingFundCents + reconciliation.kittyCents ? false : reconciliation.independentlyChecked === false ? null : reconciliation.differenceCents === 0, reconciliation.sharedEventId, reconciliation.createdAt, reconciliation.updatedAt]) : [] },
