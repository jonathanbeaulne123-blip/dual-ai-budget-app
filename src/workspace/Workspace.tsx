import { ExperienceWorktable, ExperienceArtifactSharing, type HearthsideWorkspaceBinding, type WorktableDraft } from '../hearthside/workspaceSurface.tsx';
import { workspaceExperienceDigest, workspaceExperienceKey, workspaceExperienceProjectId } from '../hearthside/workspaceContext.ts';
import { artifactPublication, assertArtifactPublicationReceipt, type ArtifactPublication, type ExperienceArtifactReview, type PreparedExperienceArtifact } from '../hearthside/workspacePublication.ts';
...
import { readEntryLocal, writeEntryLocal, clearEntryLocal } from '../hearthside/workspaceDraft.ts';
...
export function HerculesWorkspaceRoom(props: WorkspaceProps) {
  const key=JSON.stringify([props.identity,props.environment,props.householdId,props.memberId,props.hearthside?.experience.id??null,props.hearthside?.experience.version===2?'personal':'household']);
  return <HerculesWorkspaceScope key={key} {...props}/>;
}
function HerculesWorkspaceScope(props:WorkspaceProps){
  const { scene } = useAppearance();
  const theme = scene.theme;
  const experienceAudience=props.hearthside?.experience.version===2?'personal':'household';
  const personalExperience=experienceAudience==='personal';
  const scopeKey = props.hearthside?'hearthside-workspace:'+workspaceExperienceKey(props.hearthside.scope,props.hearthside.experience.id,experienceAudience):`hercules-workspace:${props.identity}:${props.environment}:${props.householdId}:${props.memberId}`;
  const experienceProjectId=props.hearthside?workspaceExperienceProjectId(props.hearthside.scope,props.hearthside.experience.id,experienceAudience):null;
  ...
  const [pendingWithdrawal,setPendingWithdrawal]=useState<ArtifactPublication|null>(()=>props.hearthside?readEntryLocal<ArtifactPublication>(scopeKey+':withdrawal'):null);
  const [moreExperienceCopies,setMoreExperienceCopies]=useState(false);
  const [experienceCopies,setExperienceCopies]=useState<PreparedExperienceArtifact[]>([]);
  const [deleteSource,setDeleteSource]=useState(false);
  const [experienceShare,setExperienceShare]=useState<ArtifactVersion|null>(()=>{
    if(!props.hearthside)return null;
    const saved=readEntryLocal<{review:ExperienceArtifactReview;pending:boolean}>(workspaceExperienceKey(props.hearthside.scope,props.hearthside.experience.id,experienceAudience)+':publication');
    return saved?.pending?{id:saved.review.artifactVersionId,artifactId:saved.review.artifactVersionId,title:saved.review.title,format:saved.review.format,content:saved.review.content,parentId:null,createdAt:'',author:'user',evidenceIds:[],validation:{status:'unchecked',details:'Original publication pending'}}:null;
  });
  ...
  const hearthside=props.hearthside?(personalExperience?props.hearthside:{...props.hearthside,onPublication:props.hearthside.onPublication??((review:ExperienceArtifactReview,digest:string)=>clientRef.current!.shareExperience(review,digest)),onWithdrawal:props.hearthside.onWithdrawal??((id:string)=>clientRef.current!.withdrawExperience(id))}):undefined;
  ...
  useEffect(() => { if (props.openProjectId && !experienceProjectId) {chooseProject(props.openProjectId);props.onProjectOpened?.();} }, [props.openProjectId]);
  useEffect(() => {
  ...
  async function submit(request: WorkspaceRequest) {
    ...
    try {
      ...
      if (alive.current) {
        ...
        if (error instanceof Error && /CHANGED|EXISTS|REUSED|^INVALID_|_LIMIT$|^TEXT_REQUIRED|TOO_LARGE|^EXPERIENCE_DISCLOSURE_REQUIRED$|^EXPERIENCE_SCOPE_READ_DENIED$/.test(error.message)) { setPending(null); await refresh(); setNotice(workspaceError(error)); }
      }
      return false;
  ...
  async function saveExperienceDraft(draft:WorktableDraft):Promise<{artifactId:string;versionId:string}|null>{
    if(!hearthside||!experienceProjectId||pending||saving)return null;
    let p=snapshotRef.current?.projects.find(p=>p.id===experienceProjectId);
    if(!p){const context=hearthside.experience;if(!await submit({commandId:crypto.randomUUID(),projectId:experienceProjectId,expectedRevision:0,command:{type:'create-experience',id:experienceProjectId,context,confirmDigest:workspaceExperienceDigest(context)}}))return null;p=snapshotRef.current?.projects.find(p=>p.id===experienceProjectId);}
    if(!p)return null;setSelected(p.id);
    const previous=latestArtifacts(p).find(a=>a.artifactId===draft.artifactId);
    if(previous?.content===draft.content&&previous.title===draft.title)return {artifactId:previous.artifactId,versionId:previous.id};
    if(previous&&previous.id!==draft.versionId){setNotice('This working source changed elsewhere. Your local draft remains intact. Compare it in the Working area before saving another version.');return null;}
    const cmd:WorkspaceCommand=previous?{type:'edit-artifact',id:crypto.randomUUID(),artifactId:previous.artifactId,parentId:previous.id,title:draft.title,content:draft.content}:{type:'create-artifact',id:draft.artifactId,title:draft.title,content:draft.content,format:'markdown'};
    if(!await command(cmd,p))return null;
    const saved=latestArtifacts(snapshotRef.current!.projects.find(row=>row.id===p!.id)!).find(a=>a.artifactId===draft.artifactId);setSurface('work');return saved?{artifactId:saved.artifactId,versionId:saved.id}:null;
  }
  async function refreshExperienceCopies(append=false){if(!hearthside||personalExperience)return;try{const copies=await clientRef.current!.experienceArtifacts(hearthside.experience.id,append?experienceCopies.at(-1)?.id??'':'');if(alive.current){setExperienceCopies(previous=>append?[...new Map([...previous,...copies].map(copy=>[copy.id,copy])).values()]:copies);setMoreExperienceCopies(copies.length===4);}}catch{if(alive.current)setNotice('Shared copies could not be loaded. Reconnect to check their current access.');}}
  async function withdrawExperienceCopy(publication:ArtifactPublication){
    if(!hearthside?.onWithdrawal||saving)return;
    if(pendingWithdrawal&&pendingWithdrawal.id!==publication.id){setNotice('Check the original withdrawal before withdrawing another copy.');return;}
    if(!writeEntryLocal(scopeKey+':withdrawal',publication)){setNotice('Allow private recovery storage before withdrawing this copy.');return;}
    setPendingWithdrawal(publication);setSaving(true);
    try{assertArtifactPublicationReceipt(await hearthside.onWithdrawal(publication.id),publication);if(!alive.current)return;clearEntryLocal(scopeKey+':withdrawal');setPendingWithdrawal(null);await refreshExperienceCopies();setNotice('The shared copy was withdrawn. Private source work is unchanged.');}
    catch{if(alive.current)setNotice('Withdrawal needs checking. Retry its original receipt to finish recording the withdrawal.');}
    finally{if(alive.current)setSaving(false);}
  }
  async function send() {
  ...
  return <section className={`hercules-workspace hw--${props.mode} hw--${theme}`} data-workspace-scope={props.view} aria-label="Hercules workspace" onKeyDown={event => { if (event.key === 'Escape' && props.mode === 'compact') props.onClose(); }}>
    ...
    {pendingWithdrawal&&<div className="hw-notice" role="status"><p>A shared-copy withdrawal needs checking. Its original identity is preserved.</p><button disabled={saving} onClick={()=>void withdrawExperienceCopy(pendingWithdrawal)}>Check original withdrawal</button></div>}
    ...
    <div className="hw-layout">
      <aside className="hw-projects" aria-label="Your projects"><div className="hw-projects-heading"><h2>On the worktable</h2>{!hearthside&&<button disabled={!snapshot || saving || !!pending} onClick={() => void newProject()}>New project</button>}</div>
        <nav aria-label="Projects">{[...(snapshot?.projects.filter(p=>!experienceProjectId||p.id===experienceProjectId) ?? [])].sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)).map(p => <button key={p.id} aria-current={selected === p.id ? 'page' : undefined} onClick={() => { chooseProject(p.id); }}><strong>{p.title}</strong><small>{p.runs.at(-1)?.progress ?? 'An open possibility'}</small></button>)}</nav>
        {!snapshot?.projects.length && <p>Your ideas can grow here, over one conversation or several months.</p>}
        <details className="hw-privacy"><summary>What Hercules can use</summary><p>{project?.experience||hearthside?'This private project is limited to the selected intention and material you deliberately place here. Personal books and unrelated Household information are excluded. Review the exact provider disclosure above before activating any run.':'This conversation is yours. An activated run can use your Personal books and authorized Household information as distinct sources. Another member’s Personal books are excluded.'}</p><p>Your instructions, selected files, project context and requested tool results are sent to Gemini when execution is activated. Sharing is a separate reviewed copy.</p></details>
        {(!hearthside||personalExperience)&&<details className="hw-shared-shelf"><summary>Shared with the household</summary><button onClick={()=>void clientRef.current!.sharedArtifacts().then(setShared).catch(()=>setNotice('Shared work could not be loaded.'))}>Refresh shared copies</button>{shared.map(a=><details key={a.id}><summary>{a.title}</summary><pre>{a.content}</pre></details>)}</details>}
        {hearthside&&!personalExperience&&<details className="hw-shared-shelf"><summary>Copies placed with this intention</summary><button onClick={()=>void refreshExperienceCopies()}>Check shared copies</button>{experienceCopies.map(copy=><details key={copy.id}><summary>{copy.title}</summary><pre>{copy.content}</pre><p>Adding this copy makes its exact visible content available to this private project and any later activated run you request.</p><button disabled={!project||saving||!!pending} onClick={()=>void command({type:'adopt-experience-copy',id:crypto.randomUUID(),copyId:copy.id,contentDigest:copy.contentDigest})}>Add this reviewed copy to my project</button>{copy.sharedBy===props.memberId&&<button disabled={saving||!!pendingWithdrawal} onClick={()=>void withdrawExperienceCopy(artifactPublication(copy,'withdrawn'))}>Withdraw this shared copy</button>}</details>)}{moreExperienceCopies&&<button onClick={()=>void refreshExperienceCopies(true)}>More shared copies</button>}</details>}
        <button className="hw-legacy" onClick={props.onLegacy}>Guided actions & earlier conversations</button>
      </aside>
      <main className="hw-main">
      {!project ? hearthside?<p className="hw-first-message">This intention has its own worktable above. Save a piece of writing to begin its private Workspace project.</p>:<div className="hw-welcome"><div className="hw-desk-art" aria-hidden="true"><span className="hw-book">a little room<br/>for possibility</span><span className="hw-pencil"/><span className="hw-cup">✦</span></div><h2>What shall we work on?</h2><p>A small date. A big decision. Something you want to understand or create.</p><div className="hw-starters">{starters.map(([title, prompt]) => <button key={title} disabled={!snapshot || saving || !!pending} onClick={() => void newProject(title, prompt)}><strong>{title}</strong><span>Begin together →</span></button>)}</div></div> : <>
