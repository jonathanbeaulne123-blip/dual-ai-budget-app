...
function HerculesWorkspaceScope(props:WorkspaceProps){
  ...
  const [selected, setSelected] = useState<string | null>(() => experienceProjectId??readEntryLocal<{ projectId?: string }>(scopeKey)?.projectId ?? null);
  ...
  useEffect(()=>{if(project?.experience&&!props.hearthside)props.onExperienceOpened?.(project.experience.context.id);},[project?.id,props.hearthside?.experience.id]);
  ...
  useEffect(() => {
    ...
    return () => { alive.current = false; clientRef.current?.cancelRequests();clearInterval(interval); window.removeEventListener('online', reconnect); window.removeEventListener('focus', reconnect); };
  }, []);
