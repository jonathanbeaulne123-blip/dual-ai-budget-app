...
import {commitVillageArrangement} from "./hearthside/villageCommand.ts";
...
import type {HearthsideWorkspaceBinding} from './hearthside/workspaceSurface.tsx';
...
import { HOUSE_WORLD_ENABLED, HOUSE_PLACES, TARGET_NAMES, houseTargetRoute, houseToolPlace, houseTabForRoute, houseLifeRoute, houseRouteFromLife, readHouseReturn, resolveHouseRouteScope, saveHouseReturn, type HouseIdentity, houseIdentity } from './house/navigation.ts';
...
export function App() {
  ...
  const [workspaceExperienceSelection,setWorkspaceExperienceSelection]=useState<{scope:string;id:string}|null>(null);
  ...
  },[houseRoute,environment,household?.householdId,session?.memberId,session?.view]);
  useEffect(() => {
  ...
  const [commandProgressAudience, setCommandProgressAudience] = useState<CommandProgressAudience>("household");
  ...
  const assertMemberPersonalUpdate = useCallback((current: Household, result: CommitResult): void => {
    ...
    if (commandKind === "personal-life") {
      const before = splitForSync(current, who);
      const after = splitForSync(result.household, who);
      const normalizedAfter = structuredClone(after.personal);
      normalizedAfter.personalLife = before.personal.personalLife;
      if (
        JSON.stringify(after.shared) === JSON.stringify(before.shared)
        && JSON.stringify(normalizedAfter) === JSON.stringify(before.personal)
      ) return;
      throw new ValidationError("Only you can change your private wishes, experiences and memories.");
    }
    if (
  ...
  useEffect(() => {
    const returned = () => {
      const completed = nativeAuthController()?.takeCompletion();
      if (!completed) return;
      // The environment comes from the secure initiating flow, never from an arbitrary URL.
      if (completed !== environment) changeEnvironment(completed);
      setSupabaseAuthReturned(true);
    };
    returned(); window.addEventListener(NATIVE_AUTH_EVENT, returned);
    return () => window.removeEventListener(NATIVE_AUTH_EVENT, returned);
  }, [environment]);
  ...
  // The backtick flips between the two worlds (Simple View Desk S1); never Tab, never while typing.
  useEditionFlipKey(HARBOUR_ENABLED && view === "household" && Boolean(household && session));
  ...
  const workspaceUiScopeKey = JSON.stringify([ledgerRenderScopeKey,view]);
  const personalExperienceWorkspaceSelected = view === "personal" && workspaceExperienceSelection?.scope === workspaceUiScopeKey;
  const workspaceEnabled = workspaceCapabilityEnabled && (view === "household" || personalExperienceWorkspaceSelected);
  useEffect(() => { setOnboardingBooksOpen(false);setWorkspaceExperienceSelection(null);setWorkspaceSnapshot(null);setWorkspaceProjectId(null);setWorkspaceCompact(false); setCommandProgressPhase("idle"); setCommandProgressAudience("household"); setLedgerCommandId(undefined); }, [workspaceUiScopeKey]);
  const nativeAuthScopeRef = useRef(ledgerRenderScopeKey);
  useEffect(() => {
    const scopeChanged = nativeAuthScopeRef.current !== ledgerRenderScopeKey;
    nativeAuthScopeRef.current = ledgerRenderScopeKey;
    const auth = loadSupabaseSession(environment);
    const scope = household && memberId && auth ? {environment, householdId: household.householdId, memberId, subject: auth.userId} : null;
    void nativeWidgetController()?.enterScope(scope).catch(() => setError("The previous home-screen sculpture could not be cleared. Open its widget controls and retry."));
  ...
  const commandProgressDisplay = useMemo(
    ...
    [commandProgressPhase, commandProgressAudience, household?.linked, environment],
  );
  ...
  async function startQrInviteGoogleSignIn(token: string, inviteEnvironment: Environment = environment): Promise<void> {
    try {
    cancelAccountFlow();
    ...
    await clearSupabaseSession(inviteEnvironment);
    startSupabaseGoogleSignIn(
      inviteEnvironment,
    ...
    } catch (caught) { setInviteFlowState("error"); setError(caught instanceof Error ? caught.message : "Secure sign-in could not start."); }
  }
    ...
    try {
      ...
      if (!found.length) {
        ...
        if (environment === "production") {
          if (supabaseAuthEnabled()) await clearSupabaseSession(environment);
  ...
  const commitHousehold = useCallback(async (
    ...
    const presentSetCommandProgressAudience = present(setCommandProgressAudience);
    const presentSetLedgerCommandId = present(setLedgerCommandId);
    const presentSetSyncState = present(setSyncState);
    const previous = householdRef.current;
    const progressAudience = options?.audience ?? "household";
    ...
    if (useLedgerSync && previous && (previous.linked || localLedgerIdentity(actorId ?? session?.memberId ?? ""))) {
      ...
      presentSetCommandProgressAudience(progressAudience);
      ...
      try {
        ...
        presentSetCommandProgressAudience(accepted.persistenceScope === "member-personal" ? "personal" : "household");
  ...
  const run = useCallback(function run(fn: (current: Household) => CommitResult, options?: {
    ...
    return enqueueWrite(async () => {
      ...
      try {
        ...
        if(['hercules-companion-gallery','hercules-play','hearthside'].includes(result.undo.commandKind??'')&&!ledgerSyncRef.current)throw new ValidationError('Connect to your household to save something shared.');
        if (memberPersonal) {
        ...
        const outcome = await commitHousehold(
          ...
          memberPersonal ? result.personalMemberId : undefined,
          {
            ...
            audience: memberPersonal ? "personal" : "household",
    ...
    return enqueueScopedWrite(enqueueWrite, reviewedKitchenScope, () => ({
      ...
      try {
        ...
        const outcome = await commitHousehold(
          ...
          result.undo,
          memberPersonal ? result.personalMemberId : undefined,
          { isCurrent: renderedWriteIsCurrent, scopeIsCurrent: renderedWriteIsCurrent, confirmationId: options?.confirmationId, onDefinitiveRejected, audience: memberPersonal ? "personal" : "household" },
        );
  ...
  async function clearThisPhoneNow() {
    ...
    try { await clearSupabaseSession(environment); }
    catch { clearThisPhoneInFlightRef.current = false; setError("Secure sign-out could not finish. Unlock this device and retry."); return; }
  ...
  async function signOutWelcomeGoogle() {
    ...
    setWelcomeMode("home");
    setError("");
    } catch { setError("Secure sign-out could not finish. Unlock this device and retry."); }
  }
  ...
  async function tryInviteWithAnotherGoogleAccount() {
    try {
    clearWorkHandoff(window.sessionStorage);
    ...
    } catch { setError("Secure account switching could not finish. Unlock this device and retry."); }
  }
  ...
  if (returningToAnotherIdentity) {
    return <div className="welcome"><ThemeSceneHeading /><section className="welcome-card"><h1>Confirm access to the saved books</h1><p>The saved ledger is still on this device. The current Google account belongs to a different person, so its contents are hidden.</p><button className="primary" type="button" onClick={async () => {
      try {
      cancelAccountFlow(); await clearSupabaseSession(environment); clearGoogleSessions(environment);
      rememberWelcomeGoogleIntent("login");
  ...
  function goTab(next: Tab, requestedBoardSurface?: "practical", houseNavigation?: { route: HouseRoute; history: "push" | "replace" }) {
    ...
    if ((HEARTHSIDE_FLAGS.presentation || HOUSE_WORLD_ENABLED) && next === "together") next = "play";
    if(next === "play" && !PLAY_ENABLED && !HEARTHSIDE_FLAGS.presentation && !HOUSE_WORLD_ENABLED)next="together";
    ...
    const url = new URL(window.location.href);
    if (!HOUSE_WORLD_ENABLED && HEARTHSIDE_FLAGS.presentation && next === "play" && requestedBoardSurface && household) {
      ...
      if (path !== `${url.pathname}${url.search}`) window.history.pushState({ hearthTab: "play" }, "", path);
      window.dispatchEvent(new PopStateEvent("popstate"));
      return;
    }
    ...
    if (nextHouse) {
      ...
      if (path !== current) {
        ...
        if (houseNavigation?.history === "push" || HOUSE_WORLD_ENABLED && !houseNavigation) window.history.pushState(state, "", path);
  ...
  function goHouse(room:HouseRoom,level:HouseLevel,replace=false){
    if(!household)return;
    const route:HouseRoute={room,level,householdId:household.householdId,...(HOUSE_WORLD_ENABLED?{scope:view,object:houseRoute?.scope===view&&houseRoute.object?.startsWith("experience/")?houseRoute.object:undefined}:{})};
    const next=tabForHouseRoute(route);
    if(room==="kitchen-table")setWorkspaceCompact(!HOUSE_WORLD_ENABLED&&level==="middle");
    else setWorkspaceCompact(false);
    goTab(next, undefined, {route, history:replace?"replace":"push"});
    window.dispatchEvent(new PopStateEvent("popstate"));
  ...
  function putHouseObjectBack(){
    if(!household||!session)return;
    const back=readHouseReturn(localStorage,{environment,householdId:household.householdId,memberId:session.memberId,scope:view},houseReturnSlot(activeHouseRoute));
  ...
  function openExperienceWorkspace(experience:SharedExperience){
    setWorkspaceExperienceSelection({scope:workspaceUiScopeKey,id:experience.id});setWorkspaceProjectId(null);setWorkspaceCompact(false);openHearthsideTool('hercules',undefined,experience.title);
  }
  function openPersonalExperienceWorkspace(experience:PersonalLifeExperience){
    if(!workspaceCapabilityEnabled||view!=="personal"||experience.createdBy!==actorId||experience.state==="archived")return;
    setWorkspaceExperienceSelection({scope:workspaceUiScopeKey,id:experience.id});setWorkspaceProjectId(null);setWorkspaceCompact(false);goTab("hercules");
  }
  function returnFromExperienceWorkspace(){
    if(!household)return;
    if(view==="personal"){
      const selectedId=workspaceExperienceSelection?.scope===workspaceUiScopeKey?workspaceExperienceSelection.id:null;
      setWorkspaceExperienceSelection(null);setWorkspaceCompact(false);
  ...
  function selectedExperienceWorkspace():HearthsideWorkspaceBinding|undefined{
    ...
    const scope={identity:ledgerRenderScopeKey,environment,householdId:household.householdId,memberId:actorId};
    if(view==="personal"){
      try{const document=decodePersonalLife(household.personalLife,actorId),experience=document.experiences.find(row=>row.id===workspaceExperienceSelection.id&&row.state!=="archived");if(!experience||experience.createdBy!==actorId)return;return {scope,experience:personalWorkspaceExperienceContext(experience,actorId),onReturn:returnFromExperienceWorkspace};}catch{return;}
    }
    ...
    return {scope,experience:workspaceExperienceContext(experience),onReturn:returnFromExperienceWorkspace};
  }
  ...
  async function readAcceptedKittyCommand(id:string,signal?:AbortSignal){
    ...
    const value=await client.acceptedCommand(id,signal);
    if(client!==ledgerSyncRef.current||signal?.aborted||!renderedWriteIsCurrent())throw Error('SCOPE_CLOSED');return value;
  }
  ...
  const hearthsideOpen = (HEARTHSIDE_FLAGS.presentation || HOUSE_WORLD_ENABLED) && view === "household" && tab === "play";
  ...
  const queenWorldHome = !HOUSE_WORLD_ENABLED && tab === "home" && view === "household" && Boolean(dashboard) && householdHomeV2Enabled() && queensNestEnabled();
  ...
  /** Little Harbour: the household switcher and the space switch render here, or inside the quick sheet when the Court owns the route. */
  const householdSwitcherNode = replicas.length > 1 && !queenWorldHome ? (
        <details className="ledger-switcher">
          <summary>Switch household</summary>
          <p className="muted">Households available on this device. Google membership remains the authority for hosted access.</p>
          <div className="ledger-switcher__grid">
  ...
  return (
    <HearthsideDesignProvider key={ledgerRenderScopeKey} household={household} memberId={session.memberId} identity={ledgerRenderScopeKey} source={()=>ledgerSyncRef.current}><WornLookContext.Provider value={import.meta.env.VITE_HERCULES_DRESSING_ROOM==='1'&&household.companionProfile?.scope.memberId===session.memberId?household.companionProfile.wornLook.value:null}><div className="app" data-house-world={HOUSE_WORLD_ENABLED||undefined} data-harbour-court={harbourOwnsRoute(activeHouseRoute,view)&&!activeHouseRoute.surface||undefined} data-harbour-door={harbourOwnsRoute(activeHouseRoute,view)&&Boolean(activeHouseRoute.surface)||undefined} data-ledger-mode={view} data-ledger-tab={tab} data-world-home={queenWorldHome ? "true" : undefined} data-books-readiness={booksReadiness.phase} data-ledger-live={useLedgerSync && realtimeStatus === "SUBSCRIBED"} data-ledger-transaction-count={household.transactions.length}>
      ...
      {!HOUSE_WORLD_ENABLED && !hearthsideOpen && !queenWorldHome && <><ThemeSceneHeading home={tab === "home"} calendar={tab === "calendar"} plan={tab === "plan"} more={tab === "more"} books={tab === "ledger" || tab === "timeMachine"} />
      ...
      {HOUSE_WORLD_ENABLED&&activeHouseRoute.surface==="conversation"&&<KitchenFolio key={`${ledgerRenderScopeKey}:${view}`} identity={`${ledgerRenderScopeKey}:${view}`} household={household} memberId={actorId} view={view} today={today} route={activeHouseRoute} onCommand={runKitchen} onNavigate={navigateHouseSurface} onOpenPlan={()=>{}} onOpenCalendar={()=>goTab("calendar",undefined,{route:houseTargetRoute(activeHouseRoute,"calendar"),history:"push"})} onOpenHercules={()=>openLegacyHercules()}/>}
      {workspaceEnabled && <HerculesWorkspaceRoom
        hearthside={selectedExperienceWorkspace()} onExperienceOpened={id=>{if(household.hearthside?.experiences.some(e=>e.id===id&&e.state!=="archived"))setWorkspaceExperienceSelection({scope:workspaceUiScopeKey,id});}}
        getReportContext={captureReportContext} reporterName={household.members.find(m => m.id === actorId)?.name}
        reportRequest={bugReportRequest?.identity === ledgerRenderScopeKey ? bugReportRequest : null} onReportOpened={() => setBugReportRequest(null)} onPrivateReportRequested={view==="household"?openPrivateBugReport:undefined}
        key={`workspace:${workspaceUiScopeKey}`} identity={workspaceUiScopeKey} environment={environment} householdId={household.householdId} memberId={actorId} view={view}
        mode={workspaceMode}
        ...
        isCurrent={() => environmentRef.current === environment && householdRef.current?.householdId === household.householdId && sessionRef.current?.memberId === actorId && sessionRef.current?.view === view}
        onExpand={() => { setWorkspaceCompact(false); goTab("hercules"); }}
        onClose={() => { if(selectedExperienceWorkspace()){returnFromExperienceWorkspace();return;}setWorkspaceCompact(false); if (tab === "hercules") goTab("home"); }}
      ...
      {(HEARTHSIDE_FLAGS.presentation || HOUSE_WORLD_ENABLED) && (tab === "play" || tab === "together") && view === "household" && <PlayBoundary onExit={() => goTab("home")} exitLabel="Return Home"><Suspense fallback={<p>Opening our shared home…</p>}><Hearthside
        controlledRoute={HOUSE_WORLD_ENABLED?houseLifeRoute(activeHouseRoute):undefined} onReturnToOrigin={HOUSE_WORLD_ENABLED?putHouseObjectBack:undefined} onNavigate={HOUSE_WORLD_ENABLED?(next,replace)=>{const toolRoute=houseRouteFromLife(next,view);const route=activeHouseRoute.village?{...toolRoute,room:activeHouseRoute.room,level:activeHouseRoute.level,village:activeHouseRoute.village}:toolRoute;navigateHouseSurface(route,replace);}:undefined}
        ...
        onReference={(reference,experience) => {
          ...
          if (reference.kind === "calendar-event") {openHearthsideTool("calendar",reference,experience.title);return;}
          herculesSourceScope.current = `${environment}:${household.householdId}:${session.memberId}:household`;
          setHerculesSourceFocus({route:"plan",view:"household",label:"Our connected intention",...(reference.kind === "plan-line" ? {planVersionId:reference.planVersionId,planLineId:reference.id} : {}),...(reference.kind === "bank" ? {goalId:reference.id} : {})});openHearthsideTool("plan",reference,experience.title);
        }}
        letters={(onClose,theme)=><Suspense fallback={<p>Opening your private writing desk…</p>}><HearthsideLetters household={household} memberId={actorId} identity={ledgerRenderScopeKey} connected={useLedgerSync && realtimeStatus === "SUBSCRIBED"} source={()=>ledgerSyncRef.current} theme={theme} onClose={onClose}/></Suspense>}
      ...
      {tab === "home" && dashboard && view === "household" && (householdHomeV2Enabled() || HEARTHSIDE_FLAGS.presentation || HOUSE_WORLD_ENABLED) && (
        <HouseholdHome
          key={`${ledgerRenderScopeKey}:${activeHouseRoute.object??""}`}
          household={household}
          memberId={actorId}
          today={today}
    ...
    clearWorkHandoff(window.sessionStorage);
              ...
              clearContinuityOutboxForHousehold(environment, household.householdId);
              void clearStagedHouseholdBooks(environment, household.householdId);
              try { await clearSupabaseSession(environment); } catch { setError("Device access was revoked. Secure credential removal needs a retry."); }
              setSyncState("error");
            }}
          />
          </DeferredSurface>
          ...
          <HerculesProPermissionsCard
            ...
            onChangeRequested={async (permissions) => {
              ...
              const outcome = await enqueueWrite(async () => {
                ...
                return commitHousehold(result.household, result.undo, result.personalMemberId, { forceFlush: true, audience: "personal" });
              });
