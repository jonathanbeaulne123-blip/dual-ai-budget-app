...
export function createLibrary(scene: THREE.Scene, options: LibraryOptions): PlaceHandle {
  ...
  const anchorList = (): Anchor[] => [
    ...
    { id: "time-machine", position: at(tx, 1.05, tz), zone: "station", label: "The month desk, thumbing back through the leaves. Open the Standing Book.", door: { target: "books", object: "chapter/record" } },
  ...
  const regionList = (): Region[] => [
    ...
    { id: "time-machine", group: "library", label: "The month desk", box: box(tx - 0.55, 0.4, tz - 0.7, tx + 0.55, 1.5, tz + 0.7) },
