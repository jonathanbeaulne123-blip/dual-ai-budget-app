...
- **Lips.** `lip` is set on the last `LIP_BAND` = 0.15 of every transition: quarterpipe, mini and bowl coping, plus the kicker end, where `kind` is `'metal'`. `lipYaw` is the uphill direction, the way you leave the ramp; it is opposite the horizontal part of the normal. `vert` is true when the authored angle is ≥ 80°, which today means the Chimney (84°) and the Kettle (80°). On tight radii the strip starts near 60°. **Launch where the rider leaves the strip on its uphill side** (the coping line), using the normal there. Do not launch on entry.
...
- `test/harbour-skate-model.test.ts` has two v1 tests that now fail by design: "rides the physical ramp profile" (`SKATE_RAMPS` is empty) and "discovers only spots visited" (it expects 6 spots; there are 7). `SkateHUD.tsx` copy says "find all six spots".
