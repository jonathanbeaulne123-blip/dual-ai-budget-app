...
export const BOOK_TABS: readonly {id: BookTab; label: string; short: string; title: string}[] = [
  {id: 'explore', label: 'Explore', short: 'Explore', title: 'Find your next line'},
  {id: 'challenges', label: 'Challenges', short: 'Goals', title: 'Own the spot'},
  {id: 'tricks', label: 'Trick book', short: 'Tricks', title: 'Every trick, drawn'},
  {id: 'decks', label: 'Decks', short: 'Decks', title: 'A board of your own'},
...
export function PauseBook(p: Props) {
  ...
  return <section ref={ref} className="skate-book" role="dialog" aria-modal="true" aria-labelledby={`${id}-title`} onKeyDown={onKeyDown}>
    <header className="skate-book__head">
      <div><span className="skate-book__eyebrow"><span className="skate-book__club">Tideline Skate Club · </span>paused</span><h2 id={`${id}-title`}>{meta.title}</h2></div>
    ...
    <div className="skate-book__tabs" role="tablist" aria-label="Skate book">
      {BOOK_TABS.map(t => <button key={t.id} type="button" role="tab" id={`${id}-tab-${t.id}`} aria-selected={t.id === p.tab} aria-controls={`${id}-panel`} aria-label={t.label} tabIndex={t.id === p.tab ? 0 : -1} onClick={() => p.onTab(t.id)} onKeyDown={onTabKey}><span className="skate-tab__long" aria-hidden="true">{t.label}</span><span className="skate-tab__short" aria-hidden="true">{t.short}</span></button>)}
    </div>
    <div className="skate-book__page" role="tabpanel" id={`${id}-panel`} aria-labelledby={`${id}-tab-${p.tab}`}>
      {p.tab === 'explore' && <>
        <p>Roll wherever you like. Spots wait to be found all over the island, and a route is there when you fancy a little race against the tide. Nothing here is required.</p>
        <h3>Routes</h3>
        ...
        <p className="skate-book__note">Skate together: when you both turn on Walk together, the other rider appears on this island while you are both sharing it.</p>
      </>}
      {p.tab === 'challenges' && <>
      ...
      {p.tab === 'decks' && <>
        <p>Every deck rides the same; they are just nice to look at. Find quiet corners of the island, or own a few spots, to collect the rest.</p>
