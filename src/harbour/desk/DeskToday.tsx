...
function DeskHouseholdToday({ household, memberId, scope, today, reading, onOpen, onTalk }: DeskPageProps) {
  ...
  return <div className="desk-today" data-desk-sitdown={sitdown?.why} data-desk-post={post ? "" : undefined}>
    ...
    <Sundial known={walk !== null} next={next} onOpen={onOpen} />
...
function Sundial({ known, next, onOpen }: { known:boolean; next: DeskNext | null; onOpen: (target: string, object?: string) => void }) {
  ...
  return <button type="button" className="desk-card desk-sundial" data-desk-sundial={next ? "dated" : "clear"} onClick={() => onOpen("cellar-bills")}
    aria-label={next ? `Sundial. Next to leave the Fund: ${next.label}, ${engravedCents(next.amountCents)}, ${shortDate(next.date)}, ${when}.${warning ? ` ${warning}` : ""} Read the bill jars.` : !known ? "Sundial. Checking next payments. Read the bill jars." : "Sundial. Nothing dated still has to leave the Fund this month. Read the bill jars."}>
    ...
    <span className="desk-sundial__words">
      ...
      <strong className="desk-sundial__label">{next ? next.label : known ? "Nothing dated" : "Checking next payments"}</strong>
      <span className="desk-card__line">{next ? <><span className="desk-figure desk-figure--inline">{engravedCents(next.amountCents)}</span> · {shortDate(next.date)} · {when}</> : known ? "Nothing else leaves the Fund this month" : "Open the Fund to review the month"}</span>
