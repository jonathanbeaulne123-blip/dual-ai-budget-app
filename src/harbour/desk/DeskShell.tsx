...
import { useSupportedDesk } from "./supportedDesk.ts";
...
export function DeskShell({ headerFlip, household, memberId, scope, today, reading, theme = "classic", status = "flat", onOpen, onQuickSheet, onTalk, onFlip, spaceSlot, initialPage, titleId, ready = true, interpretationGate, hidden, context }: DeskShellProps) {
  ...
  function onSwipeStart(event: ReactPointerEvent<HTMLDivElement>) {
    if (event.pointerType === "mouse" || !event.isPrimary || (event.target instanceof Element && event.target.closest("button,a,input,select,textarea,[role=slider],table,[data-desk-no-swipe]"))) return;
  ...
  return <section hidden={hidden} className={`desk desk--${theme}`} data-desk="" data-desk-scope={scope} data-desk-page={page.id} data-desk-status={status} aria-labelledby={headingId}
    ...
    <header className="desk__header" data-desk-header-flip={showFlip ? "" : undefined}>
      ...
      {!harbourDrawable && <p className="desk__undrawn" id={`${base}-undrawn`} role="status">{editionUnavailableWords(availability.reason) ?? "The Harbour could not be drawn here. Everything is on the Desk."}</p>}
    </header>
    {context}
    {supported.status && <p className="desk__support" role="status">{supported.status}</p>}
    ...
    <p className="desk__sign" data-desk-sign="">{page.subtitle({ reading: supported.source?.reading ?? null, scope })}</p>
    <div className="desk__page" role="tabpanel" id={panelId} aria-labelledby={tabId(page.id)} tabIndex={-1}
      ...
      {supported.source ? <Page key={page.id} {...supported.source!} onOpen={onOpen} onTalk={talk} /> : <div className="desk-card"><h2>Waiting for a supported reading</h2><p>Your tools remain available while the books reconnect.</p><button className="desk-door" onClick={()=>onOpen(scope === "personal" ? "books" : "fund")}>Open {scope === "personal" ? "Books" : "the Fund"}</button><button className="desk-door" onClick={()=>onOpen("calendar")}>Open Calendar</button></div>}
    </div>
  </section>;
}
