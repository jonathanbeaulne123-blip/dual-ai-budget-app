...
export const DESK_PLACE_DOORS:Record<HarbourPlaceId,Door[]>={
 court:[{label:'The Fund',target:'fund'},{label:'Supporting records',target:'books'}],
 bank:[{label:'The Fund',target:'fund'},{label:'Books',target:'books'}],
 kitchen:[{label:'Our conversation',target:'conversation'},{label:'The kitchen table',target:'plan-studio'}],
 tower:[{label:'Our goals',target:'loft-banks'}],
 cellar:[{label:'Bill jars',target:'cellar-bills'},{label:'Calendar',target:'calendar'}],
 atlas:[{label:'The Journey map',target:'journey'}],
 library:[{label:'Open the standing book',target:'books'}],
 glasshouse:[{label:'Glasshouse · steps',target:'planner'},{label:'Calendar',target:'calendar'}],
 ...
 campfire:[{label:'Talk with Hercules',target:'hercules'},{label:'The kitchen table',target:'plan-studio'},{label:'The Journey map',target:'journey'}],
};
const LOCATIONS:Record<HarbourPlaceId,string>={court:'The square · at the waterfront',bank:'The square · the Fund bank',kitchen:'Hearth Terrace · our home',tower:'Hearth Terrace · our loft',cellar:'Hearth Terrace · our cellar',atlas:'Hearth Terrace · the Atlas',library:'Library Woods · beside the gorge',glasshouse:'Glasshouse Meadows · among the gardens',cottage:'Orchard Hollow · Hercules’s cottage',kiln:'The square · the Kiln',boathouse:'The square · the quay',campfire:'Waterfront · by the fire'};
...
export function DeskPlace({outdoorAt,reading,place,onOpen,onGuide,onVisit}:{outdoorAt?:WorldPoint3;reading?:HarbourReading;place:HarbourPlaceId;onOpen:(target:string,object?:string)=>void;onGuide:()=>void;onVisit:(place:HarbourPlaceId)=>void}){
 ...
 return <section className="desk-place" aria-label="Your place in the neighbourhood">
  ...
  {atHome&&<nav aria-label="Rooms in our home">{(['kitchen','tower','cellar','atlas'] as const).map(id=><button type="button" key={id} aria-current={id===place?'location':undefined} onClick={()=>onVisit(id)}>{({kitchen:'Kitchen',tower:'Loft',cellar:'Cellar',atlas:'Atlas'})[id]}</button>)}</nav>}</div>
  <button className="desk-place__guide" type="button" onClick={onGuide}>Step in <span aria-hidden="true">⌁</span></button>
 </section>;
}
