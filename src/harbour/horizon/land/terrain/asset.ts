import type { TerrainField, WaterCut } from '../interfaces';
import { conserveWaterFootprint, GEOGRAPHY_REVISION } from './index';
...
/** No averaging: all LODs retain exact lattice samples of the same island. */
...
export function encodeTerrainAsset(field: TerrainField, options: { waters?: WaterCut[] } = {}): ArrayBuffer {
  ...
  levels.forEach((f, i) => {
    ...
    // Flag documents downward-conservative water samples on the two coarser grids.
    view.setUint32(entry + 24, i > 0 && options.waters ? 1 : 0, true);
