...
import type {ModeController,ModeExit,ModeId,MoverBody} from './shared/mode.ts';
import {offersAt,type CarriedProvider,type CarriedThreshold,type ThresholdOffer} from './shared/threshold.ts';

export type ModeFactory=(offer:ThresholdOffer,threshold:Threshold)=>ModeController;
/** The outcome of an accepted offer: a new controller to attach, or (for `X→feet`) where the body now stands. */
export interface AcceptResult{controller:ModeController|null;exit:ModeExit|null;threshold:Threshold}
...
export function createModeRegistry(initial:readonly Threshold[]=[]):ModeRegistry{
  let thresholds:readonly Threshold[]=initial,current:ModeController|null=null;
  ...
  function offers(body:MoverBody,reach=3.6):ThresholdOffer[]{
    const from=active();
    return offersAt(thresholds,body,reach,carriedNow()).filter(o=>o.from===from&&available(o.to));
  }
  return{
    ...
    accept(offer,body,reach=3.6){
      ...
      const threshold=thresholds.find(t=>t.id===valid.thresholdId)!,placed:Threshold=threshold.carried?{...threshold,at:[valid.at[0],valid.at[1]],height:valid.height}:threshold;
