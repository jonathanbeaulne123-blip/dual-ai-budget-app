...
export function collectCentrelines(cuts: LandCuts, lines: readonly Line[] = []): Centreline[] {
  ...
  for (const line of lines) if (line.points.length > 1 && !out.some(c => c.sourceId === line.id || c.id === line.id)) out.push({ id: line.id, sourceId: line.id, points: line.points, clearHeight: line.mode === 'gondola' ? 8 : line.mode === 'ferry' || line.mode === 'row' ? 4 : 1.25, kind: line.mode, structureIds: [] });
