...
export function buildCrossings(cuts: LandCuts, lines: readonly Line[] = []): { crossings: Crossing[]; proofs: CrossingProof[]; rawIntersections: Intersection[] } {
  ...
  const proofs = intersections.map(hit => {
    ...
    const confluence = aquatic(a) && aquatic(b), ferryTransfer = hit.a === 'FERRY' && hit.b.startsWith('ferry.') || hit.b === 'FERRY' && hit.a.startsWith('ferry.');
    const mover = [a, b].find(line => line.kind === 'cable' || line.kind === 'rail'), footLine = mover === a ? b : a;
    const endpointTransfer = !!mover && ['walk', 'road', 'trail', 'boardwalk', 'cave'].includes(footLine.kind) && [mover.points[0]!, mover.points.at(-1)!].some(p => Math.hypot(p[0] - hit.at[0], p[2] - hit.at[1]) < 1);
    ...
    const built = confluence ? clearancePass : transfer ? !!pad && !!marker && cuts.solids.some(solid => (solid.role === 'deck' || solid.role === 'floor') && solidNear(solid, hit.at, .5) && solidVerticalRangeAt(solid, hit.at[0], hit.at[1]) !== null) && clearancePass : resolution === 'threshold' ? !!pad && !!marker && kerbGap && clearancePass : physical.length > 0 && clearancePass;
    const proof: CrossingProof = { ...hit, id: `${base}${n}`, resolution, kind: confluence ? 'waterConfluence' : transfer ? 'modeTransfer' : 'crossing', registered: !!match, proposed: !match, ...(match ? { manifestIndex: match.index } : {}), separation, clearHeight, requiredClearance: resolution === 'threshold' ? transfer ? 1.25 : .5 : required, clearancePass, structureIds: relevant.map(solid => solid.id), built, ...(pad ? { padId: pad.id } : {}), ...(marker ? { markerId: marker.id } : {}), ...(resolution === 'threshold' && !confluence ? { kerbGap } : {}), ...(confluence ? { waterBodyIds: [hit.sourceA ?? hit.a, hit.sourceB ?? hit.b], note: 'Continuous water confluence; one physical waterway, no pedestrian pad or furniture in the basin.' } : transfer ? { moverPending: true, note: 'Graded boarding threshold land interface; vehicle geometry and boarding animation require the movers pass.' } : match?.row.note ? { note: match.row.note } : {}) };
    return proof;
  });
