...
type MountainPresentationHandle = PlaceHandle & {
  ...
  setTransit?:(at:Point3|null,kind?:TransportKind,frame?:{yaw:number;pitch:number})=>void;
...
export function mountHarbourWorld(host: HTMLElement, theme: ThemeId, tier: RenderTier, callbacks: HarbourCallbacks): HarbourRuntime {
  ...
  function stream(): boolean {
    ...
    if(detailCourt?.streamDetails?.(focus[0],focus[1],run?.id==='mountain-descent'&&!run.finished)){
  ...
  function raiseBody(): void {
    ...
    const eye=camera.position.clone(),orientation=camera.quaternion.clone(),fov=camera.fov;
    follow = createFollowCamera({ camera, composition, reduced: reducedMotion(), fov: fovFor(composition), groundHeightAt: (x, z) => placeId==='court'?groundHeightAt(x,z):bodyGround(x, z), blocked:(x,y,z)=>placeId==='court'&&(worldCollisionAt(x,y,z,.12)||mountainFoliageAt(x,y,z,tier)||insideVillageBuilding(x,z,.12,y)) });
 ...
 x, y, w, h, visible };
  ...
  function loop(now: number): void {
    ...
    if (walker && follow) {
      ...
      if(mountainTrip){
        ...
        if(placeId==='court'&&!walker.skate.active()){const at=walker.state(),here=platformOffer(at.x,at.y,at.z);if(here)offerRide(here);else if(rideOffer?.reason==='platform')offerRide(null);}
      }
  ...
  const api: HarbourRuntime = {
    ...
    nearestStation(kind){const at=walker?.state();return at?nearestStation(kind,at.x,at.z,at.y):0;},
    mountainTravel(at,trip){
      ...
      if(!boarding){walker.place(p[0],p[2],0,p[1]);bringCat();focus=[p[0],p[2]];setFollowing(false);setFollowing(true);follow?.snap();}
    ...
    body() {
      ...
      return {
        ...
        runLock(on) { const locked=one.runLock(on); dirty = true; schedule(); return locked; },
