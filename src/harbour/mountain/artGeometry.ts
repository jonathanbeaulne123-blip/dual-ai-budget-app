...
export type DistrictFixture={id:string;district:string;kind:'rock'|'bed'|'logs'|'crate';at:Point3;size:number;height:number;yaw:number;solid:ArtSolid};
...
export const DISTRICT_FIXTURES:readonly DistrictFixture[]=DISTRICTS.flatMap(d=>{
  ...
  for(let i=0;i<n;i++){
    const kind=kinds[i%kinds.length]!,size=kind==='rock'?.8+random()*.9:1,half=kind==='bed'?1.6:kind==='logs'?1.1:kind==='crate'?.5:size*1.1;
    const [x,z]=findSpot(d.at[0],d.at[2],i*2.399+random()*.6,d.radius*.7+random()*9,half,{level:.7});
    if(!districtArtClear(d,x,z,half))continue;
    const g=footGround(x,z,0,half,half),y=g.min;if(g.max-g.min>.9)continue;
    const yaw=random()*Math.PI,height=kind==='rock'?size*.9:kind==='bed'?.45:kind==='logs'?.8:.7;
    const hx=kind==='bed'?1.5:kind==='logs'?1.05:kind==='crate'?.45:size,hz=kind==='bed'?.75:kind==='logs'?.5:kind==='crate'?.45:size*.8;
    // An axis-aligned solid round the turned footprint.
    const ex=Math.abs(Math.cos(yaw))*hx+Math.abs(Math.sin(yaw))*hz,ez=Math.abs(Math.sin(yaw))*hx+Math.abs(Math.cos(yaw))*hz;
    const id=`district-art:${d.id}:${i}`;
    out.push({id,district:d.id,kind,at:[x,y,z],size,height,yaw,solid:{id,min:[x-ex,y,z-ez],max:[x+ex,g.max+height,z+ez]}});
  }return out;
});
...
export const SUMMIT_ART_SOLIDS:readonly ArtSolid[]=[
  ...
  ...Array.from({length:8},(_,k)=>{const a=k/8*Math.PI*2+Math.PI/16;return box(`summit-art:column:${k}`,ox+Math.cos(a)*(orad-.2),oz+Math.sin(a)*(orad-.2),.16,.16,oy.min,oy.max+.15+3.6);}),
