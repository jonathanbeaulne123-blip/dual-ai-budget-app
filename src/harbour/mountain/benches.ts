...
export const EXTRA_BENCH_LINES:readonly BenchLine[]=[
  ...PATH_EDGES.filter(e=>e.kind==='path'||e.kind==='stair').map(e=>({id:e.id,points:e.points,halfWidth:e.halfWidth,shoulder:.4,slope:1,inset:.04})),
  ...trackRuns().map((points,i)=>({id:`funicular-formation:${i}`,points,halfWidth:1.8,shoulder:.3,slope:1,inset:.35})),
];
export const EXTRA_BENCH_PADS:readonly BenchPad[]=[
  ...(Object.keys(BUILDING_SITES) as MountainBuilding[]).map(id=>{const spot=BUILDING_SITES[id],level=DISTRICTS.find(d=>d.id===DISTRICT_OF[id])!.at[1];
    return {id:`building:${id}`,at:[spot[0],level,spot[1]] as Point3,half:[BUILDING_FORMS[id].half[0]+1,BUILDING_FORMS[id].half[1]+1] as const,yaw:buildingYaw(spot),inset:.05,slope:1};}),
  ...DOOR_APRONS.map(a=>({id:`apron:${a.site}`,at:a.apron.at,half:a.apron.half,yaw:a.apron.yaw,inset:.03,slope:1})),
  ...[...FUNICULAR_LINE.stations,...GONDOLA_LINE.stations].map(s=>({id:`platform:${s.id}`,at:s.platform.at,half:[s.platform.half[0]+.4,s.platform.half[1]+.4] as const,yaw:s.platform.yaw,inset:.05,slope:1})),
