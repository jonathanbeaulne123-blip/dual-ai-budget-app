...
export function setStreamQuiet(on:boolean){quietStream=on;}
...
export function fadeIn(group:THREE.Object3D){
  ...
  if(quietStream||reduced||typeof window==='undefined')return {cancel(){}};
  ...
  const restore=()=>{if(done)return;done=true;for(const s of states){s.m.opacity=s.opacity;s.m.transparent=s.transparent;s.m.depthWrite=s.depthWrite;s.m.needsUpdate=true;}hooks.forEach(o=>{o.onBeforeRender=()=>{};});};
