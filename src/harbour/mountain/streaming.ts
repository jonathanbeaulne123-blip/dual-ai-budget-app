...
export function createDetailStream<T extends DetailResource>(sites:readonly DetailSite[],build:(site:DetailSite)=>T){
  ...
  return {
    live,
    update(x:number,z:number,pinned=false,now=performance.now(),allowBuild=true):boolean{
      ...
      for(const site of sites){
        ...
        if(!resident&&!built&&allowBuild&&(pinned||distance<=site.radius)){live.set(site.id,build(site));changed=true;built=true;}
        else if(!resident&&(pinned||distance<=site.radius))pending=true;
...
/** A calm, still view settles detail; reduced motion uses its shorter authored fade. */
const activeFades=new Set<()=>void>();
let quietStream=false;
const reducedMotion=()=>typeof window!=='undefined'&&window.matchMedia?.('(prefers-reduced-motion: reduce)').matches===true||typeof document!=='undefined'&&document.documentElement.dataset.motion==='reduced';
export function setStreamQuiet(on:boolean){quietStream=on;if(on&&!reducedMotion())for(const restore of [...activeFades])restore();}
export const DETAIL_FADE_MS=450;
export const REDUCED_DETAIL_FADE_MS=300;
/**
...
export function fadeIn(group:THREE.Object3D){
  ...
  if((quietStream&&!reducedMotion())||typeof window==='undefined')return {cancel(){}};
  ...
  group.traverse(o=>{if(!(o as THREE.Mesh).isMesh&&!(o as THREE.LineSegments).isLineSegments)return;hooks.push(o);o.onBeforeRender=()=>{
    if(done)return;const t=now();start??=t;const u=Math.min(1,(t-start)/(reducedMotion()?REDUCED_DETAIL_FADE_MS:DETAIL_FADE_MS));
