...
import './mountain.css';
export type MountainAction={kind:'go';at:Point3}|{kind:'ride';transport:TransportKind;from:number;to:number}|{kind:'view';view:'dam'|'world'}|{kind:'race'}|{kind:'skip'}|{kind:'calm';on:boolean}|{kind:'sound';on:boolean};
export function MountainPanel({reading,statusLine,onAction,onOpen,flat=false,inspect,conditionWords,riding=false,soundOn=false,calmOn=false}:{soundOn?:boolean;calmOn?:boolean;riding?:boolean;conditionWords?:string;inspect?:{section:'map'|'water'|'travel';seq:number};reading?:BasinReading;statusLine:string|null;onAction:(action:MountainAction)=>void;onOpen:(target:string)=>void;flat?:boolean}){
  const [open,setOpen]=useState(false),[tab,setTab]=useState<'map'|'water'|'travel'>('map'),[transport,setTransport]=useState<TransportKind>('funicular'),[from,setFrom]=useState(0),[to,setTo]=useState(1);
