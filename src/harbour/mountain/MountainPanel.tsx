...
import {TRANSPORT_STOPS,MONORAIL_STOPS,districtAt,type Point3,type TransportKind} from './definition.ts';
 ...
 */
export function MountainPanel({open:controlledOpen,onOpenChange,life=initialMountainInteractionState(),recoveryWords,onAction,onOpen,flat=false,inspect,conditionWords,riding=false,monorail=null,partnerName=null,soundOn=false,calmOn=false,nearestStation}:{here?:Point3;nearestStation?:(kind:TransportKind)=>number;open?:boolean;onOpenChange?:(open:boolean)=>void;hideTrigger?:boolean;life?:MountainInteractionState;recoveryWords?:string;soundOn?:boolean;calmOn?:boolean;riding?:boolean;monorail?:MonorailState|null;partnerName?:string|null;conditionWords?:string;inspect?:{section:'map'|'water'|'travel';seq:number;station?:number};reading?:BasinReading;statusLine:string|null;onAction:(action:MountainAction)=>void;onOpen:(target:string)=>void;flat?:boolean}){

  const [localOpen,setLocalOpen]=useState(false),[tab,setTab]=useState<'travel'|'tour'|'life'>('travel'),[transport,setTransport]=useState<TransportKind>('funicular'),[from,setFrom]=useState(0),[to,setTo]=useState(1);
