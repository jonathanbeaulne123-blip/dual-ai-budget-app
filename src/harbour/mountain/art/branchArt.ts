...
function deck(b:CardBuilder,pal:MountainArtPalette,F:Frame[],hw:number,metal:boolean,th=.32){
  const top=metal?shade(pal.iron,1.9):pal.plank,skin=metal?pal.iron:pal.timber;
  ...
  for(let i=0;i<F.length;i+=3){const f=F[i]!;for(const s of [-1,1]){const x=f.p[0]+f.side[0]*s*(hw-.25),z=f.p[2]+f.side[2]*s*(hw-.25),g=groundHeightAt(x,z);if(f.p[1]-th-g<.4)continue;
  ...
  for(const br of SKILL_BRANCHES){
    ...
    for(const seg of br.segments){
      ...
      if(seg.kind==='rail'){
        ...
        const walk=F.map(f=>({p:[f.p[0],f.p[1]-.75,f.p[2]] as V3,side:f.side,up:f.up}));
        deck(b,pal,walk,.55,true,.18);
        for(let i=0;i<F.length;i+=2){const f=F[i]!;b.post(f.p[0],f.p[2],f.p[1]-.75,f.p[1]+.02,.05,pal.iron,5,'steel');}
        if(br.id==='dam-promenade'){const [cx,,cz]=DAM_PARTS.centre;
          for(let i=0;i<F.length;i+=4){const f=F[i]!,dx=cx-f.p[0],dz=cz-f.p[2],l=Math.hypot(dx,dz)||1,d=l-DAM_PARTS.radius;if(d<.5||d>12)continue;
            const back:V3=[f.p[0]+dx/l*(d-.1),f.p[1]-.9,f.p[2]+dz/l*(d-.1)];b.beam([f.p[0],f.p[1]-.9,f.p[2]],back,.14,.2,pal.iron,null,'steel');b.beam([f.p[0],f.p[1]-.9,f.p[2]],[back[0],back[1]+2.2,back[2]],.08,.08,pal.iron,null,'steel');}}
