...
export type WorldAmbience={update(x:number,y:number,z:number,speed:number,wood:boolean,walking:boolean,quiet:boolean):void;pause():void;dispose():void};
...
export function createWorldAmbience():WorldAmbience|null{
  ...
  let dead=false,last:{x:number;z:number}|null=null,stride=0,lastStep=0;
  ...
  const pause=()=>{if(!dead){ease(master.gain,0);last=null;stride=0;}};
  ...
  return {
    ...
    dispose(){if(dead)return;dead=true;document.removeEventListener('visibilitychange',visibility);for(const b of [wind,water,leaves]){b.source.stop();b.source.disconnect();b.filter.disconnect();b.gain.disconnect();}master.disconnect();void ctx.close().catch(()=>{});},
  };
}
