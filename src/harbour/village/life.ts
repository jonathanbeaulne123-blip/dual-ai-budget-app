...
export function buildVillageLife(dressing: PlaceDressing, tier: RenderTier): VillageLife {
  ...
  const add = (geometry: THREE.BufferGeometry, mat: THREE.Material, name: string, at: readonly [number, number, number]) => {
    owned.add(geometry); const mesh = new THREE.Mesh(geometry, mat); mesh.name = name; mesh.position.set(...at); mesh.castShadow = tier === "full"; group.add(mesh); return mesh;
  };
  const wood = material(dressing.timber), brass = material(dressing.metal, { metalness: .62, roughness: .34 }), sail = material("#ece0bc"), catMat = material(dressing.theme === "newfoundland" ? "#667b72" : "#8a654d");
  ...
  // Hercules is an original compact roaming silhouette; it never implies a real activity or pet state.
