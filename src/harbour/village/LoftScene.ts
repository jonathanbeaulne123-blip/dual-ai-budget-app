...
export const LOFT_LAYOUT = Object.freeze({ halfX: 4.5, halfZ: 3.5, shelfY: [0.8, 1.7, 2.4] as const, maxBanks: 18 });
...
export function createLoft(scene: THREE.Scene, dressing: { timber: string; metal: string; stone: string }, reading: PlaceReading | null, quality: RenderTier): PlaceHandle {
  ...
  const wood = material(dressing.timber), brass = material(dressing.metal, 0.45), stone = material(dressing.stone, 0.92);
  const add = (size: readonly [number, number, number], at: readonly [number, number, number], name: string, source = wood): THREE.Mesh => {
    const geometry = new THREE.BoxGeometry(...size), mesh = new THREE.Mesh(geometry, source); geometry.translate(0, 0, 0); owned.push(geometry); mesh.name = name; mesh.position.set(...at); mesh.castShadow = quality === "full"; mesh.receiveShadow = true; group.add(mesh); return mesh;
  };
  ...
  add([2.35, 1.15, .08], [0, 1.75, -3.32], "loft-back-window", brass);
  add([2.62, .16, .62], [0, .67, -2.82], "loft-window-seat");
  for (let index = 0; index < LOFT_LAYOUT.shelfY.length; index++) {
    const y = LOFT_LAYOUT.shelfY[index]!;
    add([7.25, .12, .7], [0, y, -2.55], `loft-bank-shelf-${index}`);
    add([.12, .86, .58], [-3.55, y - .38, -2.55], `loft-shelf-post-left-${index}`);
    add([.12, .86, .58], [3.55, y - .38, -2.55], `loft-shelf-post-right-${index}`);
  }
  ...
  let slots: LoftSlot[] = [], current = safeReading(reading), disposed = false;
  ...
  const rebuild = (next: TowerReading) => {
    ...
    const banks = next.shelves.flatMap(shelf => shelf.banks).slice(0, LOFT_LAYOUT.maxBanks);
    const finish = plateFinish("current");
    for (let index = 0; index < banks.length; index++) {
      const bank = banks[index]!, row = Math.floor(index / 6), column = index % 6;
      const y = LOFT_LAYOUT.shelfY[row]!, x = -2.75 + column * 1.1, z = -2.52;
  ...
  return {
    group,
    update(next) { rebuild(safeReading(next)); },
    ...
    dispose() { if (disposed) return; disposed = true; clearSlots(); scene.remove(group); for (const item of owned) item.dispose(); group.clear(); },
  };
}
