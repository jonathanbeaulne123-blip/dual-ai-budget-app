 ...
 */
export function VillageHUD({flat=false,onGuide,guideOpen,appearanceRequest,place,travelling,onVisit,onArrange,onView,onWander,onJourney,avatar,avatarStatus,onAvatar,presence,fab,onQuickSheet}:{flat?:boolean;onGuide?:()=>void;guideOpen?:boolean;appearanceRequest?:number;place:HarbourPlaceId;travelling:HarbourPlaceId|null;onVisit:(id:HarbourPlaceId,instant?:boolean)=>void;onArrange?:()=>void;onView:()=>void;onWander?:(id:HarbourWanderId)=>void;onJourney?:()=>void;avatar?:PlayableAvatar|null;avatarStatus?:'idle'|'loading'|'ready'|'error';onAvatar?:(avatar:PlayableAvatar)=>void;presence?:ReactNode;fab?:CompassFab;onQuickSheet?:()=>void}){
 ...
 return <div className="village-hud" onPointerDown={e=>e.stopPropagation()}>
  {!flat&&!map&&<div className="village-presence-outside">{presence}</div>}
  {!flat&&<header className="village-address"><span className="village-address__seal">h</span><div><small>LITTLE HARBOUR</small><h1>{HARBOUR_PLACE_NAMES[place]}</h1><p>{travelling?`On our way to ${HARBOUR_PLACE_NAMES[travelling]}`:place==='court'?'A little world to come home to.':'Settle in. There’s room to make it yours.'}</p></div></header>}
  <nav className="village-tools harbour-bar" data-harbour-bar="island" aria-label="Harbour bar"><EditionFlip className="village-tools__flip"/><button type="button" id="world-guide-trigger" aria-label={onGuide?"Mountain and town guide":"Village map"} aria-expanded={onGuide?guideOpen:map} onClick={()=>onGuide?onGuide():setMap(!map)}>⌖ <span>{onGuide?"Mountain & town":"Village map"}</span></button><label className="village-quick" title="Quick travel"><span>Go straight to</span><select aria-label="Quick travel" value="" onChange={e=>{if(e.target.value)onVisit(e.target.value as HarbourPlaceId,true);setMap(false);}}><option value="">Quick travel…</option><option value="court">Village square</option>{STOPS.map(s=><option key={s.id} value={s.id}>{s.title}</option>)}<option value="tower">Home · Loft</option><option value="cellar">Home · Cellar</option><option value="atlas">Home · Atlas nook</option></select></label>{!flat&&<button type="button" onClick={onView} aria-label="Look around this place">↗ <span>Look around</span></button>}{onJourney&&<button type="button" onClick={onJourney} aria-label="Journey map">◇ <span>Journey</span></button>}{onArrange&&<button type="button" aria-label="Arrange room" onClick={onArrange}>✿ <span>Arrange room</span></button>}{fab&&<BarFab fab={fab}/>}{onQuickSheet&&<button type="button" className="village-tools__all" aria-label="All tools" aria-describedby={pawprint} title="All tools (Space)" onClick={onQuickSheet}>☰ <span>All tools</span>{paw}</button>}</nav>
