...
export function buildBankDetails(dressing: PlaceDressing, tier: RenderTier): BankDetails {
  ...
  for (let x = 0; x < 7; x++) for (let z = 0; z < 7; z++) { point.set(-1.86 + x * .62, .075, -2.8 + z * .62); const target = (x + z) % 2 ? darkTiles : lightTiles, index = (x + z) % 2 ? dark++ : light++; target.setMatrixAt(index, matrix.compose(point, rotation, scale)); }
  ...
  mesh(new THREE.BoxGeometry(1.48, .025, 4.0), teal, "bank-teal-runner", [0, .102, -.94]);
  ...
  mesh(new THREE.BoxGeometry(.035, .2, .035), timber, "bank-clock-hand-minute", [-2.65, 2.25, -3.135], [0, 0, -.35]); mesh(new THREE.BoxGeometry(.035, .13, .035), timber, "bank-clock-hand-hour", [-2.65, 2.25, -3.13], [0, 0, .95]);
