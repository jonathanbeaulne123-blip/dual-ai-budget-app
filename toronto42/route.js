...
function trainSuggestion(){
 if(!rail)return '';
 const first=plan.stops.find(x=>x.arrival);if(!first)return '';
 const walk=Math.max(5,(b.stops[0]&&b.stops[0].walk)||0)||8;
 const needArrive=min(first.arrival)-(b.id==='union-core'?8:b.id==='port-credit'?8:b.id==='oakville'?15:8);
 ...
 if(!best)return '';
 return '<div class="big">Leave Clarkson '+clock(best[0])+' → '+e(st.short||b.station)+' '+clock(best[1])+'</div><p class="note" style="margin:4px 0 0">Off-peak trains run every 30 minutes ('+(rail.dir==='east'?':08 and :38':':23 and :53')+' from Clarkson, '+rail.minutes+' min). Weekday afternoon expresses may skip smaller stations. Confirm the exact departure in the GO planner for '+e(s.date)+'.</p>';
}
function render(){
 ...
 const whole=wp.length?'https://www.google.com/maps/dir/?api=1&origin='+encodeURIComponent(b.station)+'&destination='+encodeURIComponent(wp[wp.length-1])+(wp.length>1?'&waypoints='+encodeURIComponent(wp.slice(0,-1).join('|')):'')+'&travelmode=walking':'';
 ...
 let prev=b.station,n=0;
 $('#timeline').innerHTML=plan.stops.map(x=>{const r=get(x.id),d=disc(x.id),i=intel(x.id),skip=!!x.excluded,here=addr(x.id);if(!skip)n++;
