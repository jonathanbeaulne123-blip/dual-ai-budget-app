...
try {
 for(const theme of (process.env.PROOF_THEME ? [process.env.PROOF_THEME] : ['classic','taylor','newfoundland'])) {
  ...
  await page.getByRole('button',{name:'Add money',exact:true}).waitFor({timeout:90000});
   ...
   if(Math.abs((before.y-after.y)-80)>2)throw new Error('Easy read does not move with chat');
   ...
   results.push({theme,width,scrollDelta:before.y-after.y,errors:[...errors]});
  await context.close();
  }
 }
 await writeFile(`artifacts/category-split/hercules-easy-read-results.json`,JSON.stringify(results,null,2));
