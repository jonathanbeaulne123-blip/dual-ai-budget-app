...
const banner='<aside id="whole-house-review-label" style="position:relative;z-index:100000;background:#283f36;color:#fff6d7;padding:7px 14px;font:12px system-ui;text-align:center">Fictional local house · real app commands · no hosted services <a style="color:inherit;margin-left:15px" href="/__review?member=MEM-001">Alex</a> <a style="color:inherit;margin-left:10px" href="/__review?member=MEM-002">Sam</a> <a style="color:inherit;margin-left:15px" href="/__review?seed=demo">Demo habitat</a> <a style="color:inherit;margin-left:15px" href="/__review?seed=mountain&story=growing&run=first">Mountain rehearsal</a></aside>';
const server=await createServer({configFile:false,envFile:false,root,cacheDir:resolve(runtime,'vite-cache'),logLevel:'warn',esbuild:{jsx:'automatic'},optimizeDeps:{exclude:['@electric-sql/pglite']},worker:{format:'es'},
  ...
  plugins:[{name:'fictional-whole-house-review',transformIndexHtml(html){return html.replace('<body>','<body>'+banner);},configureServer(vite){vite.middlewares.use((req,res,next)=>{if(req.url?.split('?')[0]==='/__skate-lab'){res.setHeader('Content-Type','text/html');res.end('<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Skate Lab</title><style>body{margin:0;background:#111}</style></head><body><div id="skate-lab"></div><script type="module" src="/test/browser/skateLab.ts"></script></body></html>');return;}if(req.url?.split('?')[0]!=='/__review')return next();res.setHeader('Content-Type','text/html');res.end(`<!doctype html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Hearth · fictional whole house review</title></head><body>${banner}<div id="root"><p>Opening the fictional local house…</p></div><script type="module" src="/test/browser/whole-house-review.tsx"></script></body></html>`);});}}]
});
await server.listen();
