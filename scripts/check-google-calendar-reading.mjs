...
const out=process.env.HEARTH_ARTIFACTS_DIR||'/tmp/hearth-google-calendar-reading';
...
try {
 ...
 for(const theme of ['classic','taylor','newfoundland']){
  ...
  for(const scope of ['household','personal']){
   ...
   const violations=(await new AxeBuilder({page}).include('[aria-label="Google calendar integration"]').withTags(['wcag2a','wcag2aa','wcag21aa']).analyze()).violations;
