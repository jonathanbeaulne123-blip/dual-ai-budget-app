...
GRIND_JS = """(popAt) => new Promise(resolve => {
  ...
  const finish = () => { for (const k of ['w', 'g', 'ArrowDown', 'ArrowUp']) key('keyup', k);
    resolve({frames, samples, grindFrames, phases: [...new Set(seen)], last: s}); };
  key('keydown', 'w');
  const tick = () => {
    frames++;
    ...
    if (raw && raw !== last) { last = raw; samples++; since++; s = JSON.parse(raw); seen.push(s.phase);
      ...
      else if (step === 4 && s.phase === 'grind') { grindFrames++; return finish(); }
    }
    if (frames > 600 || samples > 90 || performance.now() - started > 90000) return finish();
