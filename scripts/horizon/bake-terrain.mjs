...
try{
 ...
 const {bake}=await import(pathToFileURL(output).href),{buffer,world,horizonCards}=bake();
 const paths={terrain:resolve(root,'public/horizon/terrain/horizon-geo-1.bin'),world:resolve(root,'public/horizon/world/horizon-geo-1.json'),cards:resolve(root,'public/horizon/world/horizon-cards.json')};
 const artifacts={terrain:Buffer.from(buffer),world:Buffer.from(JSON.stringify(world)),cards:Buffer.from(JSON.stringify(horizonCards))};
