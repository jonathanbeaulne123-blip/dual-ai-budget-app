...
import {serializeHorizonJson,assertHorizonArtifact} from './artifacts.mjs';
...
try{
 ...
 for(const key of Object.keys(paths)){if(process.argv.includes('--check')){assertHorizonArtifact(key,await readFile(paths[key]),artifacts[key]);}else{await mkdir(resolve(paths[key],'..'),{recursive:true});await writeFile(paths[key],artifacts[key]);}}
