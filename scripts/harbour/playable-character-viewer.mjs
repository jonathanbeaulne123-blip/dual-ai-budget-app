...
const camera = new THREE.PerspectiveCamera(35, 1, .01, 20); camera.position.set(1.3, .85, 1.8); camera.lookAt(0, .3, 0);
...
function limbs() {
  const g = new THREE.Group(), coat = new THREE.MeshStandardMaterial({ color: 0x496b79, roughness: .9 }), trouser = new THREE.MeshStandardMaterial({ color: 0x3e4855, roughness: 1 }), shoe = new THREE.MeshStandardMaterial({ color: 0x282529, roughness: 1 });
  for (const side of [-1, 1]) { const hip = new THREE.Group(); hip.position.set(side * .048, .235, 0); const leg = new THREE.Mesh(new THREE.CapsuleGeometry(.042, .145, 4, 8), trouser); leg.position.y = -.1145; const foot = new THREE.Mesh(new THREE.BoxGeometry(.072, .034, .108), shoe); foot.position.set(0, -.212, .018); hip.add(leg, foot); g.add(hip); const shoulder = new THREE.Group(); shoulder.position.set(side * .09, .402, 0); const arm = new THREE.Mesh(new THREE.CapsuleGeometry(.031, .118, 4, 8), coat); arm.position.y = -.09; shoulder.add(arm); g.add(shoulder); }
  return g;
}
async function select(avatar) { figure?.removeFromParent(); const row = manifest.characters.find((item) => item.avatar === avatar); const gltf = await loader.loadAsync(`.${row.url}`); const g = new THREE.Group(), shell = gltf.scene; shell.scale.setScalar(.58 / row.sourceFullBounds.max[1]); shell.position.y = -.58 * row.sourceFullBounds.min[1] / row.sourceFullBounds.max[1]; g.add(limbs(), shell); scene.add(g); figure = g; }
