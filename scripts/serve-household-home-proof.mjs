...
import{saveKittyNestDesign}from'/src/core/kittyNestDesigns.ts';import{guardQueenDesignSave}from'/src/queen/world/queenAuthoring.ts';import{queenCharmKindsEarned,QUEEN_CHARM_KINDS}from'/src/core/queenCharms.ts';import{QUEEN_CHARM_KEYBOARD_SEATS}from'/src/queen/world/queenCharmSurface.ts';import{STUDIO_PALETTE}from'/src/kitty/studio/palette.ts';
const q=new URLSearchParams(location.search),theme=q.get('theme')||'classic',surface=q.get('surface')||'home',quiet=q.get('quiet')==='1',composition=q.get('composition')||'panels',state=q.get('state')||'needs-us',chrome=q.get('chrome')==='1',charms=q.get('charms');
...
function seeded(){let h=planLifeFixture('household');h=earnedActs(h);h=openChapter(h,{memberId:'MEM-001',foundationId:'make-rent-boring',at:'2026-09-01T12:00:00.000Z'}).household;const chapter=openChapterFor(h);const ritual=h.rituals.find(row=>row.chapterId===chapter.id);h=recordRitualHeld(h,{memberId:'MEM-001',ritualId:ritual.id,onDate:'2026-09-04'}).household;h=offerMove(h,{memberId:'MEM-002',chapterId:chapter.id,text:'Confirm which payday the pre-rent check belongs to',needsAcknowledgment:true}).household;return h;}
...
function dressed(h){if(!charms||charms==='none')return h;const earned=queenCharmKindsEarned(h);const kinds=QUEEN_CHARM_KINDS.filter(kind=>earned.has(kind));const seats=QUEEN_CHARM_KEYBOARD_SEATS;const gallery=charms==='gallery';const count=charms==='few'?3:gallery?kinds.length:seats.length;const rows=[];for(let i=0;i<count;i++){const seat=seats[i];rows.push({id:'fiction-'+i,kind:kinds[i%kinds.length],part:seat.part,u:seat.u,v:seat.v,spin:gallery?0:(i*37)%120-60,tilt:gallery?0:(i%3-1)*8,scale:gallery?1.5:[1,1.2,0.8,1.5,0.7][i%5],color:STUDIO_PALETTE[(i*7+5)%STUDIO_PALETTE.length].hex,by:i%2?'MEM-002':'MEM-001'});}
  const design=h.kittyNestDesigns?.find(row=>row.bankKey==='king'&&row.visibility==='household');
  return saveKittyNestDesign(h,guardQueenDesignSave({memberId:'MEM-001',view:'household',bankKey:'king',expectedRevision:design?.revision??0,name:design?.name??'Our Queen',glaze:design?.glaze??'cream',category:null,studio:{version:1,draft:{id:'queen',createdAt:'2026-09-01T12:00:00.000Z',firedAt:null,sculpt:{body:'round',profile:[1,1,1,1],head:'round',ears:'none',eyes:'closed',mouth:'serene',whiskers:'none',tail:'none',nose:'tiny'},paint:{base:design?.glaze??'cream',parts:{},strokes:[],stamps:[]},charms:rows},fired:[]}},{earned})).household;}
