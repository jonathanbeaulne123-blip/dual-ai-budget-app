...
try {
 ...
 if(errors.length||records.some(r=>r.violations.length||r.metrics.some(m=>m.scroll>m.width)||r.special.zoom.scroll>r.special.zoom.width||r.special.galleryCount>1||r.special.galleryAfterOffice===false||r.special.charm&&!r.special.charm.hit))process.exitCode=1;
 await browser.close();
}
