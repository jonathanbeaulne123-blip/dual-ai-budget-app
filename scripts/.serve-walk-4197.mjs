...
const server=await createServer({configFile:false,envFile:false,root,cacheDir:resolve(runtime,'vite-cache'),logLevel:'warn',esbuild:{jsx:'automatic'},optimizeDeps:{exclude:['@electric-sql/pglite']},worker:{format:'es'},
  ...
  server:{host:'127.0.0.1',port:4197,strictPort:true,fs:{allow:[root,await realpath(resolve(root,'node_modules'))]},proxy:{'/ledger-sync':{target:authority.base,ws:true},'/api/hearthside-vault':{target:authority.base}}},
...
await server.listen();
await writeFile(resolve(runtime,'server.json'),JSON.stringify({url:'http://127.0.0.1:4197/__review',householdId,localAuthority:authority.base,startedAt:new Date().toISOString(),hosted:false},null,2));
console.log('Hearth actual-app review: http://127.0.0.1:4197/__review');
