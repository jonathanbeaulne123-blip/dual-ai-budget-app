...
Use discover_tools with the requested scope to learn available Hearth actions. Before preparing a Hearth action, use hearth_action_options with that action id and known string values to resolve current fields and choices. Fetch more choice pages when needed. Re-read after changing job, role or other dependent choices; never invent record identifiers or treat missing input as zero. These reads provide options, not confirmation authority.
