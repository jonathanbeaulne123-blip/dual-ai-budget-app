...
import { WORKSPACE_ACTION_QUERY_NAMES, isWorkspaceActionQuery } from '../../src/workspace/actionQueries.ts';
import { DEFAULT_RUN_BUDGET } from '../../src/workspace/runtime.ts';
export const WORKSPACE_GRANTED_READ_NAMES = [...HERCULES_READ_TOOL_NAMES, ...WORKSPACE_ACTION_QUERY_NAMES];
export function requireWorkspaceReadGrant(permittedReads: readonly string[], name: string) {
  if (!permittedReads.includes(name)) throw new Error(isWorkspaceActionQuery(name) ? 'GRANT_ACTION_OPTIONS_REFRESH_REQUIRED' : 'READ_NOT_GRANTED');
}
export type PrivateRunGrant = { token: string; id: string; runId: string; projectId: string; expiresAt: string; localScope?: Scope; localPermittedReads?: string[] };
...
export async function issueGrant(env: AuthEnv, scope: Scope, authToken: string, grant: PrivateRunGrant): Promise<PrivateRunGrant> {
  if (scope.subject.startsWith('local:') && env.LEDGER_SYNC_LOCAL_AUTH === 'true') return { ...grant, id: grant.runId, localScope: scope, localPermittedReads: [...WORKSPACE_GRANTED_READ_NAMES] };
  const value = await supabase(env, '/rest/v1/rpc/hercules_issue_run_grant', authToken, {
    ...
    p_token_sha256: await hashText(grant.token), p_permitted_reads: WORKSPACE_GRANTED_READ_NAMES,
...
export async function leaseGrant(env: AuthEnv, grant: PrivateRunGrant): Promise<WorkspaceLease> {
  ...
  if (grant.localScope && env.LEDGER_SYNC_LOCAL_AUTH === 'true' && grant.localScope.environment === 'development') return { ...grant.localScope, expires: Date.now() + 60_000, aclEpoch: started, permittedReads: [...(grant.localPermittedReads ?? HERCULES_READ_TOOL_NAMES)], budget: DEFAULT_RUN_BUDGET };
  ...
  if (!Number.isFinite(expiry) || expiry <= started || !/^HH-[A-Za-z0-9_-]+$/.test(String(result.householdId)) || !/^MEM-[A-Za-z0-9_-]+$/.test(String(result.memberId))
    || typeof result.subject !== 'string' || !result.subject || !Array.isArray(result.permittedReads) || result.permittedReads.some(v => typeof v !== 'string' || !(WORKSPACE_GRANTED_READ_NAMES as readonly string[]).includes(v))
