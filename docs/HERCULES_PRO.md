...
If ChatGPT requests its full advertised scope while both writing permissions remain off, Hearth grants only `hearth.read`. This keeps the default companion and accounting reads connected without silently enabling writes; the member must opt in and reconnect before write scope can be granted.

The write surface is intentionally narrow: add one expense, income, refund, internal Hearth transfer, or one eligible worked Evidence shift. Transactions use `transaction_write_options` → `prepare_transaction` → explicit confirmation → `confirm_transaction`. Worked shifts use `shift_write_options` → `prepare_shift_from_evidence` → explicit confirmation → `confirm_shift_from_evidence`. Preparation validates the ordinary command kernel, duplicate scanner, job/rate/overtime/tip-out rules, accounts, date/period, evidence authority, integer cents, and double-entry result but writes nothing. ChatGPT must show the complete preview and receive explicit confirmation before it calls either consequential confirm tool. No tool can delete, edit, pay a bill/card/bank, change settings, treat a schedule/email as worked time, or perform a bulk write.
...
`scrub_my_7shifts_email` does not connect to Gmail. Jonathan explicitly imports 7shifts mail from Shift → Evidence using short-lived Gmail read-only consent; Hercules then reads only reduced owner-scoped Evidence observations. Raw RFC822, sender/recipient/subject/body, object ids, provider ids, source paths, and hashes never enter the tool result. Schedule and notification email is always labelled review/outlook-only and cannot produce a shift-write option.

`confirm_shift_from_evidence` additionally rechecks the exact immutable Evidence bundle and every affected Personal/Household write permission, then reruns `postWorkShift` before accepting the sealed preview. It records `hercules-pro-shift` in the ordinary command receipt and uses the same hosted exactly-once RPC. Changed evidence, expired selection, missing job mapping, conflict, missing required shift fields, settled/closed books, or a stale ledger means zero write.
