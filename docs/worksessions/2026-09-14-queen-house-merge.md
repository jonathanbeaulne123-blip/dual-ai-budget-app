...
- **Frosted, not wireframe.** Jonathan's live books are mostly planned expenses, and on the deployed page they came up as a row of grey wireframe ghosts, all the same bean, tint invisible — the rule was right and the picture was wrong. A planned expense is now its own body (a round cat, no tail) in **frosted glass**: a translucent tinted clay in 3D (`RoomVessel.frosted`, one material per hue, `depthWrite` off), a third-strength tint with a dashed edge in the flat twin. The wireframe ghost stays for the paid shard only. Recurring became the tall cat and the appointment the loaf, so the four purposes are four silhouettes.
