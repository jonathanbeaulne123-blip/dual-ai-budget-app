...
- [x] Captures at 320/390/720/1100/1440 × classic/taylor/newfoundland × {arrival, keyboard focus, piece tap → Loft/Cellar with door strip, her portrait with the phrase, quick sheet, reading edition} on the Demo Suite's synthetic "doing well" habitat (`?seed=demo`: Everyday $6,746, Protect $853, the sundial's tag "Toronto Hydro", the mailbox flag up) — `docs/evidence/little-harbour/` (91 files, `report.json` carries tier, draw calls, twin labels, console errors: none).
- [x] The actual quick gate at risk Medium (below): every phase passed except the serial browser lane, which breached the five-minute budget and failed on three files that fail identically on `origin/main` in this container.
...
- Three visual passes were needed: (1) the first desktop diorama looked down on the court with the gate arch and sundial looming in front of her — retuned to a real three-quarter (`theta 0.5, phi 1.04`, the target a little right and in front of her, `r` fit to the plinths with a floor of 11.5); (2) the retune first came in too close (the Rook cut at the right edge) — the fit dropped its shoulder subtraction; (3) a 720-wide portrait tablet was a small, cut court — a column (aspect < 1.05) now takes the phone framing. Her phrase moved left of the desktop compass pill.
- Polish pass (coordinator's review of the look, "model village on a kitchen table"): the per-tile jitter had tilted tiles dipping under the joint plane and reading as dark triangles — paving is now level, tone-only (±4 % lightness), thin joints, three chipped corners; shadows on both tiers (PCFSoft 1024, normal bias 0.02) plus radial contact discs under everything that stands; the island got an edge (lawn → sandy shore → darker shallows → sea) and a ring of village-scale low-poly trees and shrubs (three instanced draw calls); pots became thrown lathe pots with icosahedron shrubs, the mailbox a real box with a rounded lid, an outlined door and a knob, the gate a low picket garden gate with brass only on the hinges, Hercules got upright ears, a curled tail and a dark nose; the phone pose keeps the Bishop's head and plate in the column (`target [0.15,0.8,1.7], r 8.6, theta 0.06`). Draw calls rose from 110–173 to 280–347 (shadow pass included); SwiftShader render time is not a device measurement.
- Quick gate (`pnpm test -- --risk=medium --focus=test/harbour-reading.test.ts --focus=test/harbour-queen-touch.test.ts --focus=test/harbour-camera-poses.test.ts --focus=test/harbour-source-fences.test.ts --focus=test/house-world-camera.test.ts --focus=test/app-startup-p1.test.ts --focus-reason="…"`): diff-check passed (0.2 s), ai-surface passed (0.5 s), typescript passed (55.6 s), test-discovery passed (10.7 s; selected 57, fast 49, serial 8, related 65, trimmed 45), vitest-fast **49 files / 406 tests passed** (37.4 s), vitest-serial **breached the five-minute budget** (354 s; total 460 s) and failed on three files: `test/hearthside-actual-app-browser.test.ts` (one case timed out at its own 180 s limit; the other passed), `test/hearthside-bank-ack-recovery.test.ts` and `test/workspace-merge-review-browser.test.ts` (both "Executable doesn't exist at /opt/pw-browsers/chromium_headless_shell-1234" — this box ships 1194). `test/app-startup-p1.test.ts` (83), `plan-system` (19), `proof-matrix`, `hearthside-actual-app-v2-browser` and `hearthside-workspace-browser` passed inside the serial lane. The timed-out case was rerun standalone with a 420 s test timeout (still 180 s: the limit is the test's own) and then **on `origin/main@10e05ed` in a temporary worktree with the same `node_modules` — it times out identically there** (3 m 30 s run), so it is this container, not the branch. The gate's own verdict is therefore "failed: serial lane", with the failures attributed to the environment; a run on Jonathan's Mac is the real gate.
...
- The quick gate's serial browser lane cannot pass in this container (missing headless shell 1234; one actual-App browser case at its 180 s limit on `main` too); it needs Jonathan's machine.

## Handoff

Next owner: Jonathan. State: **local only** on `claude/little-harbour` — not pushed, no PR, not merged, not deployed, not live verified. The bundle, patch, evidence and HANDOFF.md are in `/home/claude/hearth/handoff/` (outside the repo). Push, open the PR for a second reader's trust review of the four App seams and the fences, run the quick gate on a machine with the Playwright browsers, and look at `docs/evidence/little-harbour/` before pulling a real phone out.
