...
The report and receipt use the current member's private Development workspace; no peer device is a host. The submitted report is disclosed only after its editable review, to the fixed spreadsheet's existing collaborators. The authorized release sets new submission on. Disable that flag to roll back new execution while keeping the additive receipt table and credential for receipt recovery.
...
- Jonathan completed the Google two-step-verification requirement. Browser automation then failed with extension-popup/debugger errors and native `noWindowsAvailable`; Google creation/key-download steps were handed to Jonathan, with no repeat approval request.
- Jonathan identified the chosen service account as `hearth-feedback-sheets-975@hearth-506304.iam.gserviceaccount.com` (Hearth Feedback Sheets). The Google Drive connector successfully added Writer access to the fixed feedback spreadsheet. Credential download/installation and live submission remain pending at this checkpoint.
- PR #465 is open. Its initial CI, Cloudflare build/document-container and Hercules confirmation/recovery workflows passed. Review found the first-name reporter mapping issue; the correction preserves Jonathan/Bianca attribution. K2 validation was re-read live and remains the verified TRUE/FALSE dropdown; the adapter now also supports an ordinary BOOLEAN checkbox, while refusing custom checkbox values incompatible with `boolValue:false`.
- The supplied JSON credential was validated locally against the exact approved service-account email and project without printing its private key. Cloudflare stored `HERCULES_FEEDBACK_SERVICE_ACCOUNT` in version `a25185b7-292c-4eb2-a8f4-0d58076685bb`; this intermediate version was not activated. Ordinary secret edit was refused because another Worker version was not deployed, so versioned secret storage was used instead.
- A real OAuth token exchange succeeded. The bounded target spreadsheet metadata read returned Sheets API disabled for project 995991264128 (hearth-506304); API enablement is the remaining Google connection step at this checkpoint. The report was not submitted by that read-only check.
