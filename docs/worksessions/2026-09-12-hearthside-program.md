...
## Integrated recovery and actual App review

- `2db4891` merges current main `a96c1ff` into the local program checkpoint. The new Nest/King, Calendar and Hercules changes are preserved alongside Hearthside. This is still an implementation checkpoint, not a frozen release candidate.
- Legacy bank compatibility `1c30fdd` is integrated with a narrow adaptation for the new default King gallery. A missing v2 connection cannot downgrade a v2 review. Actual legacy acceptance receipts are supported in the existing non-v2 writer, including previously stuck creations and uncertain false responses. The updated tests enter through the actual Nest → New bank controls. Bank UI/authority/runtime regressions passed 28 tests in 10.77 seconds.
- Lost-ACK repair `f11fbbb` is integrated. WS ACK and covered authenticated HTTP receipts settle the same durable queue/waiter once, with scope, actor, intent and receipt checks. Root also routes the household-only reader used by native funding through that settlement. Six real Chromium/IndexedDB/LedgerRoom/SQLite/R2 regressions pass in 3.71 seconds; the original unchanged client was separately shown to retain its busy latch.
- Unknown backing remains explicitly unavailable in the new Nest's labels, accessible names and dependent category projection. A fixed display scale does not imply an empty balance. Financial restore preserves current Nest creative metadata.
- Unsigned native CI package `6194299` is integrated with the workspace-package path trigger retained. Its eight local packaging/config boundary tests pass. SDK compilation, instrumented/native suites, actual bundle-copy proof, physical AR, signing and distribution are still distinct open checks.
- Actual Studio entry/exit performance passed on local headless Chrome at 390/1440 widths: five cycles, keyboard p95 12.1–18.1 ms and pointer p95 5.7–8.3 ms, no growing retained resource counts and all contexts released. These are local measurements, not physical phone/desktop FPS certification.
- Fresh Claude browser consultation is now completed despite the locked Desktop session. The critique and accepted/rejected recommendations are documented in `docs/hearthside/claude-continuity-review.md`. Root found and addressed selected unfinished-piece URL continuity; actual Studio callback/Back verification is included in the pending Nest package.
- The first real App/PGlite browser proof passed in 25.70 seconds: intention reload, Task/Calendar exact identity and return focus, three themes at 390/1440, old fitting entrance and read-only unauthenticated behavior without external requests. Subsequent stricter Task → reload → return found lost focus on lazy mount; root adds in-surface focus restoration at mount. This new case remains under rerun.
- Actual screenshots exposed generic page artwork and a second scene header above Hearthside. The shared Hearthside destination now uses its own authored room/arrival composition and atmosphere control, preserving normal App/Fund/navigation chrome. Fresh actual-App geometry and screenshots are being collected; specimen-only evidence is not used to close this integration finding.
- Program-wide High gate/build are still pending the combined Nest and Workspace fixes plus independent review. No hosted schema, activation, deployment, invitation, external calendar write, distribution or Production action occurred.


## Combined-source verification checkpoint

- Canonical Nest package `7d2cac63637a133b072009fa1294e499a7cef10a` is integrated. Shared King creation uses canonical design history without creating a financial Goal. Source-index recovery, current-source authorization, preserved paid revisions, independent undo/redo and immutable ornaments across exports/native/guest/memory/projector pass the package's narrow compiler and 9 new, 56 affected and 26 guest/surface/projector tests. Root owns the combined gate. The integration patch's blank context whitespace was normalized and its reverse applicability verified.
- Workspace review `701872f35cb158dc0fa0312e3bd8601e402dcb97` is integrated: effect-generation fencing repairs StrictMode first opening; explicit report identity opens separately from a selected intention; saved private selections cannot rebind the report. Independent review reproduced the saved-selection race before the final fix. Its 49 tests passed in 10.11 seconds with actual SQLite recovered tool admission and synthetic browser transport; no provider was called.
- The actual App/PGlite matrix now passes Task → reload Planner → exact return focus, all seven required widths, three themes, 320px enlarged text/keyboard and source/receipt stability. The latest pre-resident run passed in 30.43 seconds. Final resident and merged Nest source-hash rerun is pending at this checkpoint.
- Hearthside now uses the quiet Hercules resident label “A little company” while closed. Entering an affectionate room does not display an unsolicited financial reminder or proposal. Ordinary Home retains the canonical reminder, and deliberate Hercules opening remains available. The final actual-App proof checks these boundaries without invoking providers.
