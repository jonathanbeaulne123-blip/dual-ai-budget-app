...
- The first live chat smoke reached the new quota coordinator but failed before Google: Workers rejects Fetch `redirect: error`. Use `manual` and retain the existing non-2xx rejection so credentials never follow redirects. Add count/generation redirect rejection regressions. No live model reply is claimed from this failed smoke.
