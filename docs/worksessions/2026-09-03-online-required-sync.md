...
- A second exact-head audit found Shared-only startup/catch-up and an Auth/member shortcut. The follow-up binds Auth to the exact selected member, adopts stable Shared+Personal together on every launch-mode pull, and binds write readiness to the exact environment/household/member/revision tuple. TypeScript and 99/99 combined startup, continuity, runtime, isolation, and policy tests pass; missing Personal and mismatched Auth prove zero staging or local advancement.
- Final audit follow-up removes both manual and background legacy conflict republish in launch mode. A definitive conflict adopts a stable pair, compare-cancels only its exact outbox/stage generation, and requires a fresh Confirm; missing Personal retains the blocked generation. Realtime command acceptance invalidates the prior tuple and schedules an immediate paired refresh.
- Exact cloud-authority follow-up removes the rejected local Shared candidate from every launch conflict path. Direct Confirm conflict, background conflict, manual Retry, startup, and ordinary catch-up now install the canonical Shared+Personal pair without conflict reconciliation; the losing financial row cannot become a later pending auto-publish. Manual Retry clears readiness first and cannot report Synced until paired adoption succeeds. Focused TypeScript and 100/100 runtime, continuity, online-policy, environment-isolation, and startup UI tests pass.
- Recovery-order and scope follow-up keeps the exact conflict generation durable until canonical local repair and persistence succeed, bypasses revision-only Realtime dedupe whenever paired facts differ, and invalidates an awaited adoption across household/member/environment changes with projection restoration after an in-flight scope switch. Focused TypeScript and 103/103 tests pass.
- Final serialization follow-up places Confirm, canonical pair installation, ledger switching, and destructive local clearing on one write lane. A queued install is also bound to its starting live household, scope generation, and exact outbox contents, so a newly cloud-acknowledged Confirm cannot be rolled back by a stale replay. Conflict generations are cancelled only after canonical repair and persistence; clear/sign-out, household removal, and Development reset invalidate adoption immediately, wait for prior writes, durably erase any resulting outbox marker, and then remove staged and active local copies. The Development cloud reset also runs inside that lane so a late Confirm cannot recreate a just-deleted household.
- Hydration/cleanup follow-up makes outbox hydration single-flight and mutation-epoch guarded, so a delayed IndexedDB read cannot overwrite a newer Confirm or resurrect a cleared marker. Hydration unions localStorage, IndexedDB, and memory by monotonic tip revision and confirmation-set dominance before timestamp/generation tie-breaks, rather than trusting a stale first store or wall clock. Destructive clear reconciles those sources, fails closed on any durable-store read error, and requires deletion from every available durable store. Successful cloud acknowledgement uses the same strict deletion rule and restores the exact marker as pending if any store refuses. Staged prewarm and candidate validation share a per-household lane so an older warm-up cannot replace a newer ambiguous candidate, while local-only Development households retain their ordinary receipt-gated v8 rebuild.
...
- Launch-mode conflict recovery never auto-merges or republishes the losing staged snapshot. Realtime command materialization cannot carry its prior revision's readiness forward.
