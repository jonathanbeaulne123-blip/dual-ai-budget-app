...
- Hydration/cleanup follow-up makes outbox hydration single-flight and makes destructive clear reconcile memory, localStorage, and IndexedDB before requiring deletion from every available durable store. A deletion refusal is surfaced instead of reporting the phone cleared. Staged prewarm and candidate validation now share a per-household lane so an older warm-up cannot replace a newer ambiguous candidate, while local-only Development households retain their ordinary receipt-gated v8 rebuild.
