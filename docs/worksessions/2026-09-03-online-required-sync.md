...
- Hydration/cleanup follow-up makes outbox hydration single-flight and mutation-epoch guarded, so a delayed IndexedDB read cannot overwrite a newer Confirm or resurrect a cleared marker. Destructive clear reconciles memory, localStorage, and IndexedDB, fails closed on a durable-store read error, and requires deletion from every available durable store. A refusal is surfaced instead of reporting the phone cleared. Staged prewarm and candidate validation now share a per-household lane so an older warm-up cannot replace a newer ambiguous candidate, while local-only Development households retain their ordinary receipt-gated v8 rebuild.
