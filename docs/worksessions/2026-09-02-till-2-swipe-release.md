...
- [x] Every resolved reversal ancestor and every resolved transfer-pair leg must remain countable in compiled books and every reporting projection
- [x] PGlite projection version 7 invalidates the pre-fix proof and performs one full rebuild only from a cache that matches the accepted receipt
- [x] Card status, Ask/Hercules numeric answers and cash runway, member year review, CRA medical eligibility, cook-off, and subsequent-events counts use the same full correction-lineage predicate as recognized books
