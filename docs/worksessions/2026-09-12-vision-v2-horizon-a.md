...
- **A3** Household Home: `src/HouseholdHome.tsx` + `src/household-home.css` behind `householdHomeV2Enabled()` (on with Plan V2 in Development; `VITE_HOUSEHOLD_HOME_V2=0` or the Plan V2 rollback restores the Office-based Home). The Office remains reachable under Home as collapsed "instruments" so the Bianca mainline regression (which mounts the Office) stays green; the Month-One rehearsal access renders inside the Chapter area (Chapter 1).
...
PR #451 closes all twelve findings raised after #449 merged and during the follow-up review: unreconciled or stale Fund pulses now fail closed and remain visible when Hercules chat opens; rejected Chapter commands retain their drafts; each partner receives only valid Memory choices; comfort settings initialize at the App root, refresh after an environment change, preserve legacy clink opt-ins, and drive sound/haptics independently; Household Home cannot outlive its Plan V2 destination; and the Sitdown brief attributes held evidence to each Ritual. This follow-up changes no money command, financial hash, schema, Auth/RLS, or Production setting.
