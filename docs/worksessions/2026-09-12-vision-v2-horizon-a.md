...
PR #451 closes the eight findings raised after #449 merged: unreconciled or stale Fund pulses now fail closed (including Hercules Home); rejected Chapter commands retain their drafts; each partner receives the Memory choice; comfort settings initialize at the App root and sound/haptics are independent; Household Home cannot outlive its Plan V2 destination; and the Sitdown brief attributes held evidence to each Ritual. This follow-up changes no money command, financial hash, schema, Auth/RLS, or Production setting.
