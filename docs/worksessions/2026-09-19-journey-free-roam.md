...
- [ ] Quick gate: `pnpm test -- --risk=medium-high --focus=test/journey-free-roam-ui.test.ts --focus-reason="free roam camera in game mode"` — **blocked by this checkout, not by the change.** `diff-check` and `ai-surface` pass; the `typescript` phase fails after 71.3s with the twelve pre-existing `@capacitor/core` / `@hearth/browser-ar` / `manifold-3d` resolution errors, so the gate never reaches its test phase. Proved pre-existing: `git archive 4de2b28e` into a clean directory with the same symlinked `node_modules` gives a byte-identical `tsc --noEmit` output. `node_modules` is symlinked here and installing was out of scope, so the gate cannot go green in this environment. No time-budget breach (72.6s of 300s, `timeBudgetBreached: false`). The suites the gate would have selected were run directly instead and all pass (261 tests in 26 files, listed above).
