...
D-246 (this slice; D-245 is the planner on main). Narrows D-243's + verb set: navigation verbs leave the +, and its closed name is "Add money" in both spaces. Retires the Our Home secondary nav per Vision v2 §4.4. The planner room from D-245 stays reachable from Together.
...
Local branch only at authoring; this rebase targets a PR onto main. Next owner: Jonathan to review the PR; Codex to audit D-246, the read-path span change in `board.ts` / `calendar/google.ts`, and the three new test fences; then Bianca's read on a phone.
