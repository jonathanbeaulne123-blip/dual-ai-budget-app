...
Work on high-risk Issue #10 now continues on isolated branch `issue-10-cad-currency` as the local-only v0.0.30 candidate. Direct inspection of the retained development snapshot found one active account, 35 Raw Transactions rows, and 35 Transactions rows labeled `USD`; Jonathan confirmed every value is actually CAD and no amount conversion is permitted. The candidate makes `Accounts.Currency` authoritative, requires a single active CAD account in both Add Transaction and Add Shift, copies that currency into both record layers, expands Data Health Check to detect currency/account drift, and provides a guarded development-only migration that discovers the live row extent by stable IDs. The migration previews every cell, locks and rebuilds the plan before writing, verifies non-currency row fingerprints, supports a repeat-safe no-op, and proves rollback after simulated failures. Development Apps Script remains v0.0.29; no v0.0.30 source or Sheet change has been authorized.
