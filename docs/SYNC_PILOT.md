...
- A failed or response-lost hosted attempt leaves the prior active PGlite projection and visible/durable household unchanged. Its slim marker and isolated staged candidate remain until the same command is replayed or its exact receipt is confirmed, so no ambiguous response is labelled “nothing posted.” If the receipt is found inside a newer revision, Shared and the signed-in member's revisioned Personal envelope must come from a stable paired read before writes reopen. A definitive CAS conflict cancels that generation.
