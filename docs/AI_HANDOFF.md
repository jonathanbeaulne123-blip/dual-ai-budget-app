...
Uncertainty: run bars align across cells only when neighbours carry the run in the same chip row; a folded phone-desk object that must take focus opens the fold in the same tick (jsdom cannot show the one-frame `visibility:hidden`); the plan's "≥ 60% less visible prose on daily surfaces" was not met — what remains is labels, not explanations; `poets` paper is below AA for all small text, kind chips included, though none render there today; stored notes and engine strings still say "Sit-down" / "PGlite". Corrections to the plan: the Google multi-day "truncation" was not a live bug, and the Hercules lesson lines were already one sentence. Data/environment: fictional catalog books only; no hosted, schema, Production or real-household writes. This session's git proxy has no push credential, so the branch is **local only — not pushed, not a PR, not merged, not deployed, not live verified**. Next owner: Jonathan to review the PR onto main; Codex to audit D-246, the span read path and the three new fences; then Bianca's read on a phone. [Worksession](worksessions/2026-09-12-feedback-rows-5-7.md) · [Rebase](worksessions/2026-09-12-feedback-rows-5-7-rebase.md).
