...
**Architecture:** `householdFundEventKindForPost` classifies funded `postEntry` before clone. `purchase-funded` requires the Fund custodian with `HOUSEHOLD_PURCHASE_CUSTODY_REFUSAL`. `observedSwipeCategories` / `resolveSwipeCardAccount` are pure selectors. `Swipe` is a CadPad dialog; `submitSwipePurchase` posts through existing `run` / `commitHousehold`. More and an ambiguous card open Add. Fat-finger Undo of a funded swipe uses `reversePostedMoney` (append-only refund-funded), not `undoLedgerConfirm` deletion. While the sheet is open, App passes `adding || swipeOpen` so Hercules pauses like Add, and `.swipe-sheet` stacks at z-index 32. Worksession: [`worksessions/2026-09-02-till-2-swipe.md`](worksessions/2026-09-02-till-2-swipe.md). GPT review: [`briefs/CHATGPT_TILL_SLICE_2_REVIEW_2026-09-02.md`](briefs/CHATGPT_TILL_SLICE_2_REVIEW_2026-09-02.md).
...
- Independent books audit on `938cb33`: **PASS WITH NOTES**. P1: fat-finger `undoLedgerConfirm` left an active `purchase-funded` claim after the journal row was gone. Repaired in App `applyUndo` by routing `FUND-EVT-*` tokens through `reversePostedMoney`. P2 pre-existing: `postOneRecurrence` can still post as the first active member — not Slice 2.
- Independent privacy audits on `27f48f19` and `938cb33`: **PASS WITH NOTES**. No P0/P1. Residual P3 selector/sheet-lifetime defense-in-depth only.
- Independent verifiers on `a00cf80` and `938cb33` returned **CONDITIONAL** because they read those SHAs before exact `pnpm check` and visual proof were recorded. Those later proofs landed on `938cb33` / docs `e42b74c`.
- UX auditor failed to launch (monthly spend limit).
