## Hearthside full program — implementation open (2026-09-12)

Integration checkout `.codex-work/hearthside-program`, branch `codex/hearthside-program`, baseline429db8ef0447dac6347e89e945dd04aad05e84fb. Preserves #461 Home cats and selected-bank return focus. The full user-approved P0–P15 scope remains open; use the [package register and evidence](worksessions/2026-09-12-hearthside-program.md). Core metadata/authority, creative history/deltas/recovery, simultaneous Studio, room routes, twelve illustrated rooms, private letters/media/capsules, paired text/design memories, recurring occasions, native source and export kernels/surfaces are implemented locally at varying validation stages. Work is staged/uncommitted; no integrated frozen-head completion claim.

Integrated locally: Vault exact-memory binding and recipient letters/voice/capsules; canonical Chapter/Ritual/Task consent; real Projector audio/video; selected-intention Workspace and durable shared-copy recovery; all twelve seasonal encounters with entry-to-Studio/memory bridges; room history/furniture; guest namespace and own-account Street; native auth/widget source and immutable exports. Intention-to-bank `510ba99` is now integrated and undergoing root validation. Remaining implementation integration: explicit shared-life restore, final actual guest service assembly proof and unsigned native build configuration. Root frozen High gate/build, hosted authenticated continuity and physical native/presentation/performance acceptance remain open. All activation flags remain separately controlled. No deployment, hosted migration, invitation, external calendar write, signing or Production activation occurred.

Host contention can breach the300 s High gate budget. Room component gate functionally passed at441.958 s and is explicitly not release-green. Do not run competing heavy compilers or erase this failed performance gate. Root required High focus map/build and actual physical/authenticated acceptance are still pending.
