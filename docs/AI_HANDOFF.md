...
**Repair gates closed in source:** rejected posts use a Swipe-scoped `role=alert` with retry/More guidance and a non-destructive warning treatment; the modal ref moved to the outer overlay so `useDialog` can inert App siblings; the duplicate CAD-pad title is now `Amount`; Hercules pauses and the overlay sits above its pill; funded Undo is bounded to explicit `postEntry` and `postHouseholdFundDirectDebit` command kinds with one current transaction and a current Fund fact. Compact history preserves that identity across reload and suppresses legacy funded rows that cannot choose a safe correction. The forward repair also makes reporting and cash-flow projections follow reversal lineage, binds the ten-second strip to one environment/household/member and closes it during a ledger switch, suppresses an unusable second Undo after the append-only funded correction, and lets a focused Close button own Enter. Unknown funded command shapes refuse rather than guessing from `FUND-EVT-`.

**Verification:** the forward repair's focused Swipe/statements/accounts/opening/custody/history suite is **47/47 passed**; after integrating Register Slice 8, the focused cross-slice suite is **60/60 passed**. Exact rebased-head `pnpm check:windows` passed AI surface; **226 fast files passed / 1 skipped with 1,564 tests passed / 2 skipped**; **18 serial books files passed / 1 skipped with 146 tests passed / 1 skipped**; **1,710 passed / 3 skipped total**; TypeScript; a **416-module** production build; Hercules Pro UI; and redirect guard. It covers expense, refund, income, transfer, and reversal-of-reversal projections; exact strip scope; no second funded-correction Undo; and Close-key ownership. Independent Release, PR, post-merge CI, deployment, and live no-store/asset gates remain pending. Worksession: [`worksessions/2026-09-02-till-2-swipe-release.md`](worksessions/2026-09-02-till-2-swipe-release.md).
