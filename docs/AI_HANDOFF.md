...
## Bianca Month-One rehearsal (D-182) (2026-08-28; mainline catch-up 2026-08-31)

**Status:** High-risk local core and App integration in isolated worktree `codex/bianca-month-one`, caught up to current `origin/main@1d82a57`. The recoverable pre-catch-up head is `codex/bianca-month-one-pre-main-20260831`. Bianca Month is now a mainline projection, not a trial fork: the current App integration test opens the current Add slideshow, and rehearsal progress materializes through the D-180 command-event path. Final current-main verification is in progress. No push, merge, deploy, hosted schema, provider, Production, scaling, shift-intake, or real household entry.
...
**Verification:** current-main focused App/opening/rehearsal/Fund/continuity/materialization verification passed **83/83 across 14 files**. After review hardening, **1,319/1,319** tests passed outside the environment-sensitive deployment file and `test/api.test.ts` passed **8/8** with bundled Git/Python paths: **1,327 passed / 3 skipped / 0 failed** combined. TypeScript, AI-surface verification, the Windows-native Vite production build, Hercules Pro UI build, and no-`dist/_redirects` assertion passed. Responsive local browser proof at 320/390/720/1100 px found no horizontal overflow or console warnings/errors and retained ordinary Hearth navigation, preview-only future weeks, and visible focus. Exact September 2026 golden assertions still cover every cent, journal/trial/equation, Fund projection, receipt identity, and financial/checkpoint hash. Independent acceptance review found no P0-P2; books/trust re-review confirmed compaction coverage fixed and retained the server-authority boundary below. Local synthetic evidence is never authenticated continuity evidence.

**Next owner:** Codex after Jonathan's action-time approval — send the exact packet/diff/evidence to Gemini and disposition findings. Real two-authenticated-phone continuity/joint-signoff proof remains pending an authorized Development endpoint and identities. Jonathan remains the only owner who may approve a push or any launch/Production step.
