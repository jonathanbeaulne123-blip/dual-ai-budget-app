...
**Repair gates closed in source:** rejected posts use a Swipe-scoped `role=alert` with retry/More guidance and a non-destructive warning treatment; the modal ref moved to the outer overlay so `useDialog` can inert App siblings; the duplicate CAD-pad title is now `Amount`; Hercules pauses and the overlay sits above its pill; funded Undo is bounded to explicit `postEntry` and `postHouseholdFundDirectDebit` command kinds with one current transaction and a current Fund fact. Compact history preserves that identity across reload and suppresses legacy funded rows that cannot choose a safe correction. The forward repair makes reporting, cash-flow, snapshot P&L, and the compiled journal follow the same reversal lineage, including reinstatement; binds the ten-second strip to one environment/household/member and closes it during a ledger switch; suppresses an unusable second Undo after the append-only funded correction; and lets focused Close and `summary` disclosure controls own Enter. Unknown funded command shapes refuse rather than guessing from `FUND-EVT-`.

**Verification:** the forward repair's focused Swipe/statements/accounts/opening/custody/history suite passed **47/47**; after integrating Register Slice 8, the focused cross-slice suite passed **60/60**. The additional journal/disclosure repair passed its focused PGlite/books/UI set **51/51**. The latest local `pnpm check:windows` passed AI surface; **226 fast files passed / 1 skipped with 1,565 tests passed / 2 skipped**; **18 serial books files passed / 1 skipped with 147 tests passed / 1 skipped**; **1,712 passed / 3 skipped total**; TypeScript; a **416-module** production build; Hercules Pro UI; and redirect guard. It covers compiled-book, snapshot, budget, statement, cash-flow, and desk reversal parity; exact strip scope; no second funded-correction Undo; and native Enter ownership for Close and `Switch kind`. A new exact-head review, PR CI, post-merge CI, deployment, and live no-store/asset gates remain pending. Worksession: [`worksessions/2026-09-02-till-2-swipe-release.md`](worksessions/2026-09-02-till-2-swipe-release.md).
