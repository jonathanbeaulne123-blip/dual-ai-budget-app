...
**Verification:** focused gate/collector/roadmap tests passed **3 files / 30 tests**, including deliberate redaction, private/cross-origin refusal, and reduced-motion red fixtures. Clean exact implementation `e9bcb2ac6d86c359d068fb1290d72b87a2911310` passed AI-surface verification, **209 passed / 2 skipped files and 1,400 passed / 3 skipped tests**, TypeScript, the 383-module Vite production build, Hercules Pro UI build, and the no-`dist/_redirects` guard. Its ignored browser report passed **10/10** public-roadmap task/recovery runs over all five widths with zero serious/critical axe findings, console/page errors, all-origin network failures, timeouts, overflow failures, focus-order failures, text-resize failures, or reduced-motion failures and remains `claimable: false`. Existing React act, PGlite browser-external/eval, and chunk-size warnings remain unchanged.
