...
**Status:** Post-merge repair on `codex/till-2-postmerge-repair`, rebased to exact `origin/main@02692d7791869057d0fc422d231751cef18ab0c6`. Historical `main@2b9f77051b4abfdddce2fd2f580e41a36a6c8772` is PR #298's first Slice 2 merge: its Development deployment passed, but a late public review found two P1 and two P2 defects. PR #302's first green head then received one additional P1 journal-parity finding and one P2 disclosure-keyboard finding before merge. Independent review found two more P1s before push: transfer duplicate lineage and the missing local PGlite projection re-anchor. The public exact-head review of `643a786` then found that an intermediate duplicate reversal could still let a later reinstatement count. That final lineage edge is repaired locally; merge remains locked pending a new exact-head review and CI. **The first deployment remains release-blocked and is not a Slice 3 baseline.** This forward repair must pass a new exact-head review, PR, main CI, deployment, and live proof. Risk: **Release**.
...
**Repair gates closed in source:** rejected posts use a Swipe-scoped `role=alert` with retry/More guidance and a non-destructive warning treatment; the modal ref moved to the outer overlay so `useDialog` can inert App siblings; the duplicate CAD-pad title is now `Amount`; Hercules pauses and the overlay sits above its pill; funded Undo is bounded to explicit `postEntry` and `postHouseholdFundDirectDebit` command kinds with one current transaction and a current Fund fact. Compact history preserves that identity across reload and suppresses legacy funded rows that cannot choose a safe correction. The forward repair makes reporting, cash-flow, snapshot P&L, and the compiled journal follow the same reversal direction and requires every resolved lineage row and each resolved transfer-pair leg to remain countable, including intermediate corrections and reinstatement; local PGlite projection version 7 invalidates only the derived projection proof so an accepted snapshot receives one transactional full re-anchor; the ten-second strip is bound to one environment/household/member and closes during a ledger switch; an unusable second Undo is suppressed after the append-only funded correction; and focused Close and `summary` disclosure controls own Enter. Unknown funded command shapes refuse rather than guessing from `FUND-EVT-`.

**Verification:** the forward repair's focused Swipe/statements/accounts/opening/custody/history suite passed **47/47**; after integrating Register Slice 8, the focused cross-slice suite passed **60/60**. The journal/disclosure repair passed **51/51**; the transfer-duplicate and PGlite-v7 set passed **59/59**. After the `643a786` public review, the expanded six-file lineage/PGlite/Swipe/Add set passed **61/61**, including duplicate intermediate expense and transfer-pair reversals. The fresh exact-head `pnpm check:windows` passed AI surface; **226 fast files passed / 1 skipped with 1,565 tests passed / 2 skipped**; **18 serial books files passed / 1 skipped with 151 tests passed / 1 skipped**; **1,716 passed / 3 skipped total**; TypeScript; a **416-module** production build; Hercules Pro UI; and redirect guard. Exact-head review, PR CI, post-merge CI, deployment, and live no-store/asset proof remain pending. Worksession: [`worksessions/2026-09-02-till-2-swipe-release.md`](worksessions/2026-09-02-till-2-swipe-release.md).
...
**Next owner:** Codex completes the forward repair, independent Release review, and the user-authorized guarded merge/deploy. Cursor may start Slice 3 only from the exact deployed `origin/main` SHA named by the CLOSED worksession; never from PR #295, `25ef99e`, `3ca2f5b`, or the release-blocked first merge `2b9f770`.
