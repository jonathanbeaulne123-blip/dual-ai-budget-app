## The kitty banks dressed for their purpose, and sized by hand (2026-09-15)

Branch `claude/cellar-bank-props`, one commit on `origin/main@0ae4490` (#482); deliverable `cellar-bank-dress.patch`. Jonathan: "we need more unique shapes, hats, props, anything to make it extremely clear what purpose each bank serves. i also think we need to scale the kitty banks up a little they are too small. or users should be able to zoom in and out to toggle scale themselves." Chosen: both, my own prop vocabulary, everywhere a bank stands. **Risk Medium** — presentation behind `VITE_QUEENS_NEST`; one per-device preference in `localStorage`; no money meaning, schema, Auth/RLS, sync, hosted state, financial hash or Hercules payload change. Budget (5): +0. Engagement (3): +3 — a purpose is a hat, and the rail is the size the person wants.

What changed: `src/queen/world/queenBankDress.ts` (new, pure data) — one dressing per `BankForm`: **bill** a postman's cap + an envelope on the paws; **recurring** a wind-up key in the flank; **subscription** a collar with a bell; **appointment** a calendar leaf (header, ring, folded corner) for a hat; **planned** a folded paper hat over the frosted glass; **goal** a pennant planted beside her; **jar** bare. `queenBankSculpture.ts` builds only the pieces a form wears from primitives in the cat's own units and seats them behind the crown (the slot still accepts, the lid still refuses); the rooms supply `felt` and `paper` materials. `QueenBankFlat.tsx` draws the same dressing in SVG (`data-dress`), after the head and before the slot. `src/queen/cellarZoom.ts` (new): range 0.75–2.25, default **1.4**, quarter steps, `localStorage` in try/catch. `QueenCellarRail.tsx` sets `--cellar-zoom`; the day cell, the five size bands, the seats and the drag step follow it, so the sculpture (which follows the drawn seat) grows too; a two-finger pinch on the rail sets the scale and never the gate; ctrl-scroll / trackpad pinch; `+`/`−` keys on the focused rail; `CellarZoomPane` (smaller · % · larger) in the scrub row beside the slider, so it covers no bank. Each jar's `title` names its purpose and dressing; accessible names unchanged. The loft ledge is dressed through the same sculpture and twin.

Verification: `tsc` clean; new `test/queen-bank-dress.test.ts` 6/6; `queen-house`, `queen-cellar-ui`, `queen-cellar`, `queens-nest-ui`, `queen-rooms` 64/64; browser proofs and the focused gate — see the worksession. Not live verified.

Uncertainty: 3D verified in SwiftShader stills only — the cellar's low light keeps the cap a dark band with a peak, brighter in the flat twin; short frames crop a tall cat's hat first (existing rule); the loft's lidded things wear the bill's dressing whatever they are. Fictional fixtures only. Not pushed, not a PR, not merged, not deployed. Next owner: Jonathan to push and try the pinch on his phone; Codex for an independent read of `--cellar-zoom` under the short-frame container queries. [Worksession](worksessions/2026-09-15-cellar-bank-dress.md).
