...
**Repair gates closed in source:** rejected posts use a Swipe-scoped `role=alert` with retry/More guidance and a non-destructive warning treatment; the modal ref moved to the outer overlay so `useDialog` can inert App siblings; the duplicate CAD-pad title is now `Amount`; Hercules pauses and the overlay sits above its pill; funded Undo is bounded to explicit `postEntry` and `postHouseholdFundDirectDebit` command kinds with one current transaction and a current Fund fact. Compact history preserves that identity across reload and suppresses legacy funded rows that cannot choose a safe correction. Unknown funded command shapes refuse rather than guessing from `FUND-EVT-`.

**Verification:** focused Swipe, D-197 custody, and persisted Undo history passed **24/24**. Exact `pnpm check:windows` passed AI surface; **224 fast files passed / 1 skipped with 1,547 tests passed / 2 skipped**; **18 serial books files passed / 1 skipped with 146 tests passed / 1 skipped**; **1,693 passed / 3 skipped total**; TypeScript; a **413-module** production build; Hercules Pro UI; and redirect guard. Fresh visual proof at 320/390/720/1100 showed no horizontal overflow, 44 px controls, effective background `inert`, one title/Amount label, and Escape focus return. Final exact-head books/trust, privacy/continuity, and UX/accessibility reviews found no P0-P2. Both PR-head CI runs [33656791208](https://github.com/jonathanbeaulne123-blip/dual-ai-budget-app/actions/runs/33656791208) and [33656829180](https://github.com/jonathanbeaulne123-blip/dual-ai-budget-app/actions/runs/33656829180) passed. Post-merge main CI [33658014100](https://github.com/jonathanbeaulne123-blip/dual-ai-budget-app/actions/runs/33658014100) and Cloudflare deployment [33658014103](https://github.com/jonathanbeaulne123-blip/dual-ai-budget-app/actions/runs/33658014103) passed. Worker version `1afe500c-647b-4d3b-917f-0c39a95d2f0b` serves HTTP 200 with `Cache-Control: no-store`; live asset `index-CO1eGR2Y.js` contains the amount, accepted-strip, in-dialog error, and funded-Undo refusal markers. Worksession: [`worksessions/2026-09-02-till-2-swipe-release.md`](worksessions/2026-09-02-till-2-swipe-release.md).
