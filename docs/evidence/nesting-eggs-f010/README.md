...
[PR #466](https://github.com/jonathanbeaulne123-blip/dual-ai-budget-app/pull/466), Development fixtures only. The original implementation evidence source is `c85c5b7df72ce12f3b475409b7c15bde451f5296`, based on main `1686ccc4a569c99b5a2d92ae8b311b83873e211d`. `f7f00c6` updates only the existing Home browser test selectors and its optional local Chromium path. The final interaction-probe change places its empty-background point clear of the grown King and also asserts a successful near-edge hit. Matrix fingerprints precede that probe-only change; application source is unchanged. The evidence/docs commit `772ce22` did not change application behavior. The later release-review corrections are described below.

## Release-review corrections

The final merge review subsequently caught Both receipt history, removed-source admission and incomplete accessible names. The fixes use the canonical view/creator predicate, reject removed potentials and ineligible cost tasks, and expose visible money/date/paid details in bank names. Eight added regressions pass within the [26-test core/UI run](review-followup-tests.json); the [nine-case actual App check](review-followup-app.json) passes at 390/1100px in all three themes, including computed bank names, Return focus and setup routes. Current Personal posting policy remains Personal-only; the Personal Both regression exercises legacy receipt metadata rather than relaxing that policy. A server authority test proves a captured stale design save is rejected after another device removes its source. The [worksession](../../worksessions/2026-09-12-nesting-eggs-f010.md#authorized-development-release-review-follow-up) and PR retain the final release result. The prior full matrix below remains valid evidence for unchanged layout/paint; the follow-up App browser check exercises the changed accessibility names.
