...
- Kitchen cosmetics (`kitchen.chalkboard` including optional `ink`, Hercules companion + `kitchen.hercules` chat/memories, `kitchen.openShift`) are shared household data. They merge and tombstone like recurrences. They are not journal lines. `kitchen.books` (recs and closed months) is the Audit Office desk: also shared, also never journal lines. Reopen tombstones `CLOSE-YYYY-MM`.
