...
3. **Hosted snapshot transport** — optional, **opt-in** publish to the household Supabase project (D-110). Local ingest (`ingestHouseholdBooks`) never fetches. Empty start, demo kitchen, Hearth Pass, and unlinked commits make zero household REST calls. Invite → Publish (or Sync once linked) is the only client path that sets `linked: true` and upserts `households` + `household_snapshots`. The app does **not** fill the hosted normalized journal tables. RLS is still `USING (true) WITH CHECK (true)` for ALL, including DELETE. The bundled publishable key can read and write every hosted row. Treat hosted contents as disclosed until Auth (D-034, D-052). Possible leftover demo/unlinked rows: [HOSTED_ROW_INVENTORY.md](HOSTED_ROW_INVENTORY.md) — do not delete without Jonathan.
