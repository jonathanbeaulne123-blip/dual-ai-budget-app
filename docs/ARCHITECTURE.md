...
3. **Legacy hosted snapshot transport** — current D-110 code requires opt-in `linked: true` and upserts `households` + `household_snapshots`; local ingest never fetches. This remains accidental-upload containment while it is replaced by automatic identity/membership discovery plus a durable outbox. The app does **not** fill the hosted normalized journal tables.

Target continuity adds explicit personal-ledger and household-ledger scopes, Google identity/membership discovery, an idempotent durable outbox, automatic pull/focus/reconnect synchronization, and atomic hosted CAS. Every pulled or merged money state must pass the same environment, accounting, and idempotency acceptance boundary before persistence or display. Phrases and Hearth Pass files may remain invitation, export, backup, or recovery tools; they are not normal storage authority.
...
5. **Google engine** — Google identity is becoming account entry, ledger discovery, and recovery (D-112). Calendar, Drive, Contacts, Gmail, and Sheets remain separate opt-in integrations; they never `postEntry`, and Drive/Sheets are not the ledger. Tokens and hosted sessions never enter ledger snapshots or Git.
...
Current Supabase code is snapshot transport for **linked** households only. Unlinked, demo, empty, and Hearth Pass households make zero household REST calls. A linked write after local accept uses client compare-and-swap on revision; stale writes become visible conflicts. D-112 replaces this opt-in target with automatic Google-account continuity, without weakening local acceptance or conflict proof. `supabase/migrations/002_snapshot_cas.sql` and `docs/sql/rls_auth_ready.sql` are readiness packets. **Do not apply them.** Treat hosted rows as openly readable/writable disposable Development data through 2026-09-30; complete the reviewed Auth/RLS cutover before meaningful October data.
