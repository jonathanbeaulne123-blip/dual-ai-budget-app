...
The accepted target is Google-account cloud continuity (D-112): a signed-in person discovers their personal ledger and household memberships from any device; no device is the host. The cloud is the durable continuity layer, while every device uses PGlite as its validated accounting engine and offline replica. D-113 and D-114 implement the disposable-Development bridge in the following transitional stack:

1. **Command replicas** — IndexedDB `hearth-ledger` / store `households`, plus a `localStorage` fallback. Household snapshots are keyed by environment + household id, a catalog and active pointer drive the ledger switcher, and signed-in personal rows are also stored in a member-only envelope keyed by environment + household + member. Legacy environment-only snapshots migrate on read. Commands still operate on the accepted full household snapshot so reconciliation cannot drop a partner's personal facts.
...
D-113 supplies Development Google discovery, compacting outbox replay, and automatic pull/focus/reconnect synchronization. D-114 supplies multi-household local replicas, explicit switching, and a dedicated member-personal device replica. Target completion still adds a dedicated **hosted** personal-ledger scope, server-side membership discovery, acknowledgement/backoff, and atomic hosted CAS/journal authority. Every pulled or merged money state passes the same environment, accounting, and idempotency acceptance boundary before persistence or display. Phrases and Hearth Pass files may remain invitation, export, backup, or recovery tools; they are not normal storage authority.
