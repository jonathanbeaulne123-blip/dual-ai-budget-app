...
4. **Projections** — `monthSummary` (nets same-month reversals via `expenseEffect` / `incomeEffect` sign flip on `reversalOfId`, D-108), `weekSummary`, `buildDashboard`, `runHealthCheck`, `sitDownPreview`, `leftoverProjection`, `sitDownFacts`, `trialBalance`, `detectRhythms`, `detectHabits`, `composeNotices`, `buildMonthBoard`, `askBooks` / `askHercules`, `describeCompanion`, `herculesBriefing`, `composeHerculesChatRequest`, `householdWallet` / `creditCardView`, `auditOpinion`, `balanceSheet`, `incomeStatement`, `cashFlowStatement`, `statementOfChangesInEquity`, `liquidityWatch`, `notesToFinancialStatements`.
5. **Google engine** — `withGoogle` is a call-when-needed bridge (D-043 / D-078). Identity and Calendar are shipping kitchen features. Drive, Contacts, Gmail, and Sheets are opt-in and may each earn a Dual Course feature; they are not a daemon and they never `postEntry`. Tokens stay on the phone. The shared snapshot only stores who is linked. Mail/Drive/Contacts payloads stay off the snapshot until Auth. Sensitive household actions can step-up through Google after a member is linked.
6. **Kitchen Worker** — Cloudflare Workers + Assets (`hearth-books`). HTML is `Cache-Control: no-store`. `POST /hercules/chat` uses OpenAI or Anthropic when those **Worker secrets** are set, then Cloudflare Workers AI (D-045, D-059). Origin allowlist includes the Git `main-` workers.dev alias. Rate limit is 60 chats/IP/day (`CF-Connecting-IP`), in isolate memory when `HERCULES_RATE` KV is unbound (D-107). Third-party keys are allowed; they must never be `VITE_` or household rows. The model cannot post money, invent notice keys, or treat ledger text as instruction. Local Vite falls back to `localHerculesChat`.
