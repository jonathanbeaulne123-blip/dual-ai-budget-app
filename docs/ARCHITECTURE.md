...
7. **UI** — Home is the September Office (D-051 / D-079 / D-080): **mobile** (`< 720px`) is `OfficePhone` (glance + one-tap, three stamps, five or fewer objects); **wide** is the `.desk-wide` canvas now taking unique offices, sizes, appearance, and personalities. Same commands, colours, fonts, and bubble chrome. Weather window, paper sill, movable widgets, Hercules on furniture. Hercules is Claude's ink-on-paper SVG (D-061). He loafs during Add. Spectacles from a tied rec. Green ink from a closed month. A decorative fly never carries CAD. The chalkboard is drawable; letter detection stays on the phone (D-065). Desk games never post. Layout keys split `phone` / `wide`.
