...
| D-035 | Accepted | The website is Cloudflare Workers + Assets (`hearth-books`). The books stay on Supabase. Netlify is rollback-only until the workers.dev URL is in daily use, then those files can go. Production branch is this rebuild (`cursor/hearth-rebuild-cfde`) because `main` is still Sheets. | Netlify was only serving files. The connected Cloudflare project is Workers Builds, not classic Pages. |
