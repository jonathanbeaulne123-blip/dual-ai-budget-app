...
| D-035 | Accepted | The website is Cloudflare Pages (`hearth-books`). The books stay on Supabase. Netlify is rollback-only until the Pages URL is in daily use, then those files can go. Production branch for Pages is this rebuild (`cursor/hearth-rebuild-cfde`) because `main` is still Sheets. | Netlify was only serving files, and its Functions/Blobs are not a ledger. Cloudflare Pages is the better static/PWA host. |
