...
| D-034 | Accepted | Hosted books are the Jonathan/Bianca Supabase Postgres project (`tykhocwacaxwquhynkok`). The app uses the publishable key and PostgREST. Schema apply needs the secret key or SQL editor — never the database password in the client. The publishable key is not a privacy boundary until Auth exists. | The publishable key can read tables but cannot CREATE TABLE. Netlify Blobs are not the ledger. |
