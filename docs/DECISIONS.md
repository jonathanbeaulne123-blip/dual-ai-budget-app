...
| D-038 | Accepted | Living AI session, prompt, QA, and handoff docs sit in `docs/`. Screen steps name the app, then the tab/page, then the taps. Sheets-era agent prompts remain under `docs/reference/sheets-era/` as history. | After Hearth landed on `main`, the old Sheet-based QA pack would send reviewers to the wrong product. |
