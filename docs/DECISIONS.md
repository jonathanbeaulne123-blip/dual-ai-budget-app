...
| D-034 | Accepted | Hosted books are the Jonathan/Bianca Supabase Postgres project (`tykhocwacaxwquhynkok`, us-east-1). Schema is live in that project as of 2026-08-21. The app uses the publishable key and PostgREST. The API secret cannot CREATE TABLE. Direct `db.*.supabase.co` is IPv6-only. The publishable key is not a privacy boundary until Auth exists. | Publishable key can read/write tables once they exist; it cannot DDL. Netlify Blobs are not the ledger. |
| D-035 | Accepted | The website is Cloudflare Workers + Assets (`hearth-books`). The books stay on Supabase. Netlify is rollback-only until the workers.dev URL is in daily use, then those files can go. Production branch is this rebuild (`cursor/hearth-rebuild-cfde`) because `main` is still Sheets. SPA join URLs use Wrangler `not_found_handling`, not a Netlify `/* /index.html 200` `_redirects` file (Workers API 100324). Builds wipe `dist/` so a cached leftover cannot be uploaded. | Netlify was only serving files. The connected Cloudflare project is Workers Builds, not classic Pages. |
