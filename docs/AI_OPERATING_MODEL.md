...
All AIs must start from `AGENTS.md`, current living files under `docs/`, and verified code. Tool-specific files adapt the work style; they must not copy or redefine Hearth's product law.
...
6. **Prove current behavior.** Run focused tests, `pnpm check`, and relevant visual or trust proof. Never copy stale results from another branch.
