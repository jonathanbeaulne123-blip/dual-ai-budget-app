...
Migration 003 was applied to project `tykhocwacaxwquhynkok` on 2026-08-24 with Jonathan's explicit approval. **Migration 002 is still unapplied** (RPC probe 2026-08-24: `PGRST202`). Jonathan authorized Development apply; the Cloud Agent needs `SUPABASE_DB_PASSWORD` (or a SQL-editor paste of `supabase/migrations/002_snapshot_cas.sql`) to finish. Apply tooling: `pnpm books:apply:002`. Smoke: `pnpm books:smoke:cas`. Until then, live hosted writes still use the legacy racy client path. Membership selectors are not Supabase Auth. No peer device must remain online for a snapshot that has reached the cloud.
