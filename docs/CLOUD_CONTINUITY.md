...
- signed-in accepted writes enter a durable per-device outbox before transport; later offline writes compact into the latest snapshot while keeping the earliest expected hosted revision and all confirmation ids. The **first** hosted write (`expectedRevision === 0`) still calls `hearth_create_household` so the creator is `role='owner'`; command-log append starts only after that membership exists;
