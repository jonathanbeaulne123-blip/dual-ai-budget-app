...
## Implementation status — D-114 continuity, D-117 scopes, D-122 CAS, D-123 repair, D-149 T1 atomic push, D-176 Development-verified access controls
...
- The current Pages workflow bakes `VITE_PRODUCTION_CONTINUITY=1`, so the authenticated membership-scoped Production REST discovery/transport code path is enabled. Production Realtime remains code-refused, and Production identity, recovery, restore, and write runtime smoke is open. This is not Production readiness. Discovery never bulk-scans snapshots, membership INSERT is refused from the client, and shared pushes use Personal projection.
