...
- **D-208/D-211 Development launch policy:** cached books remain readable offline, but every Personal or Shared mutation of a cloud-backed household requires connectivity and Google Auth resolving to the exact locally selected member. An isolated, reusable PGlite staging replica applies the real Postgres schema and double-entry checks before transport; the authenticated atomic cloud acknowledgement is the commit/Saved boundary, and only then does the active PGlite replica advance. The gate follows cloud authority rather than the current ledger presentation; intentionally local-only Development households retain their bounded local behavior and make no automatic hosted request. Startup, ordinary catch-up, ambiguous acknowledgement, and manual restore all read Shared, then the signed-in member's revisioned Personal envelope, then Shared again. Hearth adopts both scopes together and reopens writes only for that exact environment/household/member/revision tuple; a missing or moving Personal generation leaves durable replay markers intact and writes blocked. A definitive retry conflict never uses the legacy auto-merge/republish path: once the stable pair is known, the canonical pair is validated, repaired, verified, and persisted locally before the exact losing outbox/stage generation is compare-cancelled; any local repair failure retains that marker and blocks writes. The person must then review and Confirm again. Manual Retry cannot report Synced until the acknowledged cloud pair is installed. Revision-only Realtime dedupe cannot skip a differing pair. Confirm, canonical pair installation, ledger switching, and local clearing share one serialized lane; an install also binds to the exact live household, scope generation, and outbox generation captured when it is queued. Household, member, or environment switches and destructive local clears invalidate in-flight adoption immediately. A Realtime command invalidates tuple readiness and immediately schedules this paired refresh. A blocked arbitrary projection mismatch validates that same assembled pair in isolation before replacing the disposable active projection. Pending outbox work or unresolved conflicts refuse that restore. Clear-this-phone and Development reset await staged-replica erasure.
...
3. Keep accepted books readable offline on either device, refuse Personal and Shared mutation for cloud-backed households, then reconnect and synchronize a retried Confirm without duplicate posting or silent loss.
