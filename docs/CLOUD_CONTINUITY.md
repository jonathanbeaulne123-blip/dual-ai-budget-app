...
- **D-122 / D-147 client:** `pushSupabaseHousehold` calls `rpc/publish_household_snapshot` first. The pure CAS contract lives in `src/ledger/snapshotCas.ts` and mirrors the SQL packet. When PostgREST reports the RPC missing (`PGRST202`), Auth/continuity paths **refuse** the legacy GET-then-compare-then-POST race (D-147). That legacy path remains only for explicit Auth-off `legacyLinkedPublish` recovery without continuity/Auth.
