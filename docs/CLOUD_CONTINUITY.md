...
- **D-208 Development launch policy:** cached books remain readable offline, but shared writes require connectivity and matching Google Auth. An isolated, reusable PGlite staging replica applies the real Postgres schema and double-entry checks before transport; the authenticated atomic cloud acknowledgement is the shared commit/Saved boundary, and only then does the active PGlite replica advance. An ambiguous response retains its slim idempotency marker plus the isolated staged candidate until authenticated retry/pull proves the exact receipt; Hearth never calls that outcome “nothing posted.” If that receipt is found inside a newer cloud revision, Hearth reads Shared, then the signed-in member's revisioned Personal envelope, then Shared again and reopens writes only when the cloud generation stayed stable. A blocked arbitrary projection mismatch uses the same stable paired read, validates the assembled snapshot in isolation, then replaces the disposable active projection. Pending outbox work or unresolved conflicts refuse that restore. Clear-this-phone and Development reset await staged-replica erasure.
