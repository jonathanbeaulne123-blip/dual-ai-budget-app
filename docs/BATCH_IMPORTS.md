...
2. Hearth parses bank files on the device. A selected image is sent once to `POST /documents/scan` for structured detection. Receipts try the bound Workers AI vision model first. **Shift-report / tip-sheet scans prefer paid OpenAI `gpt-4o` (then Anthropic) when `DOCUMENT_SCAN_ALLOW_PAID=true`**, because free vision often returns a wrong-but-valid shape; a deterministic Toast Employee Shift Report parser then re-reads labeled totals from the OCR transcript. Chat paid providers stay gated by `HERCULES_ALLOW_PAID_PROVIDERS`.
