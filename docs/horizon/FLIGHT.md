...
**Where the run starts (current behaviour, measured on `horizon-geo-1`).** `sky.launchPads[].edge` is a 6 m east–west line through the pad's marker, not its lip, so the sim (`wing.ts launchFromPad`) finds both: the outward heading is the edge's normal on the side the rider faces if the ground there falls ≥ 10 m within 20 m, else the side that falls more; the run starts three steps behind the first point (≤ 8 m past the edge line) where the ground drops ≥ 0.75 m. The Prow's deck drops 43 m on both sides (the rider's choice); the Lamp gallery runs off over the sea. **The Crown's south shoulder falls only ~1.4 m in 20 m, so the Crown always runs off north** (its north face falls 14.5 m) whichever way the rider stands — a south-facing Crown launch (the Dam Run, the ridge) turns 180° after the lip. Whether the south lip should be graded steeper is a land question for pass 3 (`HANDOFF-notes/glider.md`).
...
- **Ridge** (`ridge.crownSouth`, the box on the south face, h 80–280): +2.0 m/s while the wind is from the south (`wind.dir` within 45° of south) and the wing's heading has a component *along or into* the face (|heading − east/west| ≤ 60°, i.e. beating along the ridge, not flying away from it); full strength below h 220, fading to 0 at h 280. Ridge lift is worked by beating back and forth along the face; that is the intended way to bank height for the Throat. Measured (`workRidge`, `test/horizonGliderJourneys.test.ts`): from the Crown launch to 190 m takes **≈ 126 s** of beating in the 4 m/s south wind, and the Throat Run from there arrives **+2.6 m** above the mouth's floor — inside the aperture, not the ≥ 35 m first estimated: the ridge sits 230 m south of the launch and the tailwind run north, the turn's drift and the last leg into the wind spend most of the gain.
...
- **Earned, not given** (ruled 26 Sep; measured by the scripted Throat Run): straight from the Crown in still air the wing reaches the mouth's plane **5 m under** the aperture floor (−5.2 m: a miss); after working the ridge to 190 m in the south wind it enters with **+2.6 m** in hand; from 190 m over the Crown in still air it arrives with **+24.8 m** (6.8 m above the aperture's top — height to spend in a dive). The bar for the tests: still air ≥ −10 m at the plane; after the ridge ≥ 0 and inside; from 190 m ≥ 15 m.
...
- **Throat Run** (Crown → gate 10 → the turn → gate 12 → the Deep): see §0's last row; the tactic is *ridge first* (≈ 126 s on the ridge to 190 m, then +2.6 m at the mouth).
