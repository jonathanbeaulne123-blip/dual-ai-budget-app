...
Version 1.1 · 25 September 2026 · Owner: Jonathan (product) · Author: Claude (design lead, review)
...
7. The exhaustive gate (`pnpm test -- --risk=high --focus=... --focus-reason=...`) only on the final SHA, within its five-minute budget.
...
- `MANIFEST.json` is generated by `make_manifest.py` (kept beside it). Change the numbers in the script and regenerate; never hand-edit the JSON. Version 1.3 has passed two independent geometry reviews (every line-against-line intersection registered or explained; plots, cables, doors and quays checked against the coast and the bands).
...
- `DECISIONS.md` is the live list; D11–D15 and D17 are open at the time of writing and are named in the pass that they block.
