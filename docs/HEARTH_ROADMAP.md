...
- [ ] Deploy D-139 Hercules Pro living companion after review. The cross-origin load repair uses a versioned MCP resource, exact public-asset CORS, and bounded static fallback; verify picture-in-picture and inline fallback in the connected ChatGPT app, reduced-motion/Pause, model fallback, and that the summon payload contains no ledger credentials or raw rows. This uses the existing Worker Assets binding and needs no schema migration.
