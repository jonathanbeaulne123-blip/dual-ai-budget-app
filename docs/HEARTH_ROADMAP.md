...
- Only a visible user **Confirm** may post a command, except D-159's separately opted-in deterministic 7shifts authority for exact server-proven member/job evidence. Reversal/repost corrects mistakes; financial history is not silently rewritten.
...
| Two phones | Live pull (4 s REST poll when Realtime unavailable) + Shared CAS + slim outbox + disjoint shared absorb ship on Development; **Auth paths use Migration 012 atomic Shared+Personal** (D-149 T1-S1/S2); **Realtime primary path live on Dev** — Migration 014 applied + `VITE_CONTINUITY_REALTIME=1` (#175); two-phone smoke passed 2026-08-27. T2 command-log landed (#186) with `VITE_CONTINUITY_COMMAND_LOG=1`. | **ACTIVE:** G6 in-memory 012 proof still open; Production Realtime off. | G6 harness merge; live command-log two-phone smoke; Production remains gated. |
...
| Hearth Household Fund | D-161 is **RELEASE APPROVED in PR #237**, reconciled from exact `efbe5ed` onto current `main`: append-only Fund events, clearing/refunds/deficits, private savings scope, PGlite projection, command-log replay, Home glance, and Fund books pane are complete. September remains manual practice at $0 opening; no real money or provider action. | Run September as a controlled weekly rehearsal. D-162 October code remains a disabled read-only matcher/activation gate only. | Exact financial scenarios, private-envelope/AI/export denial, two-device replay, full checks, accessibility/responsive smoke, reviewed exact SHA, and a separate October Release approval. |
| Shared vs Personal ledger experience | D-164 audit found that the named ledger switch changes some scoped figures but leaves the same Home room, Books hierarchy, and largely identical route behavior; Calendar/Shift lack a complete mode contract. The Fund is financially correct but presented as a detached Home card plus form-first Books pane. | **NEXT:** implement Layer 1 ledger-purpose projectors and distinct Shared Story/Personal Folio experiences, then Layer 2 desktop+iPad composition. Keep iPhone OfficePhone structurally unchanged. | Route-by-route privacy denial; canonical Fund-flow arithmetic; Shared/Personal visual difference; 768/820/1024/1280/1440 proof; 320/390 phone fence; independent UX and trust review. |
...
|---|---|---|---|
| [2026-08-28 Shared Ledger story implementation](worksessions/2026-08-28-shared-ledger-story-implementation.md) | **OPEN; IMPLEMENTING D-164** | Mode-safe projectors, Shared Story / Personal Folio at `>=720px`, route contracts, phone banner only | Branch `cursor/shared-ledger-story-aef7` from `main@871e660`; no schema, provider, secret, merge, or deploy |
...
**D-114/D-117/D-147/D-149 progress:** optional Publish/`linked` client semantics are demoted; automatic continuity after Google membership is the ordinary path. Identity-tuple and hash-acceptance are checked under D-146. **T1-S1/S2 atomic Personal+Shared (Migration 012 + Auth client push) shipped on branch; merge pending.** Remaining Phase 0 owner actions: live Hercules KV bind + deploy, GitHub ruleset apply. Remaining engineering: two-device E2E and Tier 1 Realtime proof (T1-S3+).
...
- [ ] Ship the four-part [Onboarding Update](ONBOARDING_UPDATE.md): comprehensive control review → [phone/desktop animation and interaction storyboards](ONBOARDING_PART2_STORYBOARD.md) → member-scoped modular engine → integrated Hercules experience. Its [opening-truth component](worksessions/2026-08-25-opening-truth.md) uses a Toronto as-of date, balanced Opening equity, and no fabricated income/history; D-129 locks its three product choices.
...
- [ ] Run the four-week D-161 September Household Fund rehearsal: setup/private savings/opening tie; mixed-card and partial clearing; refund/shortfall/recurring reserve/correction; final tie/Kitty rollover/month close/audit. Record friction as additive follow-up work; do not silently change Fund rules.
...
**Latest approved UX packet (2026-08-28):** D-164 Shared Ledger story architecture is the next coherent presentation slice for the D-161 September rehearsal. Start with the mode-safe projector/route proof before pixels. Use [`briefs/CURSOR_SHARED_LEDGER_STORY_HANDOFF_2026-08-28.md`](briefs/CURSOR_SHARED_LEDGER_STORY_HANDOFF_2026-08-28.md); do not infer iPhone redesign, financial-command changes, provider activation, push, or release authority from this roadmap entry.
