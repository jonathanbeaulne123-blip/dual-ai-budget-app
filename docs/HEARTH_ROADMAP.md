...
| Shared vs Personal ledger experience | D-164 implemented on [#244](https://github.com/jonathanbeaulne123-blip/dual-ai-budget-app/pull/244) @ `ed708dc`: Shared is one pool plus Kitty Banks; Personal Books is the household-visible + own-room floor; Home seals are posted in/out/leftover spend. Sit-down leftover stays separate. Concurrent `main` owns D-165–D-172 Shift/Evidence/boot. | **NEXT:** Codex integration packet — land `#244` onto current `main` without restyling. Kitchen-desk law becomes D-173 so `main` D-165 stays Evidence queue. | Unique decision IDs; leftover-spend ≠ leftoverProjection; Personal floor ≠ scopedHousehold; Shift envelopes placed in paper Shift room; `pnpm check` on the integrated SHA; independent books/privacy/UX/trust. |
