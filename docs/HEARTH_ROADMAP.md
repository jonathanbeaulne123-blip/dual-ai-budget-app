...
|---|---|---|---|
| Plan System V2 | D-239 and D-241 add scope-safe monthly Plan records, private Household prework, immutable proposals/digests, complete reflection/Bridge editors, trusted Shared Hercules replies, Toronto-timed durable activation, compatibility budget projection, deterministic Plan tools and the new PlanStudio behind a reversible flag. Plan actions do not post money. | **DEVELOPMENT RELEASE AUTHORIZED:** the Development workflow defaults `VITE_PLAN_SYSTEM_V2` to `1`; a manual workflow run with `0` restores the legacy presentation without deleting Plan records. Production continuity remains off. | Exact release-head gate and deploy; then authenticated two-browser Shared Sitdown, Hercules and acknowledgement proof, deployed sleeping-client alarm observation, event compaction/restore corpus and physical accessibility/product review. |
