...
- [x] Deploy D-139 Hercules Pro living companion to the Development Worker after review. Live OAuth discovery, anonymous MCP rejection, and byte-identical companion JavaScript/model assets were verified on 2026-08-26. Picture-in-picture, inline fallback, reduced-motion/Pause, and model fallback still need a connected-ChatGPT visual smoke; the summon payload contract contains no ledger credentials or raw rows. This uses the existing Worker Assets binding and required no schema migration.
