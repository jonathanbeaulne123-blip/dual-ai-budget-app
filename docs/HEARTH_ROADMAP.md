...
| Onboarding / first-number utility | Mobile/Office, Accounts, Audit, appointments, sitdown/vault, Hercules, budget foundations, and D-127 work flow have shipped, but there is no Google-member-scoped guided setup or opening-truth command. Current first-run lessons are device-local and flat. | **ACTIVE Slice A (D-153):** foundation coordinator behind `VITE_ONBOARDING_FOUNDATION`; D-128/D-129 still govern Practice + motion. Full scenes / opening truth = Slice B+. | Automatic/Skip first Google-member entry; D-128 isolation; exact target/route/dialogue proofs; balanced opening truth; replayable member-scoped progress; both members see accepted results from independent devices. D-129's deliberate reduced-motion exception remains named accessibility debt. |
...
|---|---|---|---|
| [2026-08-27 Onboarding Slice A foundation](worksessions/2026-08-27-onboarding-foundation-slice-a.md) | **ACTIVE; PR for GPT review** | D-128/D-129 coordinator, targets, Practice, progress, Skip/Replay flag | D-153; no migration |
