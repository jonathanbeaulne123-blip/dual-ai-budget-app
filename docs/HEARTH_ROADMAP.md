...
|---|---|---|---|
| Local books | Confirm writes pass `acceptHouseholdWrite`: validate → PGlite → persist → transport. Pulled Development snapshots pass PGlite acceptance before becoming active, and conflicts keep both sides. Restore failure after successful ingest remains honestly `recovery-available`, not proven rollback. | **ACTIVE:** keep the local command boundary closed while hosted authority and two-device interleavings are built. | Failure and same-count/different-facts tests stay green; recovery and conflict outcomes never celebrate an uncertain write. |
| Google-account cloud continuity | Development has exact-subject discovery, automatic accepted-command transport, a durable compacting outbox, multi-household switching, and member-personal replicas (D-114 / D-117). Legacy phrase/`linked` paths remain, and hosted publication is not yet atomic. | **ACTIVE product gap:** finish authoritative CAS, acknowledgement/backoff, and two-device convergence; no device is the host. | A fresh signed-in device reads/writes both scopes while the old device is off; offline/outbox and two-device tests prove convergence. |
...
| AI disclosure | Merged PR #83 introduced the member projection; independent review then reproduced and closed a full-household grounded P&L/FIGURES leak. Briefing, notices, ledger, memories, grounded copy, and allowed figures rebuild at the outbound boundary (D-115); delayed replies remain bound to the originating request/environment/household/member (D-116). | **SHIPPED after review correction;** keep aggregate and delayed-response canaries green. | Outbound canaries exclude other-member personal notes, direct amounts, and derived aggregates; stale responses cannot display or persist after a context switch. |
...
| Onboarding / first-number utility | Mobile/Office, Accounts, Audit, appointments, sitdown/vault, Hercules, budget foundations, and D-127 work flow have shipped, but there is no Google-member-scoped guided setup or opening-truth command. Current first-run lessons are device-local and flat. | **ACTIVE planning:** the four-part Onboarding Update teaches the real product, establishes truthful starting balances, and prepares Bianca for September without touching Cursor's migration lane. | Play/Skip on first Google household entry; real/practice scenario separation; fresh-profile phone + desktop journey; balanced opening truth; replayable member-scoped progress; both members see accepted results from independent devices. |
...
- [ ] Bind the live Hercules KV/stronger authority and prove concurrent failure semantics. PR #79's D-121 code containment is merged; the reliable production cap is not.
...
**D-114/D-117 progress:** the Development household path no longer requires Publish/`linked` for a matching signed-in Google member; the outbox, pulled-snapshot acceptance, explicit hosted membership, and Personal scope are live. The checkboxes stay open until atomic hosted authority and end-to-end convergence are complete.
...
- [ ] Ship the four-part [Onboarding Update](ONBOARDING_UPDATE.md): comprehensive control review → phone/desktop animation and interaction storyboards → member-scoped modular engine → integrated Hercules experience. Its [opening-truth component](worksessions/2026-08-25-opening-truth.md) uses a Toronto as-of date, balanced Opening equity, and no fabricated income/history; its remaining money-command product locks still require approval.
