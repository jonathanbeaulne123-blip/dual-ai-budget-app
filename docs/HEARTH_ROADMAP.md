...
| Google-account cloud continuity | Development has exact-subject discovery, automatic accepted-command transport, a durable compacting outbox, multi-household switching, member-personal replicas, atomic 012, Realtime 014, command log 013, and session/device authority 017. Two-phone Realtime met **≤500 ms p95** historically; 4 s poll is fallback. | **D-180 PILOT + D-182 ENTRY REPAIR DEPLOYED; LIVE GATE OPEN:** command events are normal, true same-fact collisions require review, Production continuity stays off, and Google entry is chooser-first with honest edit freshness. No device is the host. | Complete the live matrix with 100 new samples and the fourteen-day Development rehearsal; Production remains separate. |
...
|---|---|---|---|
| [2026-08-31 Bianca Month mainline catch-up](worksessions/2026-08-31-bianca-month-mainline.md) | **LOCAL CATCH-UP; VERIFYING** | D-182 current App/books/command-sync trial contract | Full-App and two-device command-event regressions; no push, merge, deploy, hosted mutation, or Production |
...
- **Implemented in D-114/D-117/D-118/D-122/D-149/D-176/D-182:** exact Development Google membership discovery, PGlite acceptance, durable outbox with ack/backoff, launch/focus/reconnect replay, atomic Shared+Personal publication, command events, Realtime, session/device authority, member-keyed open shifts, explicit conflict review, and chooser-first Google entry with honest household freshness. The deployed D-180/D-182 pilot workflow bakes `VITE_PRODUCTION_CONTINUITY=0`; D-182 adds no cloud contract.
...
- [ ] Ship the four-part [Onboarding Update](ONBOARDING_UPDATE.md): comprehensive control review → [phone/desktop animation and interaction storyboards](ONBOARDING_PART2_STORYBOARD.md) → member-scoped modular engine → integrated Hercules experience. D-182 now supplies the Development-only opening-truth command and Month-One gate: one Toronto as-of batch, balanced Opening equity, complete reversal, no fabricated income/history, and no second opening after ordinary money. Broader onboarding remains additive and must not bypass this proof.
