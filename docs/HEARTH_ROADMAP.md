...
| Shared vs Personal ledger experience | D-164 audit found that the named ledger switch changes some scoped figures but leaves the same Home room, Books hierarchy, and largely identical route behavior; Calendar/Shift lack a complete mode contract. The Fund is financially correct but presented as a detached Home card plus form-first Books pane. | **IN REVIEW:** draft PR [#244](https://github.com/jonathanbeaulne123-blip/dual-ai-budget-app/pull/244) on `cursor/shared-ledger-story-aef7` implements Layer 1 projectors and Layer 2 desktop+iPad story/folio; iPhone OfficePhone stays structurally unchanged. Not merged, not deployed. | Route-by-route privacy denial; canonical Fund-flow arithmetic; Shared/Personal visual difference; 768/1280/390/320 proof on the PR; independent UX and trust review before merge. |
