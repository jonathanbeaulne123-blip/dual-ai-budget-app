...
| Two phones | Live pull (4 s REST poll when Realtime unavailable) + Shared CAS + slim outbox + disjoint shared absorb ship on Development; **Auth paths use Migration 012 atomic Shared+Personal** (D-149 T1-S1/S2); **Realtime primary path live on Dev** — Migration 014 applied + `VITE_CONTINUITY_REALTIME=1` (#175, 2026-08-26). **100–500 ms push-native target** in [`SYNC_ARCHITECTURE.md`](SYNC_ARCHITECTURE.md). | **ACTIVE:** two-browser Realtime proof (T1-S5) before claiming seamless dual-use. | Tier 1 gates G1–G6; p95 partner visibility ≤ 500 ms; interleaving harness before Production claim. |
...
**D-114/D-117/D-147/D-149 progress:** optional Publish/`linked` client semantics are demoted; automatic continuity after Google membership is the ordinary path. Identity-tuple and hash-acceptance are checked under D-146. **T1-S1/S2/S3 merged** (012 atomic push + Realtime flag + 014 publication applied Dev 2026-08-26). Remaining Phase 0 owner actions: live Hercules KV bind + deploy, GitHub ruleset apply. Remaining engineering: two-device Realtime smoke + T1-S4/S5/S6; Tier 2 stack gated on Tier 1 proof.
...
**Progress:** T1-S1/T1-S2/T1-S3 merged; Migration 014 applied Dev. **Next engineering slice:** T1-S4 push/pull coordinator + T1-S5 two-browser proof.
