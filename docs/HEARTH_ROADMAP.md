...
| Local books | Confirm writes pass `acceptHouseholdWrite`: validate → PGlite → verify canonical hash → persist → transport. Pulled Development candidates reconcile record by record, then pass PGlite acceptance before becoming active. Restore failure after successful ingest remains honestly `recovery-available`, not proven rollback. | **ACTIVE:** keep the local command boundary closed while hosted authority and two-device interleavings are built. | Failure and same-count/different-facts tests stay green; recovery never celebrates an uncertain write. |
| Google-account cloud continuity | Development has exact-subject discovery, automatic accepted-command transport, a durable compacting outbox, multi-household switching, member-personal replicas, atomic 012, Realtime 014, command log 013, and session/device authority 017. Two-phone Realtime met **≤500 ms p95** historically; 4 s poll is fallback. | **D-186 AUTOMATIC RECONCILIATION RELEASE AUTHORIZED:** command events are normal, distinct ids remain additive, later same-id entries win without a chooser, Production continuity stays off, and Google entry remains chooser-first with honest edit freshness. No device is the host. | Complete the current-main Development release, then the live matrix with 100 new samples plus the fourteen-day Development rehearsal; Production remains separate. |
...
| Hercules full synthetic brain | D-187 makes Gemini-first planning and voice the conversational brain for authorized synthetic Development. Every voice tier receives the credential-free full synthetic snapshot; typed read tools calculate complex answers and keep figures/source cards authoritative. The exact plural `bills` failure now has a deterministic read-plan fallback. | **RELEASE AUTHORIZED; REBASING:** 71 focused tests, TypeScript, AI-surface, equivalent production build, diff hygiene, and independent High-risk review passed on the pre-rebase candidate. | Reverify the exact rebased head, then fast-forward `main` and verify GitHub/Cloudflare. Production and write authority remain closed. |
...
| [2026-08-31 Hercules full brain](worksessions/2026-08-31-hercules-full-brain.md) | **REOPENED; RELEASE AUTHORIZED** | Full credential-free synthetic Development context; Gemini-first planning/voice; deterministic calculator/source tools | D-187; exact rebased-head proof pending; no schema or Production change |
