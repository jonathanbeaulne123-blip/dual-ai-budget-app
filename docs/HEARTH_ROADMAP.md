**Plan System V2 — local gated implementation (2026-09-11):** Personal and Household planning now share versioned four-lens contracts, private drafts, exact two-person Household acknowledgement, Bridge disclosures, PGlite/event continuity, deterministic Hercules reads/actions and a new twelve-combination PlanStudio. The flag remains off; authenticated two-browser continuity, full replay/restore acceptance, shared Hercules-response authority, nudge controls, and product acceptance remain release gates. [Worksession](worksessions/2026-09-11-plan-system-rebuild.md).
...
|---|---|---|---|
| Plan System V2 | D-239 adds scope-safe monthly Plan records, private Household prework, immutable proposals/digests, self-owned acknowledgements, compatibility budget projection, deterministic Plan tools and the new PlanStudio behind an off-by-default flag. Plan actions do not post money. | **ACTIVE — LOCAL HIGH QUICK GATE + BUILD + 12-CASE BROWSER MATRIX:** keep `VITE_PLAN_SYSTEM_V2` off. The legacy Plan remains the rollback presentation. | Controlled server-authored Shared Hercules replies; complete reflection/nudge/Bridge-state UX; event compaction/restore corpus; authenticated two-browser Shared Sitdown and acknowledgement proof; product review; exact release-head full acceptance when separately authorized. |
