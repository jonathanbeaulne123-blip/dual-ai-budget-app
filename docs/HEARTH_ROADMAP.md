...
|---|---|---|---|
| Priority-one Shared Money program | D-174 locks the direction: partner-backed Canadian joint account, Jonathan and Bianca first, shared bills/goals plus private accounts, equal co-owner routine authority with guardrails. No banking capability ships from the decision. | **ACTIVE — SF-01 BASELINE RECONCILED:** consume [`SHARED_MONEY_BASELINE.md`](SHARED_MONEY_BASELINE.md) and execute the remaining Phase 0 packets in [`briefs/shared-money/`](briefs/shared-money/README.md). D-172, D-161, and D-162 remain unchanged. | SF-02–SF-05 pass; then each later phase earns its own packet, evidence, and Release gate. |
