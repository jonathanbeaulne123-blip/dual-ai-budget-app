...
- [x] Validate environment + Google identity + personal/household membership tuple on every discovery/join/pass/pull/persist boundary. *(D-145: Google subject/member required when continuity identity is present; phrase/Pass remain recovery without Google; #121/#126/#127 env/invite bind.)*
...
**D-114/D-117 progress:** the Development household path no longer requires Publish/`linked` for a matching signed-in Google member; the outbox, pulled-snapshot acceptance, explicit hosted membership, and Personal scope are live. Continuity E2E/CAS convergence and optional-publish removal remain open; identity-tuple and hash-acceptance Phase 0 boxes are checked under D-145.
