...
| Hearth Household Fund | D-160 is **ACTIVE on `codex/household-fund` from exact `efbe5ed`**: append-only Fund events, clearing/refunds/deficits, private savings scope, PGlite projection, command-log replay, Home glance, and Fund books pane are implemented locally. September remains manual practice at $0 opening; no real money or provider action. | Finish High-risk independent review and use September as a controlled weekly rehearsal. D-161 October code is a disabled read-only matcher/activation gate only. | Exact financial scenarios, private-envelope/AI/export denial, two-device replay, full checks, accessibility/responsive smoke, review disposition, and Jonathan approval before push. |
...
|---|---|---|---|
| [2026-08-28 Household Fund](worksessions/2026-08-28-household-fund.md) | **ACTIVE; LOCAL ONLY** | September zero-balance practice Fund plus disabled October read-only evidence boundary (D-160/D-161) | Exact `efbe5ed` worktree; no hosted schema, provider connection, secret, deploy, Production action, or push |
...
- [ ] Complete D-160 High-risk review and run the four-week September Household Fund rehearsal: setup/private savings/opening tie; mixed-card and partial clearing; refund/shortfall/recurring reserve/correction; final tie/Kitty rollover/month close/audit. Record friction as additive follow-up work; do not silently change Fund rules.
- [ ] After the rehearsal and October security gates, run D-161 as a separate Release packet: verify Flinks savings support, exact/competing/near/grouped/unmatched evidence, revoke/privacy denial, and carry-forward history. If live support fails, stop for provider approval; statement import is test evidence only.
