...
| Google-account cloud continuity | Development has exact-subject discovery, automatic accepted-command transport, a durable compacting outbox, multi-household switching, member-personal replicas, atomic 012, Realtime 014, command log 013, and session/device authority 017. Two-phone Realtime met **≤500 ms p95** historically; 4 s poll is fallback. | **D-180 PILOT DEPLOYED; LIVE GATE OPEN. D-182 ENTRY REPAIR LOCAL:** command events are normal, true same-fact collisions require review, Production continuity stays off, and the candidate makes Google entry chooser-first with honest edit freshness. No device is the host. | Independently review and release D-182, then complete the live matrix with 100 new samples and the fourteen-day Development rehearsal; Production remains separate. |
...
| [2026-08-31 Google-first household entry](worksessions/2026-08-31-google-first-household-entry.md) | **LOCAL IMPLEMENTATION; RELEASE REVIEW PENDING** | D-182 Google-first chooser, invitation recovery, household freshness, and active identity header | [`SYNC_PILOT.md`](SYNC_PILOT.md); no schema, hosted-data mutation, Production action, push, merge, or deploy |
...
**Progress:** T1-S1…T1-S6 and T2 command-log are merged, and D-180's Development-only pilot is deployed but not daily-use proven. D-182's Google-first entry/chooser repair is local. **Next continuity work:** independent D-182 release review, separately approved push/merge/deploy, the complete two-account matrix with 100 new latency samples, and the fourteen-day rehearsal. Production remains a separately approved packet.
