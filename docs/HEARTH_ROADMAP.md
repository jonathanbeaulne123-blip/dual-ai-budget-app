...
|---|---|---|---|
| Plan System V2 | D-239 and D-241 add scope-safe monthly Plan records, private Household prework, immutable proposals/digests, complete reflection/Bridge editors, trusted Shared Hercules replies, Toronto-timed durable activation, compatibility budget projection, deterministic Plan tools and the new PlanStudio behind an off-by-default flag. Plan actions do not post money. | **ACTIVE — LOCAL QUICK GATE + REAL SQLITE AUTHORITY + BUILD + 12-CASE BROWSER MATRIX:** keep `VITE_PLAN_SYSTEM_V2` off. The legacy Plan remains the rollback presentation. | Event compaction/restore corpus; authenticated two-browser Shared Sitdown, Hercules and acknowledgement proof; deployed sleeping-client alarm observation; physical accessibility/product review; exact release-head full acceptance when separately authorized. |
