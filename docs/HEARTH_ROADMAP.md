...
| Google-account cloud continuity | Development has exact-subject discovery, automatic accepted-command transport, a durable compacting outbox, multi-household switching, member-personal replicas, atomic 012, Realtime 014, command log 013, and session/device authority 017. Two-phone Realtime met **≤500 ms p95** historically; 4 s poll is fallback. | **D-180 PILOT IMPLEMENTED LOCALLY; LIVE GATE OPEN:** command events are normal, true same-fact collisions require review, and the proposed pilot build refuses Production continuity. No device is the host. | Approved deploy, complete live matrix with 100 new samples, then fourteen-day Development rehearsal; Production remains separate. |
...
|---|---|---|---|
| [2026-08-31 Ledger-native sync pilot](worksessions/2026-08-31-google-docs-sync-pilot.md) | **LOCAL IMPLEMENTATION; LIVE PILOT PENDING** | D-180 Development-only Jonathan/Bianca command-sync and recovery rehearsal | [`SYNC_PILOT.md`](SYNC_PILOT.md); no schema, hosted-data mutation, Production action, push, merge, or deploy |
...
- **Implemented in D-114/D-117/D-118/D-122/D-149/D-176:** exact Development Google membership discovery, PGlite acceptance, durable outbox with ack/backoff, launch/focus/reconnect replay, atomic Shared+Personal publication, command events, Realtime, session/device authority, member-keyed open shifts, and explicit conflict review. The D-180 pilot workflow bakes `VITE_PRODUCTION_CONTINUITY=0`; it is local and not deployed.
...
**Progress:** T1-S1…T1-S6 and T2 command-log are merged. D-180's Development-only pilot implementation is local; it is not pushed, deployed, or daily-use proven. **Next continuity work:** independent release review, an approved pilot deploy, the complete two-account matrix with 100 new latency samples, and the fourteen-day rehearsal. Production remains a separately approved packet.
