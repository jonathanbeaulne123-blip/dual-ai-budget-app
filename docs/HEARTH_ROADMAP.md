...
**D-114/D-117/D-147 progress:** optional Publish/`linked` client semantics are demoted; automatic continuity after Google membership is the ordinary path. Identity-tuple and hash-acceptance are checked under D-146. Remaining Phase 0 owner actions: live Hercules KV bind + deploy, GitHub ruleset apply. Remaining engineering: Personal+Shared single hosted TX and two-device E2E.
