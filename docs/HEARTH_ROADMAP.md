...
| AI disclosure | `householdForAiDisclosure` + `composeHerculesChatRequest` rebuild briefing, notices, ledger, and memories for the signed-in member. Partner `personal` rows and partner memories stay off the Worker payload (D-115). Quiet redaction and grounded FIGURES remain. | **SHIPPED** on the disclosure-projection branch; keep canaries green on merge. | Outbound canaries exclude other-member personal amounts/notes; Worker receives only the projection. |
