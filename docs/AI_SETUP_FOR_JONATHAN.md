...
## A. Merge the two pull requests in order

The AI setup is stacked on the roadmap pull request. The roadmap must land first.

1. Open the repository in your web browser.
2. Click **Pull requests** near the top.
3. Open pull request **#64**.
4. Look for the checks near the bottom.
5. If any check is red, stop and ask Codex to fix it.
6. If every required check is green, click **Ready for review** if GitHub shows that button.
7. Read the file list or ask Codex for a final review.
8. Click **Merge pull request** only when you are satisfied.
9. Click **Confirm merge**.
10. Go back to **Pull requests**.
11. Open the AI customization pull request linked from `docs/worksessions/2026-08-24-ai-customization.md`.
12. Repeat steps 4 through 9.

If GitHub says the second pull request has conflicts after #64 is merged, do not click random conflict buttons. Ask Codex to rebase it.
