...
| **Hercules Pro** | Optional ChatGPT reasoning calls the same member-scoped, read-only calculations; Hearth still owns every write and Confirm. | Jonathan/Bianca get a longer-form Hercules teacher in ChatGPT; family testers keep a complete free cat in Hearth. | **Implemented locally (D-136).** Deploy/register later. Development only until security cutover. |
