...
- September Office (D-051 / **D-079** / **D-080** / **D-082** warmth / **D-151**): one kernel, three views. **Phone** (`< 720px`) is frozen `OfficePhone` Draft C. **Tablet** (`720–1279`) scales that same board. **Computer** (`≥ 1280`) is the night-cabin room ([COMPUTER_OFFICE.md](COMPUTER_OFFICE.md)). Historical two-branch prompts: [CLAUDE_DESKTOP_OFFICE.md](CLAUDE_DESKTOP_OFFICE.md), [CLAUDE_MOBILE_SHELL.md](CLAUDE_MOBILE_SHELL.md). Spec: [OFFICE.md](OFFICE.md).
...
| **Hercules Pro** | Optional ChatGPT reasoning calls member-scoped reads. D-137 optionally adds member-scoped, preview-then-confirm transaction posting with separate OAuth scope and atomic cloud receipts. | Jonathan/Bianca get a longer-form Hercules teacher and may opt into direct confirmed posting; family testers keep a complete free cat in Hearth. | **Reads deployed (D-136); confirmed writes implemented, Migration 011/deploy pending (D-137).** Development only until security cutover. |
...
| **September Office** | Posting, wallet, sit-down, bills, Health reachable without leaving Home unless you dive. | Phone: five objects, not seventeen rows. Tablet: same board scaled. Computer: night-cabin room, personalities, Play. | **Direction (D-051), split (D-079), warmth (D-082), three views (D-151).** Mobile Home is `OfficePhone`. Widgets never post. |
