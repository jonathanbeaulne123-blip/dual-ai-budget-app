...
### Current measured startup and activity work

PR #366 removes the legacy SQL-ingestion wait from already validated v2 adoption. The actual selected **onboarding test Household** now opens at Live revision 13 without the journal-validation banner. Two deployed desktop reloads recorded canonical adoption at **1540.2 ms and 1860.3 ms** (N=2). These are not paint/interactive measurements and both exceed the 500 ms goal; browser visibility distorted PaintTiming, so those paint readings were rejected.

The D-237 activity repair bounds the initial transaction DOM to 50 rows with full-list search and explicit expansion, computes duplicate comparison pairs only when review is open, and removes redundant v2 Personal reassembly. It preserves the duplicate scorer and all financial acceptance checks. Command UUID, phase and row DOM attributes make ordinary-App paint observation possible. No timer is inferred from an aggregate total, an event receipt, or a hidden row.

The full accounting guard still blocks acceptance. No incremental 500-command equivalence proof or safe optimistic pending-row overlay has been implemented. Add currently waits for the accepted receipt. Neither the 16 ms author budget nor 120 ms ACK budget is claimed. A local diagnostic with 5,056 rows and two concurrent ordinary Add confirmations measured wire ACKs at **729.4 ms and 2674.9 ms**, and author row paint upper bounds at **3013.6 ms and 4819.3 ms**. One partner paint was **4773.5 ms**, the other was unobserved; visible Saved was unobserved. This failed trial is local Vite/headless Chrome evidence, not a deployed baseline or physical LTE proof.
