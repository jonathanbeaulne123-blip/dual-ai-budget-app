...
Legacy hosted tables still use open RLS (`USING (true) WITH CHECK (true)` for ALL, including DELETE). The bundled publishable key can `GET`/`POST`/`PATCH`/`DELETE` those existing rows; the three-word phrase is not an API control. Migration 003 is applied: its new membership/personal tables are limited to Development by RLS and grant only `SELECT`/`INSERT`/`UPDATE`, but the browser-supplied Google subject remains a selector, not authentication. Through 2026-09-30 Development rows are disposable. Before meaningful October data, ship Google Auth, deny-by-default membership RLS, and the reviewed cutover. Migration 002 (D-122 `publish_household_snapshot`) is **applied to Development** (2026-08-25); additive Auth preparation 004 and forward CAS hardening 005 were applied with Development approval on 2026-08-24. The approved cleanup removed all 30 Development households and left the single Production household untouched. Deny-by-default cutover 006 is not applied. Since its grants and policies are project-wide, a Development-only 006 rehearsal requires a separate Supabase project; the `environment` column is not a schema boundary.
...
The current household snapshot stores **who is linked** (email, Google subject, granted scopes), not the token. D-114 temporarily discovers Development membership from those embedded links and keeps Production discovery off. D-117's migration 003 is applied and supplies durable identity-to-personal-ledger and identity-to-household membership outside the ledger snapshot. The repaired D-123 client can instead use a refreshed Supabase user JWT behind `VITE_SUPABASE_AUTH_ENABLED=1`; preparation/hardening 004/005 are live, while the project-wide 006 cutover remains unapplied. Default suite services are sign-in and Calendar; extra suite access remains opt-in.
