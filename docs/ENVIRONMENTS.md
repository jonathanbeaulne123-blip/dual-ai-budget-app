...
RLS is still open (`USING (true) WITH CHECK (true)` for ALL, including DELETE) on the current live schema. The bundled publishable key can `GET`/`POST`/`PATCH`/`DELETE` existing rows; the three-word phrase is not an API control. Unapplied migration 003 limits its new membership/personal tables to Development but still treats the browser-supplied Google subject as a selector, not authentication. Through 2026-09-30 these rows are disposable Development data. Before meaningful October data, ship Google Auth, deny-by-default membership RLS, and the reviewed CAS/cutover. Existing SQL packets are not applied merely because they exist; schema, Production, and destructive cleanup remain separate approvals.
