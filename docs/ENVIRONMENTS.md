...
Deny-by-default Auth/RLS cutover **006 is applied** (2026-08-25): anon household REST is revoked; access is membership-bound via `continuity_memberships` and authenticated RPCs. Google Auth is live. Migration 003 membership/personal tables remain Development-scoped. Through 2026-09-30 Development rows are still disposable fixtures, but they are not anonymously open. Create / email / QR / revoke / anon-denial smoke and October Production hardening remain open. Migration 002 (D-122 `publish_household_snapshot`) is **applied to Development** (2026-08-25); additive Auth preparation 004, CAS hardening 005, timezone 007, and SELECT bridge 008 are applied. Empty Production was removed earlier. Do not re-paste migration SQL that still carries a historical `DO NOT APPLY` banner without Jonathan's approval.
...
The current household snapshot stores **who is linked** (email, Google subject, granted scopes), not the token. D-114 discovers Development membership through Auth JWT / continuity membership rows when available. D-117's migration 003 is applied and supplies durable identity-to-personal-ledger and identity-to-household membership outside the ledger snapshot. The D-123 client uses a refreshed Supabase user JWT behind `VITE_SUPABASE_AUTH_ENABLED=1`; preparation/hardening 004/005, cutover 006, and SELECT bridge 008 are live. Default suite services are sign-in and Calendar; extra suite access remains opt-in.
