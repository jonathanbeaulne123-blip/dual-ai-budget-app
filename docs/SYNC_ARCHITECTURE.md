...
- In the launch build, if the cloud is unreachable the cached notebook stays readable, but Personal and Shared writing wait. The outbox is crash-safe in-flight state, not permission to report a local-only cloud-backed commit.
...
2. **Cloud is durable continuity; no device is host.** In D-208/D-211 launch mode, an authenticated cloud acknowledgement is required before any cloud-backed Personal or Shared write commits locally; cached reading remains available offline.
