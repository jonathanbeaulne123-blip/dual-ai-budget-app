...
>
> **Launch policy — D-208 (2026-09-03).** Development shared writes are online-required. A reusable isolated PGlite replica validates the candidate through the real Postgres schema first; the cloud command acknowledgement is the commit and `Saved` boundary, and the active PGlite replica advances afterward. Cached books remain readable offline, but shared Confirm refuses until connectivity and matching Google Auth return. Production continuity remains off.
>
