...
>
> **Launch policy — D-208 (2026-09-03).** Development shared writes are online-required. A reusable isolated PGlite replica validates the candidate through the real Postgres schema first; the cloud command acknowledgement is the commit and `Saved` boundary, and the active PGlite replica advances afterward. A newer ambiguous acknowledgement or manual projection restore pairs Shared with the signed-in member's revisioned Personal envelope between two equal Shared-revision reads before writes reopen. Cached books remain readable offline, but shared Confirm refuses until connectivity and matching Google Auth return. Production continuity remains off.
>
...
| **013** | `continuity_command_events` append log | **T2-S1** | **Applied** Development (2026-08-27); kitchen `VITE_CONTINUITY_COMMAND_LOG=1` |
