...
function renderAsk(
  household: Household,
  extra: { today?: string; memberId?: string; onRaise?: (alternative: AskAlternative) => void } = {},
) {
  act(() => {
...
describe("Ask panel states", () => {
  ...
  it("does not move a cent when Raise it is pressed", () => {
    const household = halifaxAskHousehold();
    const before = structuredClone(household);
    const raised: AskAlternative[] = [];
    renderAsk(household, {
      onRaise: (alternative) => raised.push(alternative),
    });
    ...
    expect(raised[0]?.label).toBe("Halifax");
  });
});
...
describe("Ask placement and copy fences", () => {
  it("belongs on Jonathan's desk and never on the custodian's", () => {
    ...
    expect(officeWide).toContain("<Ask household={booksHousehold}");
    expect(officeWide).not.toMatch(/onRaise=/);
