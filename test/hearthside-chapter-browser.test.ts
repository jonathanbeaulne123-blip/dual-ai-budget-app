...
describe('Chapter controls in the actual browser components', () => {
  ...
  // The Chapter room retired as a screen (Tool Atlas K3, D3): its controls live in the Campfire ritual's beats.
  const beat = (title: string) => page.getByRole('navigation', { name: 'The five beats' }).getByRole('button', { name: new RegExp(`${title}$`) }).click();
  const member = async (id: string) => { await page.evaluate(value => window.chapterProof.member(value), id); await page.getByRole('heading', { name: 'Make Rent Boring' }).or(page.getByRole('heading', { name: 'Build Breathing Room' })).first().waitFor(); };
  const terms = () => page.locator('summary', { hasText: "This Ritual's terms and its days" }).first().click();
  it('reviews a Ritual, takes its one Task, completes it, pauses only self, and seals the exact Chapter together', async () => {
    await beat('Settle'); await terms();
    expect(await page.getByRole('button', { name: 'Prepare this occurrence' }).count()).toBe(0);
    await member('MEM-002'); await beat('Settle'); await terms(); await page.getByRole('button', { name: 'I agree to these Ritual terms' }).click();
    await member('MEM-001'); await beat('Settle'); await terms(); await page.getByRole('button', { name: 'Prepare this occurrence' }).click(); await page.getByRole('button', { name: 'Take this occurrence' }).click();
    ...
    await beat('Seal'); await page.getByRole('radio', { name: /^Established/ }).check(); await page.getByLabel('What carries forward').fill('Our Sunday tea and two-minute check.'); await page.getByRole('button', { name: 'Read the seal together' }).click();
    expect((await page.evaluate(() => window.chapterProof.household())).chapters![0]!.closure).toBeUndefined(); await page.getByRole('button', { name: 'Give my seal' }).click();
    expect((await page.evaluate(() => window.chapterProof.household())).chapters![0]!.state).toBe('open');
    await page.getByText('You sealed · waiting for').waitFor();
    await member('MEM-002'); await page.getByText('your seal is needed').waitFor(); await page.getByRole('button', { name: 'Give my seal' }).click();
    expect((await page.evaluate(() => window.chapterProof.household())).chapters![0]!.state).toBe('established'); await page.getByRole('status').filter({ hasText: 'Sealed by both of you' }).waitFor(); expect(errors).toEqual([]);
  });
  it('offers real acceptance for a Move, preserves ownership when paused, and clears drafts on scope switch', async () => {
    await beat('Settle');
    const move = page.getByRole('region', { name: 'Open Moves' }).locator('li').first(); await move.getByRole('button', { name: 'Take this task' }).click(); await move.getByRole('button', { name: 'Pause my participation' }).click();
    ...
    h = await page.evaluate(() => window.chapterProof.household()); expect(h.tasks![0]!.completedAt).not.toBeNull();
    await beat('Look ahead'); await page.locator('summary', { hasText: 'Something new for this Chapter' }).click(); await page.getByLabel('Offer a small Move').fill('Private unsaved text from this household');
    ...
    await page.getByRole('heading', { name: 'Arrive' }).waitFor();
    await beat('Look ahead'); await page.locator('summary', { hasText: 'Something new for this Chapter' }).click();
    await expect.poll(() => page.getByLabel('Offer a small Move').inputValue()).toBe('');
    await beat('Seal'); await page.getByLabel('What carries forward').fill('Only for this Chapter');
    await page.evaluate(() => { const h = window.chapterProof.household(); h.householdId = 'a-third-synthetic-house'; window.chapterProof.replace(h); });
    await beat('Seal'); await expect.poll(() => page.getByLabel('What carries forward').inputValue()).toBe('');
  ...
  it('keeps the ritual and its review and edit controls readable across all themes, widths, enlarged text and keyboard', async () => {
    await beat('Settle'); await terms(); await page.getByRole('button', { name: 'Propose a shared change' }).click();
