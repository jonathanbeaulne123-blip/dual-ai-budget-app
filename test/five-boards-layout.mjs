...
const shared={household:h,memberId:q.get('member')||'MEM-001',view:'household',today:'2026-09-08',busy:false,onCommand};
...
try{
  ...
  for(const theme of['classic','taylor','newfoundland']) {
    for(const width of[320,390,719,1100,1440]) {
      await page.setViewportSize({width,height:1000});
      await page.goto(server.resolvedUrls.local[0]+'boards-proof?theme='+theme+'&route=boards&member=MEM-002');
      await page.getByRole('tab',{name:'Shift Ask',exact:true}).click();
      const geometry=await page.evaluate(()=>({width:innerWidth,scroll:document.documentElement.scrollWidth}));
      assert.ok(geometry.scroll<=geometry.width+1,JSON.stringify({theme,width,route:'eligible-ask',...geometry}));
      records.push({theme,width,route:'eligible-ask',...geometry});
    }
    for(const route of['boards','calendar','books']) {
      await page.setViewportSize({width:1100,height:1000});
      await page.goto(server.resolvedUrls.local[0]+'boards-proof?theme='+theme+'&route='+route+'&member=MEM-002');
      await page.getByText('Fictional local component proof',{exact:true}).waitFor();
      await page.evaluate(()=>{document.documentElement.style.zoom='2';});
      const geometry=await page.evaluate(()=>({width:innerWidth,scroll:document.documentElement.scrollWidth}));
      assert.ok(geometry.scroll<=geometry.width+1,JSON.stringify({theme,route,zoom:2,...geometry}));
      records.push({theme,route,zoom:2,...geometry});
      const labels=route==='boards'?['Notes','Photos','To-do','Goals','Shift Ask']:[null];
