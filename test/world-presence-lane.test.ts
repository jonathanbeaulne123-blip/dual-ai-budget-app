...
it('withholds an incompatible peer position while keeping its presence available',async()=>{
 ...
 socket.deliver({...peer,world:'hearth-mountain-geo-2',x:99,y:51,z:-174,yaw:0,moving:true});
 socket.deliver(peer);expect(h.lane.peers()[0]!.track.samples()).toHaveLength(1);
 expect(h.lane.peers()[0]!.geoMismatch).toBe(false);
 socket.deliver({...peer,world:'hearth-mountain-2',x:99,y:51,z:-174,yaw:0,moving:true});
 expect(h.lane.peers()[0]!.geoMismatch).toBe(true);
 socket.deliver({...peer,world:'hearth-mountain-1',x:99,y:51,z:-174,yaw:0,moving:true});
 expect(h.lane.peers()).toHaveLength(1);expect(h.lane.peers()[0]!.track.samples()).toHaveLength(0);h.lane.close();
});
...
describe("the world-presence lane in the browser", () => {
  ...
  it("keeps a peer's track, and drops the peer after seconds of silence", async () => {
    ...
    socket.deliver({ type: "world-peer", memberId: "MEM-002", deviceId: "MEM-002:DEVICE-b", placeId: "court", seenAt: h.now(), world:"hearth-mountain-geo-2", y:51, x: 1, z: 2, yaw: 0.4, moving: true });
    h.tick(WORLD_STEP_MS);
    socket.deliver({ type: "world-peer", memberId: "MEM-002", deviceId: "MEM-002:DEVICE-b", placeId: "court", seenAt: h.now(), world:"hearth-mountain-geo-2", y:51, x: 2, z: 2, yaw: 0.4, moving: true });
