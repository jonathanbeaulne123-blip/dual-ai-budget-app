...
import {EDGE_OPENINGS} from '../src/harbour/mountain/roads.ts';
...
describe('the mountain road',()=>{
 ...
 it('is edge-safe: every drop beyond body height has a parapet, wall or bridge rail with collision',()=>{
  ...
  for(const line of [MOUNTAIN_ROAD_LINE,ORCHARD_LANE_LINE])for(const s of line.samples)for(const [side,kind] of [[1,s.left],[-1,s.right]] as const){
   ...
   // A departure: the edge opens where a branch deck or a path/stair leaves the road, and that surface
   // continues the road at the edge (so the opening is no wider than what leaves through it).
   const ex=s.at[0]+s.normal[0]*s.halfWidth*side,ez=s.at[2]+s.normal[2]*s.halfWidth*side;
   const departure=kind==='open'&&EDGE_OPENINGS.some(o=>o.line===line.id&&o.side===(side===1?'left':'right')&&s.s>=o.s0-.01&&s.s<=o.s1+.01&&o.corridor.some(p=>Math.hypot(p[0]-ex,p[2]-ez)<o.halfWidth+2.5));
