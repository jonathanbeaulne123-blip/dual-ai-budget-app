...
describe("Our Path page leads with the simple view (D-285)", () => {
  ...
  it("keeps one Plan Studio and its tent return across the three House surfaces", async () => {
    const h = household();
    function Studio() {
      const [draft, setDraft] = useState(0);
      const tent = useContext(PathTentContext);
      return createElement("div", null,
        createElement("button", { id: "edit-studio", onClick: () => setDraft(n => n + 1) }, `Draft ${draft}`),
        createElement("button", { id: "leave-studio", onClick: () => tent?.leaveTent() }, "Leave Studio"));
    }
    const props = { classicRoom: createElement(Studio), houseWorkCentre: createElement("p", { id: "work-centre" }, "Shared work centre") };
    await mount({ ...props, houseSurface: "studio" }, h);
    await click($("#edit-studio"));
    await mount({ ...props, houseSurface: "work" }, h);
    expect($(".path-world__island").hidden).toBe(true);
    expect($("#work-centre").textContent).toBe("Shared work centre");
...
describe("Game mode: the open world, full screen (D-285)", () => {
  ...
  it("consumes game history before following a canonical house destination", async () => {
    window.history.replaceState({}, "", "/house/kitchen-table/above?household=fictional");
    const destination = "/house/home/middle?household=fictional";
    const openDestination = vi.fn(() => window.history.pushState({}, "", destination));
    await mount({ onOpenTimeMachine: openDestination });
    await openWorld();
    // The page-owned link uses the same outbound wrapper as the world cards.
    const fund = host.querySelector<HTMLButtonElement>(".path-world__time");
    ...
    await act(async () => { window.history.back(); await new Promise(r => setTimeout(r, 40)); });
    expect(window.location.pathname).toBe("/house/kitchen-table/above");
    expect(window.history.state?.hearthPathWorld).toBeUndefined();
  });

  it("consumes game history before the house moves into its persistent Plan Studio", async () => {
    window.history.replaceState({}, "", "/house/kitchen-table/above?household=fictional");
    await mount({ houseSurface: "journey", houseWorkCentre: createElement("p", null, "Work centre"), onTentChange: (open: boolean) => {
      if (open) window.history.pushState({}, "", "/house/kitchen-table/below?household=fictional");
    } });
    await openWorld();
    await click($(".path-hud .path-world__tent"));
    await act(async () => { await new Promise(r => setTimeout(r, 40)); });
    expect(window.location.pathname).toBe("/house/kitchen-table/below");
    expect($(".path-world__room").hidden).toBe(false);
    await act(async () => { window.history.back(); await new Promise(r => setTimeout(r, 40)); });
    expect(window.location.pathname).toBe("/house/kitchen-table/above");
    expect(window.history.state?.hearthPathWorld).toBeUndefined();
