...
it('encloses rooms and passages, preserves the only named mouths, and reports roof breaches',()=>{
  ...
  for(const s of cuts.solids){const g=new BufferGeometry();g.setAttribute('position',new Float32BufferAttribute(s.positions,3));g.setIndex(s.indices);const material=new MeshBasicMaterial({side:DoubleSide}),mesh=new Mesh(g,material);mesh.updateMatrixWorld();expect(new Raycaster(eye,direction,0,150).intersectObject(mesh),`${s.id} obstructs the Deep-to-Throat sightline`).toHaveLength(0);g.dispose();material.dispose();}
