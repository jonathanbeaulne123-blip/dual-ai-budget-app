...
describe("the time machine surface", () => {
  ...
  it("offers a door from the + control that moves money nowhere", () => {
    const action = fabActionsFor("household", "ledger").find((row) => row.id === "see-a-month");
    expect(action?.money).toBe(false);
    expect(action?.kind).toBe("go");
  });
});
