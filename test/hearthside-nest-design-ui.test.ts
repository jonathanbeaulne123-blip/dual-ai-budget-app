...
beforeEach(()=>{vi.stubGlobal('crypto',webcrypto);vi.stubGlobal('IS_REACT_ACT_ENVIRONMENT',true);localStorage.clear();sessionStorage.clear();h=catalogHousehold();h.householdId='HH-nest-ui';docs=new Map();lost=false;requests=[];render=()=>{};host=document.createElement('div');document.body.append(host);root=createRoot(host);client=new HearthsideDesignClient({environment:h.environment,householdId:h.householdId,memberId:'MEM-001',identity:'fictional',token:async()=> 'synthetic',storage:localStorage,fetch:async(_url,init)=>{
 ...
 if(request.kind==='create'){d=request.nestSource?[...docs.values()].find(doc=>doc.nest?.designKey===request.nestSource.designKey)??migrateNestDesign(h,'MEM-001',request.designId,request.nestSource):docs.get(request.designId)??createKittyDesignDocument(request.designId,{environment:h.environment,householdId:h.householdId,ownerMemberId:request.audience==='personal'?'MEM-001':null});}
...
it('creates free Personal pottery with explicit owner scope and indexes only the private shelf',async()=>{
 render=()=>root.render(provider(createElement(CollaborativeStudio,{audience:'personal',household:h,memberId:'MEM-001'})));await act(async()=>render());await click('Start a piece');
 const create=requests.find(row=>row.kind==='create');expect(create).toMatchObject({audience:'personal',bankId:null});
 const personal=[...docs.values()][0]!;expect(personal.scope.ownerMemberId).toBe('MEM-001');expect(h.personalLife?.designs.map(row=>row.designId)).toEqual([personal.id]);expect(h.hearthside?.designs??[]).toEqual([]);
 expect(button('My piece 1')).toBeTruthy();expect(button('Our piece 1')).toBeUndefined();
});
it('refuses cross-audience deep links, keeps shelves partitioned, and cannot join or keep the wrong document',async()=>{
 const make=(id:string,ownerMemberId:string|null,pieceId:string)=>acceptKittyDesignOperation(createKittyDesignDocument(id,{environment:h.environment,householdId:h.householdId,ownerMemberId}),{version:1,id:`OP-${id}`,designId:id,pieceId,gestureId:`GESTURE-${id}`,kind:'create-piece',base:'cream'},{environment:h.environment,householdId:h.householdId,actorId:'MEM-001',order:1,acceptedAt:'2026-09-19T12:00:00.000Z'}).document;
 const shared=make('DESIGN-shared',null,'PIECE-shared'),personal=make('DESIGN-personal','MEM-001','PIECE-personal');docs.set(shared.id,shared);docs.set(personal.id,personal);h=applyAcceptedDesignReference(h,shared,null);h=applyAcceptedDesignReference(h,personal,null);
 const kept:string[]=[];render=()=>root.render(provider(createElement(CollaborativeStudio,{audience:'personal',household:h,memberId:'MEM-001',initialDesignId:shared.id,initialPieceId:'PIECE-shared',onKeep:id=>kept.push(id)})));await act(async()=>{render();await new Promise(resolve=>setTimeout(resolve,20));});
 expect(host.textContent).toContain('This is a shared piece. Open it from Our studio');expect(button('Unavailable in this studio').disabled).toBe(true);expect(host.querySelector('[data-stage="PIECE-shared"]')).toBeNull();expect(button('My piece 1')).toBeTruthy();expect(button('Our piece 1')).toBeUndefined();expect(kept).toEqual([]);
 await click('My piece 1');expect(host.querySelector('[data-stage="PIECE-personal"]')).toBeTruthy();await click('Join this piece');const before=requests.length;await click('Start another piece');expect(requests.slice(before).every(row=>(row.operation?.designId??row.designId)===personal.id)).toBe(true);
 render=()=>root.render(provider(createElement(CollaborativeStudio,{household:h,memberId:'MEM-001',initialDesignId:personal.id,initialPieceId:'PIECE-personal',onKeep:id=>kept.push(id)})));await act(async()=>{render();await new Promise(resolve=>setTimeout(resolve,20));});
 expect(host.textContent).toContain('This is a private piece. Open it from your Personal studio');expect(button('Unavailable in this studio').disabled).toBe(true);expect(button('Our piece 1')).toBeTruthy();expect(button('My piece 1')).toBeUndefined();expect(kept).toEqual([]);
});
