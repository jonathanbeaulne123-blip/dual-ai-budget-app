...
import { ChapterRoom, ChapterClose } from '../../src/ChapterPanel.tsx';
...
function Proof() {
  ...
  return <main style={{ maxWidth: 1080, margin: 'auto', padding: '20px 12px 180px' }}><nav aria-label="Synthetic proof controls"><p>Local synthetic Chapter proof · no account, network or payment</p><label>Current participant<select value={memberId} onChange={event => setMember(event.target.value)}>{household.members.map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select></label></nav>{error && <p role="alert">{error}</p>}<ChapterRoom household={household} memberId={memberId} today="2026-09-12" onCommand={run} busy={false} /><ChapterClose household={household} memberId={memberId} today="2026-09-12" onCommand={run} busy={false} /></main>;
}
