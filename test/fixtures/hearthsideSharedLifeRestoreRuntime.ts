import {commitSharedLifeRestore} from '../../src/hearthside/sharedLifeRestore.ts';
import type {SharedLifeRestoreOperation} from '../../src/hearthside/sharedLifeRestoreContracts.ts';
...
export async function sharedLifeRestoreHarness(householdId:string,options:{workspaceCopies?:boolean;guestCatalogue?:boolean}={}){
 const bundle=await build({stdin:{resolveDir:process.cwd(),contents:`
  import {LedgerRoom as BaseLedgerRoom} from './workers/ledgerRoom.ts';
  export class LedgerRoom extends BaseLedgerRoom {
    async checkpointForTest(scope){return this.serial(async()=>{this.hearthsideMember(scope);await this.checkpoint();await this.archiveBarrier();return JSON.parse(this.meta('restorePoints','[]'));});}
    async epochForTest(){await this.revoke(Date.now());return true;}
  }
  ...
  export default {async fetch(request,env){
   if(new URL(request.url).pathname.endsWith('/restore-test')){try{const {scope}=await authorizeRequest(request,env,'development','${householdId}'),body=await request.json(),room=env.LEDGER_ROOMS.get(env.LEDGER_ROOMS.idFromName('development/${householdId}'));return Response.json(body.kind==='checkpoint'?await room.checkpointForTest(scope):body.kind==='members'?await room.reconcileMembers(scope,body.members):await room.epochForTest());}catch(e){return Response.json({error:e.message},{status:409});}}
 ...
 async function restoreCommand(operation:SharedLifeRestoreOperation,actor='MEM-001'){
  const replica=await snapshot(actor),current=assembleHousehold(replica.shared,replica.personal,{linked:true});clearCapturedIntent(current);
  const result=commitSharedLifeRestore(current,{version:1,id:crypto.randomUUID(),scope:{environment:'development',householdId,memberId:actor},operation});
  const scope:Scope={environment:'development',householdId,memberId:actor,subject:'local:'+actor,role:'owner',aclEpoch:1,expires:Date.now()+60000};
  return commandFromCapture(capturedIntent(result.household)!,scope,crypto.randomUUID());
 }
 return {initial,post,get:(suffix:string,actor='MEM-001')=>fetch(base+path+'/'+suffix,{headers:headers(actor)}),snapshot,command,send,restoreCommand,restoreSubmit:async(operation:SharedLifeRestoreOperation,actor='MEM-001')=>send(await restoreCommand(operation,actor),actor),
  disconnect:()=>{for(const ws of sockets)ws.close();channels.clear();},bucket:()=>mf.getR2Bucket('LEDGER_ARCHIVE'),
  submit:async(operation:HearthsideOperation,actor='MEM-001')=>send(await command(operation,actor),actor),
