...
for(const which of ['home','kitty'] as const)it(`accepts existing non-v2 ${which} creation only after Final Confirm and selects the actual accepted goal`,async()=>{
 await begin(which);await click('Final Confirm');expect(calls).toHaveLength(1);expect(persisted).toHaveLength(1);expect(h.goals).toHaveLength(1);expect(h.goals[0]).toMatchObject({name:'A little time away',targetCents:12000,shared:which==='home',ownerMemberId:which==='home'?null:'MEM-001'});expect(document.querySelector('.guard')).toBeNull();expect(document.body.textContent).not.toContain('SCOPE_CLOSED');expect([...Object.keys(localStorage)].filter(k=>k.includes('bank-create'))).toEqual([]);expect(document.querySelector('.kitty-bank-tabs [aria-pressed=true]')?.textContent).toContain('A little time away');
});
it('keeps rejected non-v2 details editable and creates no bank before a new explicit confirmation',async()=>{
 reject=true;await begin();await click('Final Confirm');expect(h.goals).toEqual([]);expect(persisted).toEqual([]);expect(document.body.textContent).toContain('This creation was rejected');const rejectedId=calls[0];await click('Cancel');expect((document.querySelector('form.kitty-folio input')as HTMLInputElement).value).toBe('A little time away');reject=false;await click('Review shared bank');await click('Final Confirm');expect(calls).toHaveLength(2);expect(calls[1]).not.toBe(rejectedId);expect(h.goals).toHaveLength(1);
});
it('recovers a previously stuck legacy creation from its actual older receipt after reload without resubmission',async()=>{
 oldLabel=true;lose=true;await begin();await click('Final Confirm');expect(h.goals).toHaveLength(1);const key=Object.keys(localStorage).find(k=>k.includes('bank-create'))!,saved=JSON.parse(localStorage.getItem(key)!);delete saved.authority;localStorage.setItem(key,JSON.stringify(saved));
 await act(async()=>root.unmount());root=createRoot(host);await act(async()=>draw());await click('Build your King');await click('Check saved creation');expect(calls).toHaveLength(1);expect(persisted).toHaveLength(1);expect(h.goals).toHaveLength(1);expect(document.querySelector('.guard')).toBeNull();expect(localStorage.getItem(key)).toBeNull();
});
it('does not downgrade an attempted v2 review when its receipt service disappears',async()=>{
 await begin();const key=Object.keys(localStorage).find(k=>k.includes('bank-create'))!,saved=JSON.parse(localStorage.getItem(key)!);saved.authority='ledger-v2';saved.attempted=true;localStorage.setItem(key,JSON.stringify(saved));await act(async()=>root.unmount());root=createRoot(host);await act(async()=>draw());await click('Build your King');await click('Check saved creation');expect(calls).toEqual([]);expect(document.body.textContent).toContain('belongs to the receipt service');expect(h.goals).toEqual([]);
});
...
it('does not release a confirmation identity when an accepted write returns an uncertain false outcome',async()=>{
 uncertain=true;await begin();await click('Final Confirm');expect(h.goals).toHaveLength(1);expect(document.body.textContent).toContain('Acceptance is not confirmed');expect([...document.querySelectorAll<HTMLButtonElement>('.guard button')].find(b=>b.textContent?.trim()==='Cancel')?.disabled).toBe(true);const id=calls[0];uncertain=false;await click('Check saved creation');expect(calls).toEqual([id]);expect(persisted).toHaveLength(1);expect(document.querySelector('.guard')).toBeNull();
});
