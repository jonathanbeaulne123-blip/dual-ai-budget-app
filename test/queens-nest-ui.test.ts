...
describe("The Still Queen — two interactions, and they are different", () => {
  ...
  it("opens a peek from each bank button, and the same door again goes all the way in; the stair is the way back", async () => {
    ...
    // The room opens on this month's bill rail; a bill's own months are one step in.
    expect(cellar.querySelector(".queen-cellar-rail")).not.toBeNull();
    await click([...cellar.querySelectorAll<HTMLButtonElement>(".queen-cellar-views .queen-pick")].find((row) => /months/i.test(row.textContent ?? ""))!);
