...
it('persists private source deletion and reviewed shared copies in actual separate SQLite Agents with authoritative receipt validation',async()=>{
 ...
 const privateExperience:WorkspaceExperienceContext={version:2,audience:'personal',ownerMemberId:'MEM-A',id:'PRIVATE-experience',revision:1,title:'A quiet train trip',intention:'See the coast slowly',state:'dreaming',horizon:'season',livedOn:null};
 const bundle=await build({stdin:{resolveDir:process.cwd(),contents:`
  ...
  export class TestAuthority extends DurableObject {
    async workspaceExperience(scope,reference){if(reference?.version!==1||!['MEM-A','MEM-B'].includes(scope.memberId))throw new Error('FORBIDDEN');if(reference.audience==='household'&&reference.ownerMemberId===null&&reference.personalLifeVersion===null&&reference.id===${JSON.stringify(experience.id)})return ${JSON.stringify(experience)};if(reference.audience==='personal'&&reference.ownerMemberId===scope.memberId&&reference.personalLifeVersion===1&&reference.id===${JSON.stringify(privateExperience.id)})return {...${JSON.stringify(privateExperience)},ownerMemberId:scope.memberId};throw new Error('FORBIDDEN');}
 ...
 try{
  ...
  const personalProjectId=workspaceExperienceProjectId(scope,privateExperience.id,'personal');response=await command(personalProjectId,0,{type:'create-experience',id:personalProjectId,context:privateExperience,confirmDigest:workspaceExperienceDigest(privateExperience)});expect(response.status,await response.clone().text()).toBe(200);
  const personalProject=(await response.json()as WorkspaceSnapshot).projects.find(project=>project.id===personalProjectId);expect(personalProject?.experience?.context).toEqual(privateExperience);
