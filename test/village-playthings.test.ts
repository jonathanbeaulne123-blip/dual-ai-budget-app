...
describe("village room playthings", () => {
  it("makes a distinct, animated tactile object for every room", () => {
    ...
    for (const room of rooms) {
      ...
      expect(made.anchors()).toHaveLength(1); expect(made.regions()[0]!.box?.isEmpty()).toBe(false); expect(made.interact(`${room}-plaything`)).toBeTruthy(); expect(made.animate(1, .016)).toBe(true);
