...
it('actual local R2 conditional upload is immutable/idempotent and disabled deletion preserves existing bytes', async () => {
  ...
  try {
    ...
    expect((await mf.dispatchFetch(url, { headers, method: 'DELETE' })).status).toBe(405);
    expect((await mf.dispatchFetch(url, { headers, method: 'PUT', body })).status).toBe(200);
    const afterDelete = await mf.dispatchFetch(url, { headers });
    expect(afterDelete.status).toBe(200);
    expect(new Uint8Array(await afterDelete.arrayBuffer())).toEqual(body);
    const bucket = await mf.getR2Bucket('BOARD_MEDIA');
    expect((await bucket.head(`v1/development/HH-TEST/${mediaId}`))?.size).toBe(body.byteLength);
