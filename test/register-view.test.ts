...
describe("register drawing", () => {
  it("draws arrival order, unfunded outline, and canonical totals", () => {
    ...
    expect(Array.from(container.querySelectorAll(".register-legend-label")).map((node) => node.parentElement?.getAttribute("transform")))
      .toEqual(["translate(470,4)", "translate(570,4)", "translate(670,4)", "translate(770,4)"]);
  ...
  it("repeats the same facts in the phone list", () => {
    ...
    expect(container.querySelector(".register")?.getAttribute("aria-label")).toBe("September 2026 · the register");
    expect(container.querySelector(".register-scroll")?.getAttribute("aria-label")).toBeNull();
    expect(container.querySelector(".register-scroll")?.getAttribute("tabindex")).toBeNull();
  });
...
describe("register fences", () => {
  it("does not grow a second allocator or writer path", () => {
    ...
    expect(cssSource).toMatch(/@media \(forced-colors: active\)[\s\S]*data-register-segment="carried"[\s\S]*data-register-segment="hers"[\s\S]*data-register-segment="his"/);
