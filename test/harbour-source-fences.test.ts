...
describe("src/harbour source fences", () => {
  ...
  it("keeps every place's directory inside the same fences", () => {
    const dirs = ["court", "tower", "cellar", "glasshouse", "kitchen", "boathouse", "library", "scene", "flat", "nav", "data", "camera", "assets"];
    const seen = new Set(files.map((f) => relative(harbour, f).replace(/\\/g, "/").split("/")[0]).filter((part) => part && !part.endsWith(".ts") && !part.endsWith(".tsx")));
    for (const dir of ["court", "tower", "cellar", "glasshouse", "kitchen", "boathouse", "library", "scene", "flat"]) expect([...seen]).toContain(dir);
