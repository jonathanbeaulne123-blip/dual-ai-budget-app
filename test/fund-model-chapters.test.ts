...
describe("Chapters are calendar months (slice 8)", () => {
  it("gives a new Chapter its Toronto month and reads legacy Chapters as the month they opened", () => {
    ...
    const late = openChapter(migrated(catalogHousehold()), { memberId: ALEX, foundationId: "see-our-shared-life", at: "2026-09-01T02:00:00.000Z" }).household;
    ...
    // Flags off (an unsorted household): no month is written, so older phones are never asked to reload.
    expect(openChapterFor(openChapter(catalogHousehold(), { memberId: ALEX, foundationId: "see-our-shared-life", at: AUG }).household)!.intendedMonth).toBeUndefined();
  });
