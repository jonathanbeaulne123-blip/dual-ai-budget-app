...
describe("Prise accepted review",()=>{
 it("keeps similarity fixed through parting and cancel; only named Confirm excludes and Include remains reachable",async()=>{
  const m=await mount();try{await m.open();const signal=m.host.querySelector(".prise-signal")!.textContent;
   ...
   await act(async()=>m.button("Review exclusion · left").click());await act(async()=>m.button("Show Confirm exclusion").click());await act(async()=>m.button("Confirm exclusion").click());expect(m.calls).toHaveLength(1);expect(m.legacy).toEqual([]);expect(m.button("Include")).toBeDefined();
 ...
 it("keeps readable card fields outside the slider and cancels a drag begun on text with focused Escape",async()=>{
  const m=await mount();try{await m.open();const field=m.host.querySelector(".prise-drag-card .prise-amount")!;expect(m.handle().querySelector(".prise-field")).toBeNull();
   ...
   await act(async()=>m.button("Review exclusion · left").click());await act(async()=>m.button("Show Confirm exclusion").click());await act(async()=>m.button("Confirm exclusion").click());expect(document.activeElement).toBe(m.button("Include"));
  }finally{await m.close();}
 });
 ...
 it("revalidates inside the queued action and refuses a room round trip",async()=>{
  const m=await mount();try{let queued!:Parameters<DuplicateCommand>[0],release!:(result:{ok:boolean})=>void;m.state.props.onDuplicateCommand=fn=>{queued=fn;return new Promise(resolve=>release=resolve);};await m.render();await m.open();await act(async()=>m.button("Review exclusion · left").click());await act(async()=>m.button("Show Confirm exclusion").click());await act(async()=>m.button("Confirm exclusion").click());expect(document.querySelector(".confirm-status")?.textContent).toBe("Working…");
 ...
 it("shows rejected acceptance inside the review without changing the ledger",async()=>{
  const m=await mount();try{m.state.props.onDuplicateCommand=async()=>({ok:false,userMessage:"Ledger unavailable"});await m.render();await m.open();await act(async()=>m.button("Review exclusion · left").click());await act(async()=>m.button("Show Confirm exclusion").click());await act(async()=>m.button("Confirm exclusion").click());expect(document.querySelector("[role=dialog]")!.textContent).toContain("Ledger unavailable");expect(m.calls).toEqual([]);expect(m.button("Cancel").disabled).toBe(false);expect(document.querySelector("[role=dialog]")!.textContent).not.toContain("This review changed");expect(m.button("Show Confirm exclusion").disabled).toBe(false);
  }finally{await m.close();}
 });
});
