...
beforeEach(()=>{sessionStorage.clear();host=document.createElement('div');document.body.append(host);root=createRoot(host);Object.assign(HTMLElement.prototype,{setPointerCapture(){},releasePointerCapture(){},hasPointerCapture(){return false;}});});
...
async function trustChoice(label: string) { await act(async()=>[...host.querySelectorAll<HTMLButtonElement>(".trust-stop")].find(button => button.getAttribute("aria-label")?.startsWith(label+"."))!.click()); }
...
async function mount(h:Household,source=context(h)){await act(async()=>root.render(createElement(Reach,{household:h,memberId:MEMBER,today:TODAY,source})));await until(()=>!!host.querySelector('.reach-fields'));return source;}
...
describe("Claude's Reach consumer",()=>{
  it('keeps receipt consent separate from a fixed46 choice and preserves current Ask and accepted history geometry',async()=>{
    const {h,accountId}=fixture(), before=JSON.stringify(h), projectSpy=vi.spyOn(projection,'projectFundScenario');await mount(h);
    ...
    await contribution(`cash:${accountId}`,'46');expect(deficit()).toBe('$551.00');await trustChoice('+ Estimated');expect(deficit()).toBe('$505.00');expect(host.querySelector('[data-ask-figure]')!.textContent).toBe(ask);
    ...
    const calls=projectSpy.mock.calls.length;await trustChoice('Confirmed');expect(deficit()).toBe('$551.00');await trustChoice('+ Estimated');expect(deficit()).toBe('$505.00');expect(projectSpy.mock.calls.length).toBe(calls);
  ...
  it('allows a cash-only up-to scenario while forecast evidence is unavailable, and rejects exact over-allocation',async()=>{
    ...
    await contribution(`cash:${accountId}`,'46.01',TODAY,'available-up-to');await trustChoice('+ Estimated');expect(deficit()).toBe('$505.00');expect(host.querySelector('.reach-cone')).toBeNull();
  });
  it('rejects malformed cents and outside-horizon dates without changing accepted books',async()=>{
    const {h,accountId}=fixture(),before=JSON.stringify(h);await mount(h);await cashReceipt(accountId);await contribution(`cash:${accountId}`,'1.001');expect(host.textContent).toContain('exact CAD');expect(deficit()).toBe('$551.00');
  ...
  it('models one explicit forecast source, refuses a contribution before its receipt, and keeps cash results during effort preview',async()=>{
    ...
    await change('Dated contribution source','');await change('Dated contribution source',source);const received=field('Contribution date').value;await contribution(source,'1000',received,'available-up-to');await trustChoice('+ Estimated');
  ...
  it('removes a displayed Estimated cone immediately when the accepted source changes and explains the fallback',async()=>{
    const h=forecastReceiptHousehold();await mount(h);
    const rail=host.querySelector<HTMLInputElement>('input[type=range]')!;rail.setAttribute('aria-label','rail');await change('rail','2');await change('Receipt source','forecast');
    await act(async()=>host.querySelector<HTMLInputElement>('.reach-check input')!.click());await click('Review receipt assumption');await until(()=>!!field('Dated contribution source'));
    const source=(field('Dated contribution source') as HTMLSelectElement).options[1]!.value;
    await change('Dated contribution source',source);const received=field('Contribution date').value;await contribution(source,'1000',received,'available-up-to');await trustChoice('+ Estimated');
    expect(host.querySelector('.reach-cone')).not.toBeNull();
    const renewed = context(h,()=>true,'accepted-source-renewed');
    await act(async()=>root.render(createElement(Reach,{household:h,memberId:MEMBER,today:TODAY,source:renewed})));
    expect(host.querySelector('.reach-cone')).toBeNull();expect(host.querySelector('.reach-lift')).toBeNull();
    const stops=[...host.querySelectorAll<HTMLButtonElement>('.trust-stop')];
    expect(stops.find(b=>b.getAttribute('aria-label')?.startsWith('+ Estimated.'))!.disabled).toBe(true);
    expect(stops.find(b=>b.getAttribute('aria-label')?.startsWith('Confirmed.'))!.getAttribute('aria-pressed')).toBe('true');
    expect(host.textContent).toContain('+ Estimated is unavailable. Showing confirmed contributions and scheduled obligations.');
  });
  ...
  it('hides prior private results synchronously and rejects a delayed projection after a source change',async()=>{
    ...
    const initial=context(h,()=>true,'new');const next:ScenarioSourceContext={...initial,accepted:{...initial.accepted,ownBooks:'unavailable'}};await act(async()=>root.render(createElement(Reach,{household:h,memberId:MEMBER,today:TODAY,source:next})));expect(host.querySelector('.reach-fields')).toBeNull();
