...
describe("Little Harbour · the Court's poses (BUILD_PLAN #18)", () => {
  ...
  it("names poses that already sit inside the clamp bounds, and clamps the rest", () => {
    ...
    expect(COURT_BOUNDS).toEqual({ minR: 3, maxR: 22, minPhi: 0.25, maxPhi: 1.2, targetRadius: 9 });
    ...
    expect(clampCourtPose({ ...wild, r: 0.1, phi: 5 })).toMatchObject({ r: 3, phi: 1.2 });
