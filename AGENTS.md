...
- Time zone: books civil dates stay `America/Toronto` (D-126 Q2 C). Each phone may display another IANA clock zone. Currency: CAD, integer cents.
...
- Current hosted code (D-114/D-117/D-123): exact Google subject/email discovery and a durable compacting outbox are implemented for disposable Development snapshots. Signed-in writes replay on launch/focus/reconnect without requiring `linked: true`. Migration 003 is applied: D-117 explicit hosted membership and member-personal rows are live. Deny-by-default Auth/RLS **006 is applied** (anon household REST revoked). Migration **016** `hearth_reset_development_households` is applied (D-151; Development-only bulk reset). Missing membership tables still retain the D-114 open bridge fallback. Demo, empty, Hearth Pass, and households without a matching signed-in Google member make zero household REST calls. Legacy phrase/`linked` transport remains during migration. PGlite is the on-device books engine.
...
- Development-data window: through 2026-09-30, hosted Development information is disposable and may still be incomplete for Production readiness. Deny-by-default Auth/RLS cutover **006 is applied** (D-123): anon household REST is revoked; access is membership-bound. Live Create / invite / anon-denial smoke and October-grade Production hardening remain open. Credentials and secrets are never disposable.
- During Bianca Month (D-183), the rehearsal is a mainline projection over the ordinary App, commands, books, and continuity—not a trial fork. Changes to App routes, Add, Books, Fund, reconciliation, close, household shape/sync, command events, or PGlite must keep the full-App Bianca regression in `test/app-startup-p1.test.ts` and the command-sync contract in `test/month-rehearsal-mainline.test.ts` green. The rehearsal never duplicates a financial writer.
