...
- Current hosted code: household Supabase Postgres is still D-110 opt-in snapshot transport. Local ingest never publishes. Demo, empty, Hearth Pass, and `linked: false` households currently make zero household REST calls. PGlite is the on-device books engine.
- Target cloud continuity (D-112): Google sign-in must reveal the person's personal ledger and household memberships on any device. No phone is the host. The cloud is durable continuity; PGlite is each device's validated offline replica. Automatic post-sign-in sync replaces `linked`, phrases, Pass files, and explicit publish as the normal access gate. See `docs/CLOUD_CONTINUITY.md`.
