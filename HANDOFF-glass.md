...
2. **QuickSheet** (both mounts, `App.tsx:9210` and `:9299`), add:
   ...
   `onWorld={(a)=>{setQuickSheetOpen(false); /* step-in | skate | arrange → HarbourWorld */}}` (rows hidden without it) ·
...
Results and tsc: see the bottom of this file.
...
- Dead `.village-tools` / `.village-map` selectors remain in `village.css`'s minified head (harmless; nothing renders them).
